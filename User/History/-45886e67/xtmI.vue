...
const DISPLAY_ITEMS = [
  { value: 'numberOfNewClients', i18nKey: 'report-client-by-period.new', color: '#1976D2', isCalculated: false },
  { value: 'numberOfRevisitClients', i18nKey: 'report-client-by-period.revisit', color: '#FF9800', isCalculated: false },
  { value: 'newPlusRevisit', i18nKey: 'report.new-plus-revisit', color: '#4CAF50', isCalculated: true },
  { value: 'numberOfUnregisteredClients', i18nKey: 'report.unregistered', color: '#9C27B0', isCalculated: false },
  { value: 'totalClients', i18nKey: 'general.total', color: '#F44336', isCalculated: false },
]

const DEFAULT_DISPLAY_ITEMS = ['numberOfNewClients', 'numberOfRevisitClients', 'newPlusRevisit', 'numberOfUnregisteredClients', 'totalClients']
...
export default {
  ...
  methods: {
    getDatasetValues(item) {
      ...
      // Handle calculated field: New + Revisit
      if (item.value === 'newPlusRevisit') {
        return this.sectionData.data.map(d =>
          (d.numberOfNewClients || 0) + (d.numberOfRevisitClients || 0),
        )
      }

      return this.sectionData.data.map(d => d[item.value] || 0)
    },
  },
}
