...
export default {
  ...
  methods: {
    initializeSelectedItems() {
      // Get "Not Selected" option (staffId = 0) if exists
      const notSelectedOption = this.displayItemOptions.find(option => option.staffId === 0)
      ...
      // Include "Not Selected" option first if it exists
      const topStaffOptions = topStaffByRevenue.map(item => item.option)
      this.selectedItems = notSelectedOption
        ? [notSelectedOption, ...topStaffOptions]
        : topStaffOptions
    },
