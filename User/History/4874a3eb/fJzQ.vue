...
export default {
  ...
  computed: {
    // Get staff list from first data item, excluding staffId -1 (All) and staffId 0 (Not Selected)
    staffList() {
      if (!this.sectionData?.data?.[0]?.staffs) {
        return []
      }
      return this.sectionData.data[0].staffs.filter(staff => staff.staffId !== -1 && staff.staffId !== 0)
    },
  ...
  methods: {
    initializeSelectedItems() {
      // Calculate total revenue for each staff across all months
      const staffTotalRevenue = this.displayItemOptions.map(option => {
        const totalRevenue = this.calculateStaffTotalRevenue(option.staffId)
        return {
          ...option,
      ...
      const topStaffByRevenue = [...staffTotalRevenue]
        .sort((a, b) => b.totalRevenue - a.totalRevenue)
        .slice(0, DEFAULT_SELECTED_STAFF_COUNT)

      // Map back to original displayItemOptions format (without totalRevenue)
      this.selectedItems = topStaffByRevenue.map(staff => ({
        value:   staff.value,
        text:    staff.text,
        color:   staff.color,
        staffId: staff.staffId,
