...
export default {
  ...
  computed: {
    ...
    isDisabledActionVoice() {
      return !this.bookedResource.bookingResourceSetupId
      || this.bookedItems.length >= MAX_BOOKED_ITEMS
    },

    isDisabledActionSearchAvailableTimes() {
      return !this.bookedResource.bookingResourceSetupId
      || this.hasRepeatBooking
    },
    ...
    actionVoiceClass() {
      return [
        'action',
        'action--voice',
        {'action--disabled': this.isDisabledActionVoice},
      ]
    },
    ...
    actionSearchAvailableTimesClass() {
      return [
        ...
        {'action--disabled': this.isDisabledActionSearchAvailableTimes},
      ]
    },
  },
  ...
  methods: {
    ...
    onShowSearchAvailableTimesModal() {
      if(this.isDisabledActionSearchAvailableTimes) {
        return
      }

      CalendarEventBus.$emit('show-search-available-times-popup-from-booking-items', {
        bookedResource: this.bookedResource,
        bookingDateTs:  this.bookingDateTs,
      })
    },

    onStartVoice() {
      if(this.isDisabledActionVoice) {
        return
      }

      CalendarEventBus.$emit('show-aha-ai-booking-items-popup', {
        bookedResource: this.bookedResource,
      })
    },
  },
