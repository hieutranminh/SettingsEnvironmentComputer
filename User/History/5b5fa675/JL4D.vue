...
export default {
  ...
  mounted() {
    ...
    if (savedShowExpired !== this.includeExpiredCard) {
      this.$emit('include-expired-card-change', savedShowExpired)
    }
  },
  ...
  methods: {
    ...
    prepaidCardClass(prepaidCard) {
      return ['prepaid-card-row', {
        'prepaid-card-row--expired': prepaidCard.status === options.good_status.inactive,
      }]
    },
