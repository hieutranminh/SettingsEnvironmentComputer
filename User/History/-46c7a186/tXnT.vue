...
export default {
  ...
  computed: {
    ...
    calendarSlotClass() {
      return ['calendar-slot', {
        ...
        'calendar-slot--has-bookings': this.hasBookings,
        'calendar-slot--has-blocked':  this.hasBlockedTimes,
        'calendar-slot--reset-hover':  this.forceResetHover,
    ...
    bookings: {
      handler(newVal, oldVal) {
        ...
        this.$nextTick(() => {
          ...
          // Force reset hover state when bookings are removed
    ...
    blockedTimes: {
      handler(newVal, oldVal) {
        ...
        this.$nextTick(() => {
          ...
          // Force reset hover state when blocked times are removed
          if (oldVal?.length > 0 && newVal?.length === 0) {
            this.resetHoverState()
          }
        })
  ...
  mounted() {
    ...
    CalendarEventBus.$on('load-booking-calendar', this.handleBookingUpdated)
  ...
  beforeDestroy() {
    ...
    CalendarEventBus.$off('load-booking-calendar', this.handleBookingUpdated)
  },
  ...
  methods: {
    ...
    resetHoverState() {
      // Temporarily add a class to force CSS re-evaluation and reset hover state
      this.forceResetHover = true
      
      // Force browser to repaint by triggering a reflow
      if (this.$refs.slot) {
        // eslint-disable-next-line no-void
        void this.$refs.slot.offsetHeight
      }
      
      this.$nextTick(() => {
        this.forceResetHover = false
      })
    },

    handleBookingCancelled(cancelledBooking) {
      // Check if the cancelled booking belongs to this slot
      const hasCancelledBooking = this.bookings.some(booking => 
        booking.bookingId === cancelledBooking?.bookingId,
      )
    ...
    handleBookingUpdated() {
      // Reset hover state when booking calendar is reloaded
      this.$nextTick(() => {
        this.handleContextMenuHide()
        this.resetHoverState()
      })
