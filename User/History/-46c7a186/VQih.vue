...
export default {
  ...
  computed: {
    ...
    calendarSlotClass() {
      return ['calendar-slot', {
        ...
        'calendar-slot--no-hover': this.isHoverDisabled,
  ...
  watch: {
    isDragging: {
      handler(newVal, oldVal) {
        // Reset hover state when dragging ends (booking cancelled or drag finished)
        if (oldVal && !newVal) {
    ...
    dragBooking: {
      handler(newVal, oldVal) {
        // Reset hover state when dragBooking is cleared (booking cancelled)
        if (oldVal && !newVal) {
          this.resetHoverState()
        }
      },
  ...
  mounted() {
    ...
    // Listen for booking cancellation to reset hover state
    CalendarEventBus.$on('cancel-booking', this.handleBookingCancelled)
  ...
  beforeDestroy() {
    ...
    CalendarEventBus.$off('cancel-booking', this.handleBookingCancelled)
  },
  ...
  methods: {
    ...
    resetHoverState() {
      // Force reset hover state by disabling hover temporarily
      this.isHoverDisabled = true

      // Force browser to recalculate styles
      this.$nextTick(() => {
        if (this.$refs.slot) {
          // Force style recalculation by accessing offsetHeight
          // This forces browser to re-evaluate CSS including :hover state
          const _ = this.$refs.slot.offsetHeight

          // Trigger mouseleave event programmatically to force browser to remove :hover state
          const mouseLeaveEvent = new MouseEvent('mouseleave', {
            bubbles:    true,
            cancelable: true,
            view:       window,
          })
          this.$refs.slot.dispatchEvent(mouseLeaveEvent)

          // Also trigger mouseleave on drop-zone if it exists
          const dropZone = this.$refs.slot.querySelector('.calendar-slot__drop-zone')
          if (dropZone) {
            dropZone.dispatchEvent(new MouseEvent('mouseleave', {
              bubbles:    true,
              cancelable: true,
              view:       window,
            }))
          ...
          // Force style recalculation again after event dispatch
          const __ = this.$refs.slot.offsetHeight
        }

        // Keep hover disabled longer to ensure CSS has time to reset
        setTimeout(() => {
          this.isHoverDisabled = false
          // Force one more style recalculation after re-enabling
          if (this.$refs.slot) {
            const ___ = this.$refs.slot.offsetHeight
          }
        }, 300)
      })
    },
    ...
    handleBookingCancelled() {
      // Reset hover state when any booking is cancelled
      // Always reset hover state to prevent stuck hover after cancellation
      this.resetHoverState()
    },
    ...
    handleSlotDropZoneMouseLeave() {
      ...
      // Check if dragBooking still exists in bookingSet before removing
      // This prevents trying to remove a booking that was already cancelled
      const bookingId = this.dragBooking.bookingId || this.dragBooking.originalBookingId
      const bookingSet = store.state._calendar?.bookings?.bookingSet
      const bookingExists = bookingSet?.[bookingId] ||
        (this.dragBooking.isGhostBooking && bookingSet?.[this.dragBooking.originalBookingId])

      if (!bookingExists) {
        // If booking doesn't exist (was cancelled), cleanup drag state
        this.resetHoverState()
    ...
    handleContextMenuHide() {
      // Reset hover disabled state when mouse actually leaves
