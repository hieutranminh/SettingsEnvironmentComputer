    ...
    >
      ...
      <div class="ai-message-result-list__actions">
        <template v-if="isDesktopDevice">
          ...
          <a
            v-b-tooltip="{
              title: item.isSavedToMyMessage ? $t('aha-ai.saved-to-my-messages') : $t('aha-ai.save-to-my-messages'),
              placement: 'bottom',
            }"
            class="ai-message-result-list__save-button"
            @click.prevent="handleSaveToMyMessages(item)"
          >
            <b-icon-bookmark-fill v-if="item.isSavedToMyMessage" />
...
import { BIconTrash, BIconBookmark, BIconBookmarkFill } from 'bootstrap-vue'
