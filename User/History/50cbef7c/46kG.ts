...
import { PDF_MARGINS, PDF_TEXT_SPACING, PDF_FONT_SIZES, PDF_TEXT_RENDERING } from '../constants/pdf'
...
export const usePdfText = (): UsePdfTextReturn => {
  ...
  const getLineHeight = (pdf: jsPDF): number => {
    ...
    return fontSize * TEXT_CONSTANTS.LINE_HEIGHT_MULTIPLIER
  }
  ...
  const getAvailableHeight = (currentY: number, pageHeight: number): number => {
    ...
    return pageHeight - currentY - PDF_MARGINS.BOTTOM - TEXT_CONSTANTS.PAGE_BREAK_BUFFER
  }
  ...
  const shouldAddNewPageForLine = (yPosition: number, lineHeight: number, pageHeight: number): boolean => {
    ...
    const bottomLimit = pageHeight - PDF_MARGINS.BOTTOM - TEXT_CONSTANTS.PAGE_BREAK_BUFFER
