...
interface ITextRenderingContext {
  pdf: jsPDF
 ...
 */
export interface IUsePdfTextReturn {
  processText: (
    pdf: jsPDF,
 ...
 */
export const usePdfText = (): IUsePdfTextReturn => {
   ...
   */
  const calculateTextMetrics = (pdf: jsPDF, text: string, maxWidth: number): ITextMetrics => {
   ...
   */
  const addNewPage = (renderContext: ITextRenderingContext): number => {
  ...
  const renderTextLines = (
    renderContext: ITextRenderingContext,
    lines: string[],
    startY: number,
    lineHeight: number,
  ): number => {
   ...
   */
  const preparePdfDimensions = (
    pdf: jsPDF,
  ): { pageWidth: number; pageHeight: number; maxWidth: number } => {
  ...
  const processTextElements = (
    ...
    for (let i = 0; i < textElements.length; i++) {
      ...
      // Skip paragraph spacing for the last element
      if (i >= textElements.length - 1) continue
  ...
  const processText = async (
    ...
    try {
      ...
      const renderContext: ITextRenderingContext = {
        pdf,
        config,
        context,
        pageWidth,
      ...
      const message = parseError(error, 'Failed to process text section')
