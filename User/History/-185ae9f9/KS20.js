...
export default class SalesUtils {
  ...
  static isExpiredCard(expiry_date_ts){
    if (expiry_date_ts === options.enum_no_limit) {
      return false
    }

    // Use start of today in shop timezone to compare with expiry date
    // expiry_date_ts is end of day (23:59:59), so card is expired when today > expiry_date_ts
    const today_ts = getStartOfTimezoneDateTS(new Date(), 'day')
    return today_ts > expiry_date_ts
  }
