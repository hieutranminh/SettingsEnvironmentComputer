...
export const addPdfHeader = (doc: jsPDF, options: PrintPreviewOptions): void => {
  ...
  // Font constants
  const regularFont = 'NanumGothic-Regular'
  ...
  // Date text
  if (options.headerDate && options.headerDate.length > 0) {
    const dateText = options.headerDate.length === 1 ? `(${options.headerDate[0] || ''})` : `(${options.headerDate[0] || ''} - ${options.headerDate[1] || ''})`
  ...
  doc.setFontSize(regularFontSize)
  doc.setFont(regularFont, 'normal')
  doc.text(currentDate, pageWidth - PRINT_PREVIEW_CONFIG.DOC_MARGIN, currentY, { align: 'right' })
}
