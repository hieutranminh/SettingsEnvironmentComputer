...
export const renderPdfHeader = (options: HeaderRenderOptions): number => {
  ...
  if (headerConfig.dateRange) {
    const dateText = headerConfig.dateRange.from === headerConfig.dateRange.to ? `(${headerConfig.dateRange.from})` : `(${headerConfig.dateRange.from} - ${headerConfig.dateRange.to})`
