...
export const renderPdfHeader = (options: HeaderRenderOptions): number => {
  ...
  // Title
  doc.setFontSize(DEFAULT_PRINT_CONFIG.FONT.SIZE.TITLE)
  ...
  if (headerConfig.subtitle) {
    doc.setFontSize(DEFAULT_PRINT_CONFIG.FONT.SIZE.SUBTITLE)
  ...
  if (headerConfig.dateRange) {
    const dateText = headerConfig.dateRange.from === headerConfig.dateRange.to
      ? `(${headerConfig.dateRange.from})`
      : `(${headerConfig.dateRange.from} - ${headerConfig.dateRange.to})`

    doc.setFontSize(DEFAULT_PRINT_CONFIG.FONT.SIZE.BODY)
    doc.setFont('NanumGothic-Regular', 'normal')
  ...
  if (headerConfig.showCurrentDate !== false) {
    ...
    doc.setFontSize(DEFAULT_PRINT_CONFIG.FONT.SIZE.SMALL)
    doc.setFont('NanumGothic-Regular', 'normal')
