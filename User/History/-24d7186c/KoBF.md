...
export const usePdfPageManager = () => {
  const addNewPageWithHeader = (pdf: jsPDF, config: Required<PdfConfig>, context: PdfProcessingContext): number => {
    pdf.addPage()
  ...
  const needsPageBreak = (
    ...
    return requiredHeight > availableHeight && currentY > minSpaceFromHeader
  }
