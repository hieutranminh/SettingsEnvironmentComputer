...
// Constants
import { CHART_DISPLAY_TYPE, CHART_DISPLAY_VALUE, PRINT_TYPE, REPORT_BY_TYPE } from '@/constants'
// Types
import type { IPrintSection } from '@/types/print'
import type { IServiceSalesReportItem } from '@/types/sales-report/ServiceSales'
import type { ChartDisplayType, ChartDisplayValue, ReportByType } from '@/constants'
...
// Composables
const { t } = useI18n()
const { formatNameWithNoInput, formatDateOfWeek, formatHourToRange } = useFormat()
...
const handleDisplayReportByKey = (key: string): string => {
  switch (props.reportByType) {
    case REPORT_BY_TYPE.DATE_OF_WEEK:
      return formatDateOfWeek(key)
    case REPORT_BY_TYPE.HOUR_OF_DAY:
      return formatHourToRange(key)
    default:
      return formatNameWithNoInput(key)
  }
}
