...
const prepaidGoodsModel = computed({
  get: () => {
    switch (props.modelValue.reportByType) {
      case REPORT_BY_TYPE_IN_PREPAID_GOODS_REPURCHASE.ALL:
        return [REPORT_BY_TYPE_IN_PREPAID_GOODS_REPURCHASE.CARD, REPORT_BY_TYPE_IN_PREPAID_GOODS_REPURCHASE.SERVICE]
      case REPORT_BY_TYPE_IN_PREPAID_GOODS_REPURCHASE.SERVICE:
        return [REPORT_BY_TYPE_IN_PREPAID_GOODS_REPURCHASE.SERVICE]
      case REPORT_BY_TYPE_IN_PREPAID_GOODS_REPURCHASE.CARD:
  ...
  set: (value) => {
    ...
    if (value.length === 2) {
      emitUpdateModelValue({
        reportByType: REPORT_BY_TYPE_IN_PREPAID_GOODS_REPURCHASE.ALL,
      })
    } else if (value.length === 1) {
      emitUpdateModelValue({
        reportByType:
          value[0] === REPORT_BY_TYPE_IN_PREPAID_GOODS_REPURCHASE.CARD
            ? REPORT_BY_TYPE_IN_PREPAID_GOODS_REPURCHASE.CARD
            : REPORT_BY_TYPE_IN_PREPAID_GOODS_REPURCHASE.SERVICE,
      })
    } else {
      reportType.value = REPORT_BY_TYPE_IN_PREPAID_GOODS_REPURCHASE.NONE
    }
  },
})
