...
export default {
  ...
  computed: {
    ...
    maximum_excel_table_length() {
      ...
      // When only text sections (e.g. title only), no table → length 0; avoid 0 and empty-array -Infinity for Excel
      return excel_table_length_numbers.length ? Math.max(1, ...excel_table_length_numbers) : 1
    },
  ...
  methods: {
    ...
    async generateReportContentAsync() {
      // Wait for all pending Vue DOM updates to complete before capturing elements
      await this.$nextTick()
      ...
      return new Promise(async (resolve, reject) => {
        try {
          ...
          for (let print_section of this.print_sections) {
            ...
            const enableWordWrap = print_section?.enableWordWrap || false
            const customFontSize = print_section?.fontSize || 12
            const hangingIndent = print_section?.hangingIndent || 0
            const drawBorder = print_section?.drawBorder || false
            const borderPadding = print_section?.borderPadding || 10
            const fontWeight = print_section?.fontWeight || 'normal'
            const marginBottom = print_section?.marginBottom ?? spaceSize
            ...
            await new Promise((resolve, reject) => {
              try {
                ...
                if (is_table_ref_type) {
                  ...
                  const ignoreClasses = print_section.ignoreElementsClasses || []
                  const removeBorder = print_section.removeBorder || false
                  const html2canvasOptions = {
                    width:   section_ref.clientWidth,
                    // Fix unexpected white space on top of image
                    scrollX: 0,
                    scrollY: -window.scrollY || -window.pageYOffset,
                    // Fix for unsupported oklch color function
                    onclone: (clonedDoc) => {
                      // Replace oklch colors with fallback colors in cloned document
                      ...
                      // Hide elements with classes in ignoreElementsClasses
                      if (ignoreClasses.length > 0) {
                        ignoreClasses.forEach(className => {
                          const elementsToHide = clonedDoc.querySelectorAll(`.${className}`)
                      ...
                      // Remove border from the captured element
                      if (removeBorder) {
                        const clonedEl = clonedDoc.querySelector('[data-print-remove-border]')
                        if (clonedEl) {
                  ...
                  // Mark element for border removal before cloning
                  if (removeBorder) {
                    section_ref.setAttribute('data-print-remove-border', '')
                  }
                  ...
                  html2canvas(section_ref, html2canvasOptions).then(canvas => {
                    ...
                    if (removeBorder) {
                      section_ref.removeAttribute('data-print-remove-border')
                    }
                  ...
                  this.doc.setFontSize(customFontSize)
                  this.doc.setFont('NanumGothic', fontWeight)

                  // Word wrap support
                  if (enableWordWrap) {
                    const textPadding = drawBorder ? borderPadding : 0
                    const textStartX = this.doc_padding + textPadding
                    const maxWidth = this.doc_body_width - (textPadding * 2)
                    const lineHeight = customFontSize * 1.4 // Line height ~1.4x font size for readability

                    // Split text by newlines first (for bullet points)
                    const textContent = Array.isArray(print_section.section_excel_text)
                      ? print_section.section_excel_text.map(item => item.text || '').join('')
                      : print_section.section_excel_text
                    const paragraphs = textContent.split('\n')

                    // const paragraphs = print_section.section_excel_text.split('\n')
                    const allLines = []
                    ...
                    paragraphs.forEach(paragraph => {
                      ...
                      const isBulletPoint = paragraph.startsWith('•')
                      const firstLineMaxWidth = maxWidth
                      const continuationMaxWidth = maxWidth - hangingIndent

                      if (isBulletPoint && hangingIndent > 0) {
                        // First line with bullet
                        const firstLineLines = this.doc.splitTextToSize(paragraph, firstLineMaxWidth)

                        if (firstLineLines.length > 0) {
                          allLines.push({ text: firstLineLines[0], indent: 0 })

                          // Continuation lines with hanging indent
                          if (firstLineLines.length > 1) {
                            const remainingText = firstLineLines.slice(1).join(' ')
                            const continuationLines = this.doc.splitTextToSize(remainingText, continuationMaxWidth)
                            continuationLines.forEach(line => {
                              allLines.push({ text: line, indent: hangingIndent })
                            })
                          }
                        }
                      } else {
                        // Regular text without hanging indent
                        const lines = this.doc.splitTextToSize(paragraph, firstLineMaxWidth)
                        lines.forEach(line => {
                          allLines.push({ text: line, indent: 0 })
                        })
                      }
                    })

                    const borderStartY = current_start_y_position + spaceSize
                    const textStartY = borderStartY + textPadding + (customFontSize * 0.8)

                    // Border drawing helper
                    const drawBorderRect = (startY, endY) => {
                      if (!drawBorder) return
                      const borderX = this.doc_padding
                      const borderWidth = this.doc_body_width
                      const borderHeight = endY - startY + textPadding

                      this.doc.setDrawColor(200, 200, 200)
                      this.doc.setLineWidth(0.5)
                      this.doc.rect(borderX, startY, borderWidth, borderHeight)
                    }

                    let currentTextY = textStartY
                    let pageBorderStartY = borderStartY // Track border start for current page

                    allLines.forEach((lineObj, lineIndex) => {
                      // Check if need new page before drawing
                      if (currentTextY + lineHeight > this.doc_body_height - this.docBottomSpacing) {
                        // Draw border for current page before switching
                        drawBorderRect(pageBorderStartY, currentTextY)

                        current_page += 1
                        this.addNewPage(current_page)
                        current_start_y_position = init_start_y_position

                        // Reset Y position and border start for new page
                        pageBorderStartY = init_start_y_position + spaceSize
                        currentTextY = pageBorderStartY + textPadding + (customFontSize * 0.8)
                      }

                      this.doc.text(lineObj.text, textStartX + lineObj.indent, currentTextY, { align: print_section?.alignment || 'left' })
                      currentTextY += lineHeight
                    })

                    // Draw border for the last page
                    drawBorderRect(pageBorderStartY, currentTextY)

                    // Update position based on where we ended
                    current_start_y_position = currentTextY + (drawBorder ? textPadding : 0) + marginBottom - spaceSize
                  } else {
                    // Original logic without word wrap
                    const textContent = Array.isArray(print_section.section_excel_text)
                      ? print_section.section_excel_text.map(item => item.text || '').join('')
                      : print_section.section_excel_text
                    const textParts = textContent.split(/(\d{1,3}(?:,\d{3})*)/g)
                    ...
                    current_start_y_position += this.doc.getTextDimensions(print_section.section_excel_text).h + marginBottom - spaceSize
                  }
    ...
    async saveReportAsExcel() {
      ...
      // Add print date at the bottom (safe when report has only title, no table)
      const dateColCount = Math.max(0, this.maximum_excel_table_length - 1)
      const output_date_row = [...new Array(dateColCount), this.print_date]
    ...
    setChartSectionToExcel(wb, ws, canvas, row_add_canvas, canvas_column_span = null) {
      const image = wb.addImage({
        base64:    canvas.toDataURL('image/png'),
        extension: 'png',
      })

      // Use canvas_column_span if provided, otherwise use maximum_excel_table_length
      const columnSpan = canvas_column_span || this.maximum_excel_table_length
      let canvasWidth = columnSpan * (ws.properties.defaultColWidth * 7) // convert to px
      ...
      // Return the canvas height in points for row calculation
      return 3 / 4 * canvasHeight
    },
    ...
    setTextSectionToExcel(ws, section_excel_text, text_align = 'center', enableWordWrap = false, fontWeight = 'normal', fontSize = 11) {
      const ws_row = ws.addRow([section_excel_text])
      const tmp_cell_address = `${excel_cells[0]}${ws_row.number}`
      const lastColIndex = Math.max(0, this.maximum_excel_table_length - 1)
      ws.mergeCells(`${tmp_cell_address}:${excel_cells[lastColIndex]}${ws_row.number}`)
      ...
      // Set font style
      ws.getCell(tmp_cell_address).font = {
        bold: fontWeight === 'bold',
        size: fontSize,
      }

      // Auto-adjust row height for wrapped text
      if (enableWordWrap && section_excel_text && typeof section_excel_text === 'string') {
        const lineCount = (section_excel_text.match(/\n/g) || []).length + 1
        const estimatedLines = Math.max(lineCount, Math.ceil(section_excel_text.length / 100))
        ws_row.height = Math.max(20, estimatedLines * 18)
      }
    ...
    setPrintSectionsToExcel(wb, ws) {
      ...
      this.print_sections.forEach(print_section => {
        ...
        if (ref_type === print_options.print_ref_type.canvas) {
          ws.addRow() // Add spacing row before canvas
          row_set_canvas = ws.rowCount + 1
          const canvas_column_span = print_section.canvas_column_span || null
          this.setChartSectionToExcel(wb, ws, this.canvas_images[index], row_set_canvas, canvas_column_span)
          // Sync row_set_canvas to position after canvas
          row_set_canvas = ws.rowCount + 1
        }

        if (ref_type === print_options.print_ref_type.text) {
          // Add spacing before text (1 row)
          ws.addRow()

          // Use larger font for titles (when fontWeight is bold)
          const excelFontSize = print_section.fontWeight === 'bold' ? 14 : (print_section.fontSize || 11)

          this.setTextSectionToExcel(
            ws,
            print_section.section_excel_text,
            print_section.alignment,
            print_section.enableWordWrap,
            print_section.fontWeight || 'normal',
            excelFontSize,
          )

          // Skip spacing after if marginBottom is small (title case)
          const isTitle = print_section.marginBottom !== undefined && print_section.marginBottom < 5
          if (!isTitle) {
