...
import type {
  ExcelCellTracker,
  ExcelProcessingContext,
  ExcelTextFormatState,
  HtmlToExcelResult,
  PdfConfig,
  PrintPreviewState,
  PrintSection,
...
export const usePrintPreviewStore = defineStore('printPreview', () => {
  ...
  const createCellTracker = (): ExcelCellTracker => {
  ...
  const processTableHeaders = (
    context: ExcelProcessingContext,
  ...
  const processTableToExcel = (
    ...
    const context: ExcelProcessingContext = {
      worksheet,
  ...
  const createHtmlTagRegex = (): RegExp => /<(\/?)(b|i|strong|em)>/gi
   ...
   */
  const createRichTextPart = (text: string, formatState: ExcelTextFormatState): ExcelJS.RichText => ({
    text,
    font: {
  ...
  const updateFormatting = (
    tagName: string,
    isClosing: boolean,
    currentState: ExcelTextFormatState,
  ): ExcelTextFormatState => {
   ...
   */
  const addTextIfExists = (
    text: string,
    formatState: ExcelTextFormatState,
    richTextParts: ExcelJS.RichText[],
  ): void => {
    if (text) {
   ...
   */
  const parseHtmlToRichText = (htmlString: string): HtmlToExcelResult => {
    ...
    let lastIndex = 0
    let formatState: ExcelTextFormatState = { isBold: false, isItalic: false }
  ...
  // Constants for PDF text rendering
  const PDF_TEXT_PATTERNS = {
    BOLD_TAG_REGEX: /(<b>.*?<\/b>)/g,
    BOLD_TAG_CLEANER: /<\/?b>/g,
  } as const
  ...
  /**
   * Parses mixed text with bold tags into structured text parts
   * @param text - Text containing <b></b> tags for bold formatting
   * @returns Array of text parts with bold flag
   * @example
   * // Input: "Hello <b>World</b>!"
   * // Output: [{content: "Hello ", isBold: false}, {content: "World", isBold: true}, {content: "!", isBold: false}]
   */
  const parseTextParts = (text: string): TextPart[] => {
    const parts = text.split(PDF_TEXT_PATTERNS.BOLD_TAG_REGEX)
    ...
    return parts
      ...
      .map((part) => {
        if (part.startsWith('<b>') && part.endsWith('</b>')) {
          return {
            content: part.replace(PDF_TEXT_PATTERNS.BOLD_TAG_CLEANER, ''),
            isBold: true,
          }
        }
        return {
  ...
  /**
   * Saves current PDF font state for restoration later
   * @param pdf - jsPDF instance
   * @returns Font state object
   * @example
   * // Input: pdf with Arial 12pt normal font
   * // Output: {fontName: "Arial", fontStyle: "normal", fontSize: 12}
   */
  ...
  /**
   * Restores PDF font state from saved state
   * @param pdf - jsPDF instance to restore font on
   * @param fontState - Previously saved font state
   * @example
   * // Input: pdf, {fontName: "Arial", fontStyle: "normal", fontSize: 12}
   * // Output: PDF font restored to Arial 12pt normal
   */
  const restoreFontState = (pdf: jsPDF, fontState: FontState): void => {
    pdf.setFontSize(fontState.fontSize)
    pdf.setFont(fontState.fontName, fontState.fontStyle)
  }

  /**
   * Renders a single text part with appropriate font styling
   * @param pdf - jsPDF instance
   * @param textPart - Text part with content and formatting info
   * @param x - X coordinate for text positioning
   * @param y - Y coordinate for text positioning
   * @param options - Text alignment options
   * @returns New X position after rendering the text part
   * @example
   * // Input: pdf, {content: "Hello", isBold: true}, x=10, y=20
   * // Output: 25 (if text width was 15)
   */
  const renderTextPart = (
    ...
    // Set font based on text formatting
    if (textPart.isBold) {
      pdf.setFont(FONTS.BOLD, 'bold')
    } else {
      pdf.setFont(FONTS.REGULAR, 'normal')
    ...
    // Render text and calculate new position
    pdf.text(textPart.content, x, y, options)
    return x + pdf.getTextWidth(textPart.content)
  }

  /**
   * Renders mixed text with bold formatting support for PDF
   * Handles text containing <b></b> tags by applying appropriate font styles
   * @param pdf - jsPDF instance for rendering
   * @param text - Text containing <b></b> tags for bold formatting  
   * @param x - X coordinate for text start position
   * @param y - Y coordinate for text baseline
   * @param options - Text alignment options
   * @returns Final X position after rendering all text
   * @example
   * // Input: pdf, "Hello <b>World</b>!", x=10, y=20
   * // Output: 45 (final X position after rendering)
   */
  const renderMixedText = (
    ...
    const originalFontState = saveFontState(pdf)
    
    // Parse text into structured parts
    const textParts = parseTextParts(text)
    
    // Render each text part sequentially
    let currentX = x
    textParts.forEach((textPart) => {
      currentX = renderTextPart(pdf, textPart, currentX, y, options)
    })

    // Restore original font settings
    restoreFontState(pdf, originalFontState)
