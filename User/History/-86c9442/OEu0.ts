...
export const usePrintPreviewStore = defineStore('printPreview', () => {
  ...
  const HTML_TAG_REGEX = /<(\/?)([bi]|strong|em)>/gi
  ...
  const updateFormatting = (
    tagName: string,
    isClosing: boolean,
    currentState: FormatState,
  ): FormatState => {
    const { isBold, isItalic } = currentState
    const normalizedTag = tagName.toLowerCase()

    if (HTML_TAGS.BOLD.includes(normalizedTag as any)) {
      return { isBold: !isClosing, isItalic }
    }
    
    if (HTML_TAGS.ITALIC.includes(normalizedTag as any)) {
  ...
  const parseHtmlToRichText = (
    ...
    const htmlRegex = /<(\/?)([bi]|strong|em)>/gi
    let lastIndex = 0
    let formatState = { isBold: false, isItalic: false }
    ...
    while ((match = htmlRegex.exec(htmlString)) !== null) {
      ...
      if (textBefore) {
        richTextParts.push(createRichTextPart(textBefore, formatState.isBold, formatState.isItalic))
      }
    ...
    if (remainingText) {
      richTextParts.push(createRichTextPart(remainingText, formatState.isBold, formatState.isItalic))
    }
