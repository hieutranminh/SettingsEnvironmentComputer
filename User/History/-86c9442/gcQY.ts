...
export const usePrintPreviewStore = defineStore('printPreview', () => {
  ...
  const renderMixedText = (
    ...
    parts.forEach((part) => {
      const isBold =
        (part.startsWith('<b>') && part.endsWith('</b>')) || (part.startsWith('<strong>') && part.endsWith('</strong>'))

      const isItalic =
        (part.startsWith('<i>') && part.endsWith('</i>')) || (part.startsWith('<em>') && part.endsWith('</em>'))

      const content = part.replace(/<\/?(b|strong|i|em)>/g, '')
      if (!content) return

      pdf.setFont(FONTS.BOLD, 'italic')
      console.log('isItalic')

      // if (isBold && isItalic) {
      //   pdf.setFont(FONTS.BOLD, 'italic') // cần có font italic hoặc bold-italic
      // } else if (isBold) {
      //   pdf.setFont(FONTS.BOLD, 'bold')
      // } else if (isItalic) {
      //   pdf.setFont(FONTS.REGULAR, 'italic')
      // } else {
      //   pdf.setFont(FONTS.REGULAR, 'normal')
      // }

      pdf.text(content, currentX, y, options)
