...
export const usePrintPreviewStore = defineStore('printPreview', () => {
  ...
  const renderMixedText = (
    ...
    parts.forEach((part) => {
      ...
      const isItalic = part.startsWith('<i>') && part.endsWith('</i>')
      const content = part.replace(/<\/?b>|<\/?i>/g, '').trim()
      if (!content) return

      pdf.setFont(isBold ? FONTS.BOLD : FONTS.REGULAR, isItalic ? 'italic' : 'normal')
