...
// Types
import type { ExcelCellTracker, ExcelProcessingContext, ExcelTextFormatState, HtmlToExcelResult, PdfConfig, PrintPreviewState, PrintSection } from '@/types/print'
// Utils
...
export const usePrintPreviewStore = defineStore('printPreview', () => {
   ...
   * @returns The current row in the worksheet
   */
  const processTableBody = (context: ExcelProcessingContext, tableElement: HTMLTableElement, startRow: number): number => {
    let currentRow = startRow
   ...
   * @returns The current row in the worksheet
   */
  const processTableFooter = (context: ExcelProcessingContext, tableElement: HTMLTableElement, startRow: number): number => {
    let currentRow = startRow
   ...
   */
  const updateFormatting = (tagName: string, isClosing: boolean, currentState: ExcelTextFormatState): ExcelTextFormatState => {
   ...
   */
  const addTextIfExists = (text: string, formatState: ExcelTextFormatState, richTextParts: ExcelJS.RichText[]): void => {
    if (text) {
