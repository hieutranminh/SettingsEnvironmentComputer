...
export default {
  ...
  computed: {
    ...
    moreOptions() {
      const baseOptions = this.estimatedTimeOptions.slice(6)
      if (this.customTimeOption) {
        return [...baseOptions, this.customTimeOption]
      }
      return baseOptions
    },
    ...
    customTimeOption() {
      if (this.value > 600) {
        return {
          text:  this.formatCustomTime(this.value),
          value: this.value,
        }
      }
      return null
    ...
    selectedMoreOptionText() {
      ...
      if (this.customTimeOption && this.selectedValue === this.customTimeOption.value) {
        return this.customTimeOption.text
      }
  ...
  methods: {
    ...
    formatCustomTime(minutes) {
      const hours = Math.floor(minutes / 60)
      const remainingMinutes = minutes % 60
      if (remainingMinutes === 0) {
        return `${hours} ${this.$t('aha-ai.hours')}`
      }
      return `${hours} ${this.$t('aha-ai.hours')} ${remainingMinutes} ${this.$t('aha-ai.minutes')}`
    },
  },
}
</script>
