...
export const useDateRangeFilter = <T extends DateRangeFilterBase>(filters: Ref<T>, options: DateRangeOptions = {}) => {
  ...
  const { defaultDateType = DATE_TYPE.DATE, autoUpdateOnDateTypeChange = true, timezone = TIMEZONE_TYPE.UTC } = options
   ...
   * ```
   */
  const validateDateRange = (options: DateRangeValidationOptions = {}): DateRangeValidationResult => {
    ...
    if (!filters.value.fromDateTs || !filters.value.toDateTs) {
      return {
        ...
        errorContext: 'Both from and to dates are required',
      }
    }
    ...
    if (checkToDateNotBeforeFromDate && validateToDateNotBeforeFromDate(fromDate, toDate)) {
      return {
        ...
        errorContext: 'To date cannot be before from date',
      }
    }
    ...
    if (checkExceedOneMonthRange && validateExceedOneMonthRange(fromDate, toDate)) {
      return {
        ...
        errorContext: 'Date range cannot exceed one month',
      }
    }
   ...
   * ```
   */
  const validateRangeTypeFilters = (): DateRangeValidationResult => {
    if (filters.value.dateType !== DATE_TYPE.RANGE) {
      return { isValid: true }
    }
