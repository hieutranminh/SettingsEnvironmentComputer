...
export const useDateRangeFilter = <T extends DateRangeFilterBase>(filters: Ref<T>, options: DateRangeOptions = {}) => {
  ...
  const getDateRangeForDateType = (): DateRangeValues => {
    const startOfDay = getStartOf(getCurrentDate(), PERIOD_TYPES.DAY).toDate()
    const endOfDay = getEndOf(getCurrentDate(), PERIOD_TYPES.DAY).toDate()
  ...
  const getDateRangeForMonthType = (): DateRangeValues => {
    const startOfMonth = getStartOf(getCurrentDate(), PERIOD_TYPES.MONTH).toDate()
    const endOfMonth = getEndOf(getCurrentDate(), PERIOD_TYPES.MONTH).toDate()
