...
export interface IDateRangeOptions {
  ...
  savedFilters?: Ref<IDateRangeFilterBase | null>
}
...
export interface IDateRangeValidationOptions {
  ...
  /** Whether to check if date range exceeds three month */
  checkExceedThreeMonthRange?: boolean
  /** Whether to check if date range exceeds one year */
  checkExceedOneYearRange?: boolean
}

export interface IUseDateRangeFilterReturn {
  dateRangeText: ComputedRef<string>
  getDefaultDateRange: (dateType: DateType) => IDateRangeValues
  getDateRangeForDateType: () => IDateRangeValues
  getDateRangeForMonthType: () => IDateRangeValues
  getDateRangeForRangeType: () => IDateRangeValues
  ...
  validateRangeTypeFilters: () => IDateRangeValidationResult
  watchDateTypeChanges: () => void
}
 ...
 * ```
 */
export const useDateRangeFilter = <T extends IDateRangeFilterBase>(
  filters: Ref<T>,
  options: IDateRangeOptions = {},
): IUseDateRangeFilterReturn => {
  ...
  // Composables
  const { formatDate, getCurrentDate } = useDateFormat()
   ...
   */
  const toDateWithLocalTime = (dayjsDate: Dayjs): Date => {
    return new Date(
      dayjsDate.year(),
      dayjsDate.month(),
      dayjsDate.date(),
  ...
  const getDateRangeForDateType = (): IDateRangeValues => {
    ...
    return createDateRange(toDateWithLocalTime(startOfDay), toDateWithLocalTime(endOfDay))
  }
  ...
  const getDateRangeForMonthType = (): IDateRangeValues => {
    ...
    return createDateRange(toDateWithLocalTime(startOfMonth), toDateWithLocalTime(endOfMonth))
  }
  ...
  const getDateRangeForRangeType = (): IDateRangeValues => {
    ...
    return createDateRange(toDateWithLocalTime(startOfMonth), toDateWithLocalTime(endOfDay))
  }
  ...
  const dateRangeText = computed((): string => {
    ...
    const fromDate = fromUnixTimestamp(savedFiltersValue.fromDateTs)
    const toDate = fromUnixTimestamp(savedFiltersValue.toDateTs)

    switch (savedFiltersValue.dateType) {
      case DATE_TYPE.DATE:
  ...
  const validateDateRange = (
    ...
    const {
      ...
      checkExceedThreeMonthRange = false,
    ...
    // Check if date range exceeds three month
    if (checkExceedThreeMonthRange && validateExceedThreeMonthRange(fromDate, toDate)) {
      return {
        isValid: false,
        errorKey: 'general.validate-date-range-exceed-three-month',
      }
    }
  ...
  /**
   * Validate date range for RANGE type specifically
   * Applies common validation rules used across report components
   * @returns Validation result
   *
   * @example
   * ```typescript
   * if (filters.value.dateType === DATE_TYPE.RANGE) {
   *   const result = validateRangeTypeFilters()
   *   if (!result.isValid) {
   ...
   * ```
   */
  const validateRangeTypeFilters = (): IDateRangeValidationResult => {
    if (filters.value.dateType !== DATE_TYPE.RANGE) {
      return { isValid: true }
    }
