...
const { loginByKey, simpleClearAllCache, setLanguage } = useAuth()
...
onMounted(() => {
  ...
  const countryParam = route.query.country as string
  const cultureParam = route.query.culture as string
  country.value = countryParam || getCountryFromCulture(cultureParam) || DEFAULT_VALUES.COUNTRY

  authStore.updateLoginCountry(country.value)
  authStore.updateLoginPath('')
  setLanguage(country.value)

  const loginKey = (route.query.key as string) || (route.params.key as string)
  if (loginKey && loginKey.length > 1) {
    language.value = DEFAULT_VALUES.LANGUAGE
