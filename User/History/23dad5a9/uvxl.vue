...
export default {
  ...
  computed: {
    ...
    formatMultiBookingDate() {
      if (!this.sortedBookingDates.length) return ''

      // Convert timestamps to dates
      const dates = this.sortedBookingDates.map(timestamp =>
    ...
    sortedBookingDates() {
      ...
      return this.navigableBookingDates
    },
    ...
    isShowBookingDate() {
      return this.currentFilters?.isPeriodSelection
        ...
        : this.sortedBookingDates.length > 1
    },
    ...
    canNavigatePrevious() {
      return this.sortedBookingDates.length > 0 && this.currentDateIndex > 0
    },

    canNavigateNext() {
      return this.sortedBookingDates.length > 0
        && this.currentDateIndex < this.sortedBookingDates.length - 1
    },
  },
  ...
  methods: {
    ...
    initializeDateIndex() {
      ...
      if (this.resultData?.bookingDateTS && this.sortedBookingDates.length) {
        const index = this.sortedBookingDates.findIndex(date => date === this.resultData.bookingDateTS)
      ...
      if (this.currentDateIndex >= this.sortedBookingDates.length) {
        this.currentDateIndex = Math.max(0, this.sortedBookingDates.length - 1)
      }
    },
    ...
    onSelectAgain() {
      this.$emit('select-again-search-available-times-popup')
    },
    ...
    async onSelectAvailableTime(resource, slot) {
      ...
      const bookingActionModal = document.querySelector('.booking-action__modal.show')
      if (this.isSearchAvailableTimesFromBookingItems && this.selectedBookedResource) {
        ...
        CalendarEventBus.$emit('close-client-information-when-select-available-time')
        await this.openBookingActionBySlot(selectedSlotData)
    ...
    async updateBookedResourceFromSlot(selectedSlotData) {
      try {
        ...
        // Add booked items from search available times if available
        await this.addBookedItemsToUpdatedBookedResource()
      } catch (error) {
        this._showDialogAlert(error.message)
      }
    },

    async addBookedItemsToUpdatedBookedResource() {
      if (!this.searchAvailableTimesItemsData) {
        return
      }
      ...
      // Use selectedBookedResource since it's already been updated
      if (!this.selectedBookedResource) {
        return
      }

      // Clone the booked resource and replace bookedItems (not add)
      const clonedBookedResource = this.selectedBookedResource.clone()

      // Replace bookedItems with new items (set guid and isNew)
      clonedBookedResource.bookedItems = bookedItems.map(bookedItem => {
        bookedItem.setIsNew(true)
        bookedItem.bookedItemId = guid()
        return bookedItem
      })

      // Update estimatedTime based on new bookedItems
      clonedBookedResource.estimatedTime = clonedBookedResource.bookedItemsEstimatedTime

      await this.updateBookedResource(clonedBookedResource)
    },
  },
}
</script>
