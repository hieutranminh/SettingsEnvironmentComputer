...
export default {
  ...
  methods: {
    ...
    onShow() {
      ...
      // If queryParametersFromAhaAi is provided, fetch available times using it
      if (this.queryParametersFromAhaAi) {
        this.onFetchAvailableTimesFromAhaAi()
      }
    },

    async onFetchAvailableTimesFromAhaAi() {
      try {
        this.preLoader()

        const response = await getAvailableTimes(this.queryParametersFromAhaAi)
