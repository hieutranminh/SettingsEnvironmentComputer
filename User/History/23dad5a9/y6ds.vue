...
export default {
  ...
  methods: {
    ...
    async addBookedItemsFromSearchAvailableTimes(fallbackEstimatedTime) {
      ...
      const totalItemsEstimatedTime = bookedItems.reduce((total, item) => total + item.getEstimatedTime(), 0)
      const usesFallbackEstimatedTime = totalItemsEstimatedTime === 0 && fallbackEstimatedTime > 0

      const targetResource = usesFallbackEstimatedTime ? this.cloneResourceWithFallbackEstimatedTime(bookedResource, fallbackEstimatedTime) : bookedResource

      await this.addBookedItemsToBookedResource({
        bookedItems,
        bookedResource: targetResource,
      })
    },

    cloneResourceWithFallbackEstimatedTime(bookedResource, fallbackEstimatedTime) {
      const clonedResource = bookedResource.clone()
      clonedResource.setDefaultEstimatedTime(fallbackEstimatedTime)
