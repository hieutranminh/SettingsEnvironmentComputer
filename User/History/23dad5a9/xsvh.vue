...
import { convertTimeStampToDate, formatDate } from 'CommonHelpers'
  ...
  methods: {
    ...
    async onSelectAvailableTime(resource, slot) {
      const selectedSlotData = {
        ...
        bookingDateTS: this.currentBookingDateTS,
      ...
      if (this.isBookingActionVisible) {
        await this.updateExistingBookedResourceAndAppendItems(selectedSlotData)
      } else {
        if (!this.isSearchAvailableTimesFromBookingItems) {
          CalendarEventBus.$emit(EVENTS.CLOSE_CLIENT_INFO_ON_SELECT)
        }
        await this.openBookingActionBySlot(selectedSlotData)
        await this.addBookedItemsFromSearchAvailableTimes(selectedSlotData.estimatedTime)
      }

      this.onCancel()
    },

    async addBookedItemsFromSearchAvailableTimes(fallbackEstimatedTime) {
      if (!this.searchAvailableTimesItemsData) return

      const bookedItems = this.buildBookedItemsFromSearchAvailableTimes(fallbackEstimatedTime)
      ...
      if ((bookedItems.length + (bookedResource.bookedItems?.length || 0)) > MAX_ITEMS_RESOURCE) {
        this._showDialogAlert(this.$t('bookings.warning_can_not_exceed_booked_items_max', { items_max: MAX_ITEMS_RESOURCE }))
        return
      }
    ...
    buildBookedItemsFromSearchAvailableTimes(fallbackEstimatedTime) {
      ...
      for (const serviceItem of exactServiceItems) {
        const bookedItem = this.createBookedItemFromService(serviceItem, fallbackEstimatedTime)
      ...
      for (const bookingItem of exactBookingItems) {
        const bookedItem = this.createBookedItemFromBookingItem(bookingItem, fallbackEstimatedTime)
        bookedItems.push(bookedItem)
      }
      ...
      if (selectedServiceItems) {
        for (const selectedServiceItem of Object.values(selectedServiceItems)) {
          const bookedItem = this.createBookedItemFromService(selectedServiceItem, fallbackEstimatedTime)
          bookedItems.push(bookedItem)
        }
      }
      ...
      if (selectedBookingItems) {
        for (const selectedBookingItem of Object.values(selectedBookingItems)) {
          const bookedItem = this.createBookedItemFromBookingItem(selectedBookingItem, fallbackEstimatedTime)
          bookedItems.push(bookedItem)
        }
      }
    ...
    createBookedItemFromService(service, fallbackEstimatedTime) {
      ...
      bookedItem.setEsimatedTime(service.estimatedTime || fallbackEstimatedTime || 0)
      return bookedItem
    },

    createBookedItemFromBookingItem(bookingItem, fallbackEstimatedTime) {
      const bookedItem = new BookedItem()
      bookedItem.setIsNew(true)
      ...
      bookedItem.setEsimatedTime(bookingItem.estimatedTime || fallbackEstimatedTime || 0)
      return bookedItem
    },

    async updateExistingBookedResourceAndAppendItems(selectedSlotData) {
      try {
        const bookedResource = this.bookedResources?.[0]
        if (!bookedResource) return
        ...
        clonedResource.startTimeInMinutes = selectedSlotData.startTime
        clonedResource.bookingResourceSetupId = selectedSlotData.resource.id
        clonedResource.resourceId = selectedSlotData.resource.resource_id
        clonedResource.resourceName = selectedSlotData.resource.resource_name

        if (selectedSlotData.bookingDateTS) {
          this.setBookingDateTS(selectedSlotData.bookingDateTS)
        }

        await this.updateBookedResource(clonedResource)
