...
export default {
  ...
  methods: {
    ...
    async onSelectAvailableTime(resource, slot) {
      const bookingEstimatedTime = this.currentFilters.originalTotalEstimatedTime ?? this.currentFilters.estimatedTime
      const selectedSlotData = {
        ...
        estimatedTime: bookingEstimatedTime,
      }
      ...
      if (this.isBookingActionVisible) {
        ...
        await this.addBookedItemsFromSearchAvailableTimes(bookingEstimatedTime)
      }

      this.onCancel()
    },

    async addBookedItemsFromSearchAvailableTimes(searchEstimatedTime) {
      ...
      this.ensureMinEstimatedTime(searchEstimatedTime)
    },

    ensureMinEstimatedTime(minEstimatedTime) {
      if (!minEstimatedTime) return

      const bookedResource = this.bookedResources?.[0]
      if (!bookedResource) return
      if (bookedResource.estimatedTime >= minEstimatedTime) return

      const clonedResource = bookedResource.clone()
      clonedResource.estimatedTime = minEstimatedTime
