...
export const useBranchPrepaidGoods = () => {
  ...
  const items = computed(() => reportData.value?.result || [])
  ...
  const fetchReport = async (
    ...
    try {
      ...
      const errorMessage = err instanceof Error ? err.message : 'Failed to fetch branch prepaid goods report'
      error.value = errorMessage
      console.error('Error fetching branch prepaid goods report:', err)
      return null
    } finally {
      startLoading(false)
    }
