...
export default [
  ...
  // Supports Vue 3 Composition API, <script setup>, and TypeScript
  {
    ...
    rules: {
      ...
      /**
       * Prevent the use of promises in locations where they are not expected
       *
       * Why: Promises should be used in asynchronous contexts
       *
       * Example:
       * ❌ Bad: button.addEventListener('click', async () => { await handleGoToPage() })
       * ✅ Good: button.addEventListener('click', () => { handleGoToPage() })
       */
      '@typescript-eslint/no-misused-promises': ['error', { checksVoidReturn: false }],

      /**
       * Require all promises to have a return statement
       *
       * Why: Promises should have a return statement to ensure proper error handling
       *
       * Example:
       * ❌ Bad:
       * doA()
      ...
      /**
       * Disallow nesting of promises
       *
       * Why: Nesting promises makes code harder to read and maintain
       *
       * Example:
       * ❌ Bad:
       * doA().then((a) => {
      ...
      // ========================================
      // 🔴 CRITICAL: TypeScript Type Checking Rules
      // ========================================
      // These rules enable ESLint to catch TypeScript type errors

      /**
       * Disallow unsafe member access
       *
       * Why: Prevents accessing properties that might not exist
       */
       ...
       *
       * Why: Prevents calling functions with unsafe types
       */
      '@typescript-eslint/no-unsafe-call': 'error',

      /**
       * Disallow unsafe assignment
       *
       * Why: Prevents assigning unsafe values
       */
      '@typescript-eslint/no-unsafe-assignment': 'error',

      /**
       * Disallow unsafe return
       *
       * Why: Prevents returning unsafe values
       */
      '@typescript-eslint/no-unsafe-return': 'error',

      /**
       * Disallow unsafe argument
       *
       * Why: Prevents passing unsafe arguments
       */
  ...
  // Override max-lines rule for this config file
  {
    files: [
      'eslint.config.ts',
      'src/constants/**/*.ts',
      'src/composables/print/constants/**/*.ts',
      'src/themes/**/*.ts',
    ],
    rules: {
