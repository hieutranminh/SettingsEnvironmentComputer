...
export default [
  ...
  // Supports Vue 3 Composition API, <script setup>, and TypeScript
  {
    ...
    rules: {
      ...
      /**
       * Disable vue/no-v-html in favor of custom rule
       *
       * Why: We use custom/require-serialize-for-v-html instead
       * which allows v-html but requires serialize functions for security
       */
      'vue/no-v-html': 'off',
      ...
      'vue/no-restricted-static-attribute': [
        'error',
        {
          ...
          element: '/^[a-z][a-z0-9-]*$/',
          message:
            'Do not use inline style for HTML elements. Use class/CSS instead.',
        },
      ],
      ...
      /**
       * Require v-html to use serialize function for security
       *
       * Why: v-html can lead to XSS attacks if not properly sanitized
       * This rule enforces the use of serialize functions to ensure content is sanitized
       *
       * Example:
       * ❌ Bad: <div v-html="userContent"></div>
       * ✅ Good: <div v-html="serializeHtml(userContent)"></div>
       * ✅ Good: <div v-html="serializeContent(userContent)"></div>
       * ✅ Good: <div v-html="serializeUserHtml(userContent)"></div>
       */
      'custom/require-serialize-for-v-html': 'error',
    },
  },
  ...
  // Global rules that apply to all files for consistent naming
  {
    rules: {
      ...
      'id-length': [
        ...
        {
          min: 2,
          exceptions: ['i', 't', 'e'], // Allow these common short names
        },
      ],
    },
  },
