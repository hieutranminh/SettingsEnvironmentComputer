    ...
    <template #content>
      ...
      >
        ...
        <Column field="prepaidCardAmount">
          <template #body="{ data }">
            {{ handleDisplayAmount(data.prepaidCardAmount) }}
          </template>
        </Column>
        <Column field="prepaidServiceAmount">
          <template #body="{ data }">
            {{ handleDisplayAmount(data.prepaidServiceAmount) }}
          </template>
        </Column>
        <Column field="total">
          <template #body="slotProps">
        ...
        <ColumnGroup v-if="transformedData.length" type="footer">
          <Row>
            ...
            />
            <Column
              :footer="handleDisplayAmount(mapDataTotal.prepaidCardAmount)"
              footerClass="bg-gray"
            />
            <Column
              :footer="handleDisplayAmount(mapDataTotal.prepaidServiceAmount)"
              footerClass="bg-gray"
            />
...
const transformedData = computed<ISalesByMonthReportItem[]>(() => {
...
const handleDisplayAmount = (amount: number): string => {
  if (props.prepaidSalesCountingType === PREPAID_SALES_COUNTING_TYPE.USED) return '-'
  return formatAmount(amount, { showZeroValues: true })
}
