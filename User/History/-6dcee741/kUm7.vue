...
const averageLabel = computed(() => {
  return `${t('general.average')} (${props.chartDisplayValue === CHART_DISPLAY_VALUE.AMOUNT ? formatAmount(averageAmount.value, { decimalCount: 0 }) : formatAmount(averageQuantity.value, { decimalCount: 0 })})`
})
 ...
 */
const getBarDatasets = (): ChartDataset[] => [
  {
    ...getBaseDatasetProps(),
 ...
 */
const getLineDatasets = (): ChartDataset[] => [
  {
    ...getBaseDatasetProps(),
    fill: false,
 ...
 */
const getBarLineDatasets = (): ChartDataset[] => [
  {
    ...getBaseDatasetProps(),
    type: 'line',
    fill: false,
...
const getDatasetsByType = (type: string): ChartDataset[] => {
  const datasetMap: Partial<Record<ChartDisplayType, () => ChartDataset[]>> = {
