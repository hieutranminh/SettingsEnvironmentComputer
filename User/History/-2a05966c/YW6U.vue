...
const handleConfirm = (): void => {
  // Trigger the confirm emit through the dialog system
  if (dialogRef?.value?.close) {
    // The dialog system will handle the emit before closing
    dialogRef.value.close();
  }
};

const handleCancel = (): void => {
  // Trigger the cancel emit through the dialog system
  if (dialogRef?.value?.close) {
    // The dialog system will handle the emit before closing
    dialogRef.value.close();
  }
