...
// Constants local
const YEAR_INDEX_START = 0
const YEAR_INDEX_END = 4
const MONTH_INDEX_START = 4
const MONTH_INDEX_END = 6
...
const transformedData = computed<IServiceSalesByMonthReportItem[]>(() => {
  ...
  return props.data.map((item) => ({
    ...item,
    monthOfYear: `${item.monthOfYear.slice(YEAR_INDEX_START, YEAR_INDEX_END)}-${item.monthOfYear.slice(MONTH_INDEX_START, MONTH_INDEX_END)}`,
    ratio: totalAmount > 0 ? ((item.amount || 0) / totalAmount) * PERCENTAGE : 0,
  }))
})
