 ...
 */
export function useFilter<T extends BaseFilter>(options: UseFilterOptions<T>): UseFilterReturn<T> {
  const { initialFilters, debounceMs = 300, onFilterChange, autoTrigger = false } = options
  ...
  const debouncedFilterChange = useDebounceFn((newFilters: T) => {
