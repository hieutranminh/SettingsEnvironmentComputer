...
export default defineConfig({
  plugins: [
    ...
    checker({
      ...
      eslint: {
        ...
        dev: {
          logLevel: ['error'], // Chỉ hiển thị error, không hiển thị warning để tăng tốc
          overrideConfig: {
            cache: true,
      ...
      // Thêm cấu hình để tối ưu performance
      terminal: false, // Tắt terminal output để tăng tốc
      typescript: false, // Tắt TypeScript checking riêng vì đã có vueTsc
    }),
  ],
  resolve: {
    alias: {
  ...
  server: {
    ...
    // Tối ưu HMR performance
    fs: {
      strict: false, // Cho phép serve files ngoài root
    },
    // Tăng timeout để tránh HMR timeout và tối ưu overlay
    hmr: {
      overlay: true,
