...
const PHONE_LAST_DIGITS_LENGTH_ALT = 4
...
const VN_PHONE_LAST_DIGITS_LENGTH_10 = 3
const VN_PHONE_LAST_DIGITS_LENGTH_11 = 4
...
export const getTimezoneByCountry = (
  ...
  | undefined => {
  return TIMEZONE_CONFIG.find((x) => x.country_code === countryCode)
}
...
export const getShopLocation = (country: string): string => {
  ...
  const location = shopLocations.find((x) => x.country_code === country)
...
export const formatMobileAndPhoneNumber = (number: string, country = 'KR'): string => {
  ...
  const formatKR = (num: string): string => {
    ...
    if (num.length === 8) return `${num.substring(0, 4)}-${num.substring(4, 8)}`
    if (num.length >= 11)
      return `${num.substring(0, 3)}-${num.substring(3, num.length - 4)}-${num.substring(num.length - 4, num.length)}`
    return num
  }

  const formatVN = (num: string): string => {
    if (num.length === 10)
      return `${num.substring(0, 4)}-${num.substring(4, 7)}-${num.substring(7, 10)}`
    if (num.length === 11)
      return `${num.substring(0, 4)}-${num.substring(4, 7)}-${num.substring(7, 11)}`
    return num
  }
...
export const showRemoteSupport = (): void => {
  const w = window.open('https://988.co.kr', 'popup', 'width=1000,height=800')
  if (w) w.opener = null
}

export const showContactChat = (): void => {
  const w = window.open('https://talk.naver.com/WSYF626', 'popup', 'width=400,height=600')
  if (w) w.opener = null
}

export const subtractMonths = (ts: number, months: number): number => {
  const dateFormat = new Date(ts * 1000) // timestamp -> Date
  dateFormat.setMonth(dateFormat.getMonth() - months) // lùi n tháng
  return Math.floor(dateFormat.getTime() / 1000) // -> timestamp giây
}
