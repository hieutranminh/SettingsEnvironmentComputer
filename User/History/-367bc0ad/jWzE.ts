...
const PERCENTAGE_MAX_WITH_DECIMAL = '100.0'
const PERCENTAGE_MAX_WITHOUT_DECIMAL = '100'
...
export const formatAmount = (value: number, options: IFormatAmountOptions = {}): string => {
  ...
  if ((value === 0 && !showZeroValues) || value === undefined || value === null) return ''
  ...
  const formatValue = new Intl.NumberFormat(locale, formatOptions).format(value)
  return formatValue === PERCENTAGE_MAX_WITH_DECIMAL ? PERCENTAGE_MAX_WITHOUT_DECIMAL : formatValue
}

export const formatPercentage = (
  value: number,
  options: { decimalCount?: number; showZeroValues?: boolean } = {},
): string => {
...
export function convertMinutesToHours(minutes: number): string {
  ...
  const minuteNum = Math.floor(minutes - hourNum * 60)
