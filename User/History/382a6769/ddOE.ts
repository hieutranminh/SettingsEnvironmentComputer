...
export interface BranchPrepaidGoodsReportResponse {
  result: BranchPrepaidGoodsReportItem[]
...
export const getBranchPrepaidGoodsReport = async (
  ...
  try {
    const response = await apiRead.post<BranchPrepaidGoodsReportResponse>(
      '/api/read/v1/Sales/BranchPrepaidGoods/GetReport',
      request
    )
    ...
    return {
      ...
      message: response.data.isOK ? 'Report retrieved successfully' : 'Failed to retrieve report',
