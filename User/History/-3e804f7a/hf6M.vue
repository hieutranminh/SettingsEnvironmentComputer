<template>
  <Card>
    <template #content>
      <!-- Table Main -->
      <BranchSalesTableMain
        ref="dataTableRefMain"
        :data="transformedData"
        :totalData="mapDataTotal"
      />

      <!-- Table Print -->
      <BranchSalesTablePrint
...
// Types
import type { IListResponse } from '@/types/ApiResponse'
import type { IPrintSection } from '@/types/print'
// Utils
import type {
  IBranchSalesTotalReportItem,
  IBranchSalesReportData,
...
interface Props {
  data?: IBranchSalesTotalReportItem[]
  pagingInfo?: IListResponse<IBranchSalesTotalReportItem>['pagingInfo']
}
...
// DataTable ref
const dataTableRefMain = ref<InstanceType<typeof BranchSalesTableMain>>()
const dataTableRefPrint = ref<InstanceType<typeof BranchSalesTablePrint>>()

// Transform API data to table format
const transformedData = computed<IBranchSalesReportData[]>(() => {
  ...
  return props.data.map((item) => {
    ...
    const totalPrepaidGoodsSalesAmount =
      item.prepaidCardSalesAmount + item.prepaidServicesSalesAmount
...
const mapDataTotal = computed<Omit<IBranchSalesReportData, 'shopId' | 'branch'>>(() => {
  return transformedData.value.reduce(
    (acc, item) => {
      return {
        serviceSales: acc.serviceSales + item.serviceSales,
        servicePrepaidCardDeduction:
          acc.servicePrepaidCardDeduction + item.servicePrepaidCardDeduction,
        servicePrepaidServiceDeduction:
          acc.servicePrepaidServiceDeduction + item.servicePrepaidServiceDeduction,
        ...
        productPrepaidCardDeduction:
          acc.productPrepaidCardDeduction + item.productPrepaidCardDeduction,
        ...
        prepaidGoodsDeductionTotal:
          acc.prepaidGoodsDeductionTotal + item.prepaidGoodsDeductionTotal,
...
const getPrintConfiguration = (): IPrintSection => {
  const tableElementPrint = dataTableRefPrint.value?.getDataTableDOM()
  const tableElementMain = dataTableRefMain.value?.getDataTableDOM()
  if (!tableElementPrint || !tableElementMain) {
    throw new Error('Table element not found')
  }

  return {
    refType: PRINT_TYPE.TABLE,
    sectionRef: tableElementPrint,
    sectionRefExcel: tableElementMain,
    tableStyles: {
