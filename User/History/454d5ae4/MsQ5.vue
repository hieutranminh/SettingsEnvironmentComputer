<template>
  ...
  <template v-if="filters.shopId">
    ...
    <SalesByDateChart
      ...
      :data="items"
      :prepaidSalesCountingType="
        filterData?.prepaidSalesCountingType ?? PREPAID_SALES_COUNTING_TYPE.SOLD
      "
    />
    ...
    <SalesByDateTable
      ...
      :data="itemsDataFromPrepaidService"
    />

    <SalesByDateTable ref="table" id="idTable" class="table" aria="aria" />
  </template>
</template>
...
const salesByDateChartRef = ref<InstanceType<typeof SalesByDateChart> | null>(
  null,
)
const salesByDateTableRef = ref<InstanceType<typeof SalesByDateTable> | null>(
  null,
)
...
const handlePrint = async (): Promise<void> => {
  ...
  if (salesByDateChartRef.value)
    sections.push(salesByDateChartRef.value.getPrintConfiguration())
  if (salesByDateTableRef.value)
    sections.push(salesByDateTableRef.value.getPrintConfiguration())
