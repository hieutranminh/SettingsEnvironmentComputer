...
export default {
  ...
  computed: {
    ...
    rangeModel: {
      ...
      set() {
        // Setter is required for v-model but we handle changes via @input event
      },
    },
  },
  ...
  methods: {
    ...
    onRangeChange(range) {
      ...
      if (daysDifference > MAX_DAYS_SELECTED) {
        ...
        this.rangeModel = {
          start: range.start,
          end:   new Date(range.start.getTime() + (MAX_DAYS_SELECTED - 1) * 24 * 60 * 60 * 1000),
        }
        const correctedToDateTS = convertDateToTimeStamp(this.rangeModel.end)
