<template>
  <div>
    ...
    <br>
    <pre>{{ isPeriodSelection }}</pre>
    <pre>{{ fromDateTS }}</pre>
    <pre>{{ toDateTS }}</pre>
    <pre>{{ bookingDates }}</pre>
  </div>
</template>
  ...
  methods: {
    onDayClick(day) {
      ...
      if(this.isMaxDaysSelected) {
        this._showDialogAlert(this.$t('bookings.validate-you-can-select-up-to-30-days-at-once'))
        return
      }
      ...
      index > -1 ? updatedBookingDates.splice(index, 1) : updatedBookingDates.push(selectedDateTS)
