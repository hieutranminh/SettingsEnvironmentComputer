...
export default {
  ...
  watch: {
    fromDateTS() {
      this.localRangeModel = null
    },
    toDateTS() {
      this.localRangeModel = null
    },
    isPeriodSelection(newValue) {
      if (!newValue) {
        this.localRangeModel = null
      }
    },
  ...
  computed: {
    ...
    rangeModel: {
      get() {
        ...
        return {
          start: new Date(this.fromDateTS * 1000),
          end:   new Date(this.toDateTS * 1000),
        }
      },
      set(value) {
  ...
  methods: {
    ...
    onRangeChange(range) {
      if (!range || !range.start || !range.end) {
        this.localRangeModel = null
        this.$emit('update:fromDateTS', 0)
        this.$emit('update:toDateTS', 0)
        return
      }

      const fromDateTS = convertDateToTimeStamp(range.start)
      const toDateTS = convertDateToTimeStamp(range.end)

      const daysDifference = Math.ceil((toDateTS - fromDateTS) / (24 * 60 * 60))

      if (daysDifference > MAX_DAYS_SELECTED) {
        this._showDialogAlert(this.$t('aha-ai.validate-you-can-select-up-to-30-days-at-once'))
        const correctedEndDate = new Date(range.start.getTime() + (MAX_DAYS_SELECTED - 1) * 24 * 60 * 60 * 1000)
        const correctedToDateTS = convertDateToTimeStamp(correctedEndDate)
        this.localRangeModel = {
          start: range.start,
          end:   correctedEndDate,
        }
        this.$emit('update:fromDateTS', fromDateTS)
        this.$emit('update:toDateTS', correctedToDateTS)
        return
      }

      this.localRangeModel = range
      this.$emit('update:fromDateTS', fromDateTS)
