  ...
  computed: {
    ...
    disabledDates() {
      ...
      if (this.isPeriodSelection && this.fromDateTS) {
        ...
        maxEndDate.setDate(maxEndDate.getDate() + MAX_DAYS_SELECTED)
        maxEndDate.setHours(23, 59, 59, 999)

        // Return function to disable dates dynamically
        return (date) => {
          const dateToCheck = new Date(date)
          dateToCheck.setHours(0, 0, 0, 0)
          const yesterdayCheck = new Date(yesterday)
          yesterdayCheck.setHours(0, 0, 0, 0)
          const maxEndDateCheck = new Date(maxEndDate)
          maxEndDateCheck.setHours(0, 0, 0, 0)

          // Disable dates before yesterday or after 30 days from startDate
          return dateToCheck <= yesterdayCheck || dateToCheck > maxEndDateCheck
        }
      }
