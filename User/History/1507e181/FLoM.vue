      ...
      >
        ...
        <ColumnGroup v-if="transformedData.length" type="footer">
          <Row>
            ...
            <Column
              :footer="formatAmount(calculatedTotals.unregisteredQty)"
              footerClass="bg-gray"
            />
            <Column
              :footer="formatAmount(calculatedTotals.unregisteredAmount)"
              footerClass="bg-gray"
            />
...
// Types
import type { ISalesByRepeatClientsReportItem } from '@/types/client-report/SalesByRepeatClients'
import type { IPrintSection } from '@/types/print'
// Utils
import { formatAmount } from '@/utils/common'

interface IProps {
  data?: ISalesByRepeatClientsReportItem[]
}
...
const transformedData = computed<ITableRow[]>(() => {
  if (!props.data || props.data.length === 0) {
    return []
  }

  const serviceGroups: Record<string, ITableRow> = {}
