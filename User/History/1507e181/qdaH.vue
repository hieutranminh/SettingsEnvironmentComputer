...
// Constants
import { ITEM_TYPE, PRINT_TYPE, type ItemType } from '@/constants'
// Types
...
const TYPE_TO_COL: Record<ItemType, { qty: keyof TableRow; amount: keyof TableRow }> = {
  [ITEM_TYPE.UNREGISTERED]: { qty: 'unregisteredQty', amount: 'unregisteredAmount' }, // Unregistered
  [ITEM_TYPE.NEW]: { qty: 'newQty', amount: 'newAmount' }, // New
  [ITEM_TYPE.REPEAT]: { qty: 'repeatQty', amount: 'repeatAmount' }, // Repeat
}
...
const transformedData = computed<TableRow[]>(() => {
  ...
  const byService = new Map<string, TableRow>()

  const makeEmpty = (service: string): TableRow => ({
    service,
    newQty: 0,
    newAmount: 0,
    repeatQty: 0,
  ...
  for (const it of data) {
    const row = byService.get(it?.key) ?? makeEmpty(it?.key)
    const col = TYPE_TO_COL[it?.itemType]

    // cộng dồn theo từng loại
    row[col.qty] += it?.quantity ?? 0
    row[col.amount] += it?.amount ?? 0

    // cập nhật tổng (sau vòng lặp cũng có thể tính lại)
