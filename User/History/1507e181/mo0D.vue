      ...
      >
        ...
        <Column field="newQty">
          <template #body="slotProps">
            {{ formatAmount(slotProps.data.newQty) || 0 }}
          </template>
        </Column>
        <Column field="newAmount">
          <template #body="slotProps">
            {{ formatAmount(slotProps.data.newAmount) || 0 }}
          </template>
        </Column>
        <Column field="repeatQty">
          <template #body="slotProps">
            {{ formatAmount(slotProps.data.repeatQty) || 0 }}
          </template>
        </Column>
        <Column field="repeatAmount">
          <template #body="slotProps">
            {{ formatAmount(slotProps.data.repeatAmount) || 0 }}
          </template>
        </Column>
        <Column field="unregisteredQty">
          <template #body="slotProps">
            {{ formatAmount(slotProps.data.unregisteredQty) || 0 }}
          </template>
        </Column>
        <Column field="unregisteredAmount">
          <template #body="slotProps">
            {{ formatAmount(slotProps.data.unregisteredAmount) || 0 }}
          </template>
        </Column>
        <Column field="totalQty">
          <template #body="slotProps">
            {{ formatAmount(slotProps.data.totalQty) || 0 }}
          </template>
        </Column>
        <Column field="totalAmount">
          <template #body="slotProps">
            {{ formatAmount(slotProps.data.totalAmount) || 0 }}
          </template>
        </Column>
...
const transformedData = computed<TableRow[]>(() => {
  ...
  for (const item of props.data) {
    ...
    if (item.itemType === ITEM_TYPE.NEW) {
      // New
      row.newQty += item.quantity
      row.newAmount += item.amount
    } else if (item.itemType === ITEM_TYPE.REPEAT) {
      // Repeat
      row.repeatQty += item.quantity
      row.repeatAmount += item.amount
    } else if (item.itemType === ITEM_TYPE.UNREGISTERED) {
      // Unregistered
