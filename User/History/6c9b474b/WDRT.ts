...
export const useSalesByMonthReport = (): {
  filterData: Ref<SalesByMonthFilterInterface | null>
  reportData: Ref<SalesByMonthReportResponse | null>
  error: Ref<string | null>
  isLoading: Ref<boolean>
  hasData: ComputedRef<boolean>
  items: ComputedRef<SalesByMonthReportItem[]>
  fetchReport: (filters: SalesByMonthFilterInterface) => Promise<SalesByMonthReportResponse | null>
  clearReport: () => void
  refreshReport: (filters: SalesByMonthFilterInterface) => Promise<void>
  buildRequestPayload: (filters: SalesByMonthFilterInterface) => SalesByMonthReportRequest
} => {
  // State
  ...
  const items = computed(() => {
    ...
    return (reportData.value?.result?.reportItems ?? []).map((item, index) => {
      ...
      const date = startDate.add(index, 'month').format(STANDARD_DATE_FORMAT.YMD)
