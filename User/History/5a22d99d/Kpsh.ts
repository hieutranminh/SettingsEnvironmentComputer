...
export interface PdfHeaderService {
  ...
  renderMixedText: (pdf: jsPDF, text: string, x: number, y: number, options?: { align?: 'left' | 'center' | 'right' | 'justify' }) => number
}
...
export const createPdfHeaderService = (): PdfHeaderService => {
   ...
   */
  const addDateAndTotalSection = (pdf: jsPDF, config: Required<PdfConfig>, pageWidth: number, startY: number): number => {
