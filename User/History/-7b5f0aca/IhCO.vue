  ...
  <Card>
    <template #content>
      ...
      <DataTable
        ...
        scrollHeight="550px"
        tableClass="prepaid-goods-repurchase-table"
        showGridlines
      >
        <!-- Header -->
        <ColumnGroup type="header">
          <Row>
            <Column
              :rowspan="2"
              :header="$t('prepaid-goods-repurchase.label-service')"
              headerClass="bg-gray"
              style="width: 25%"
            />
            <Column
              :colspan="2"
              :header="$t('prepaid-goods-repurchase.label-new')"
              headerClass="bg-gray border-bottom-0"
            />
            <Column
              :colspan="2"
              :header="$t('prepaid-goods-repurchase.label-repurchase')"
              headerClass="bg-gray border-bottom-0"
            />
            <Column
              :colspan="2"
              :header="$t('prepaid-goods-repurchase.label-total')"
              headerClass="bg-gray border-bottom-0"
            />
          </Row>

          <Row>
            <Column :header="$t('prepaid-goods-repurchase.label-qty')" headerClass="bg-gray" />
            <Column :header="$t('prepaid-goods-repurchase.label-amount')" headerClass="bg-gray" />
            <Column :header="$t('prepaid-goods-repurchase.label-qty')" headerClass="bg-gray" />
            <Column :header="$t('prepaid-goods-repurchase.label-amount')" headerClass="bg-gray" />
            <Column :header="$t('prepaid-goods-repurchase.label-qty')" headerClass="bg-gray" />
            <Column :header="$t('prepaid-goods-repurchase.label-amount')" headerClass="bg-gray" />
        ...
        <Column field="repeatQty">
          <template #body="slotProps">
            {{ formatAmount(slotProps.data.repurchaseQty) }}
          </template>
        </Column>
        <Column field="repeatAmount">
          <template #body="slotProps">
            {{ formatAmount(slotProps.data.repurchaseAmount) }}
          </template>
        </Column>
        ...
        <ColumnGroup v-if="transformedData.length" type="footer">
          <Row>
            <Column :footer="$t('prepaid-goods-repurchase.label-total')" footerClass="bg-gray" />
            ...
            <Column :footer="formatAmount(calculatedTotals.repurchaseQty)" footerClass="bg-gray" />
            <Column :footer="formatAmount(calculatedTotals.repurchaseAmount)" footerClass="bg-gray" />
...
// Types
import type { PrepaidGoodsRepurchaseReportItem } from '@/types/client-report/PrepaidGoodsRepurchase'
import type { PrintSection } from '@/types/print'
// Utils
import { formatAmount } from '@/utils/common'

interface Props {
  data?: PrepaidGoodsRepurchaseReportItem[]
}
  ...
  for (const item of props.data) {
    ...
    switch (item.itemType) {
      ...
      case ITEM_TYPE.REPEAT:
        // Repeat
        row.repurchaseQty += item.quantity
        row.repurchaseAmount += item.amount
        break
    }

    row.totalQty = row.newQty + row.repurchaseQty
    row.totalAmount = row.newAmount + row.repurchaseAmount
  }
...
const calculatedTotals = computed(() => {
  return transformedData.value.reduce(
    (acc, item) => ({
      ...
      repurchaseQty: acc.repurchaseQty + item.repurchaseQty,
      repurchaseAmount: acc.repurchaseAmount + item.repurchaseAmount,
...
:deep(.prepaid-goods-repurchase-table) {
  .bg-gray {
