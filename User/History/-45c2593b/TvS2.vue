...
const transformedData = computed<IServiceSalesByItemReportItem[]>(() => {
  ...
  return props.data.map((item) => ({
    ...
    ratio: totalAmount !== 0 ? (item.amount / totalAmount) * PERCENTAGE_MULTIPLIER : 100,
  }))
})
...
const mapDataTotal = computed(() => {
  ...
  // Calculate ratio based on total amount - always 100% for totals
  const totalRatio = totals.amount !== 0 ? (totals.amount / totals.amount) * PERCENTAGE_MULTIPLIER : 100
