    ...
    <div class="modal-aha-ai-body">
      ...
      <div class="modal-aha-ai-body__textarea">
        <b-form-textarea
          ...
          @keypress.enter.exact="onEnterKeyPress"
        />
  ...
  methods: {
    ...
    onShown() {
      ...
      if (textarea && textarea.$el) {
        textarea.$el.focus()
        this.attachNativeEnterHandler(textarea.$el)
      }
    },

    onHide() {
      this.removeNativeEnterHandler()
      this.reset()
      this.$emit('cancel')
    },
    ...
    onEnterKeyPress(event) {
      // Handle Enter key on iOS Safari (newer versions)
      if (this.hasPrompt && this.currentMode === MODE_AI_ACTION.TYPING) {
        event.preventDefault()
        event.stopPropagation()
        this.onSend()
    ...
    attachNativeEnterHandler(textareaElement) {
      // Remove existing listener if any
      if (this._nativeEnterHandler) {
        textareaElement.removeEventListener('keydown', this._nativeEnterHandler)
        textareaElement.removeEventListener('keypress', this._nativeEnterHandler)
      }
      ...
      this._nativeEnterHandler = handleEnter

      // Attach to both keydown and keypress for iOS compatibility
      textareaElement.addEventListener('keydown', handleEnter, { capture: true })
      textareaElement.addEventListener('keypress', handleEnter, { capture: true })
    },
    ...
    removeNativeEnterHandler() {
      const textarea = this.$refs.promptTextarea
      if (textarea && textarea.$el && this._nativeEnterHandler) {
        textarea.$el.removeEventListener('keydown', this._nativeEnterHandler, { capture: true })
        textarea.$el.removeEventListener('keypress', this._nativeEnterHandler, { capture: true })
        this._nativeEnterHandler = null
      }
    },
  },
