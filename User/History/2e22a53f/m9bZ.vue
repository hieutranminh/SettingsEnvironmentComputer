...
export default {
  ...
  methods: {
    ...
    setSearchAvailableTimesData(resultData) {
      ...
      // If no relative match, auto confirm
      if (!this.hasRelativeMatchResult) {
        this.onConfirmSearchAvailableTimes()
      }
    },
    ...
    onConfirmSearchAvailableTimes() {
      this.$emit('confirm-search-available-times', this.searchAvailableTimesData)
      this.$refs.modalAhaAi.hide()
    },
  },
  ...
  computed: {
    ...
    hasSearchAvailableTimesResult() {
      return this.searchAvailableTimesData !== null
    },
  },
}
</script>
