...
export const useFormat = (): {
  ...
  formatHourToRange: (hour: string | number, stepMinutes?: number) => string
  formatDateOfWeek: (dateOfWeek: string | number) => string
} => {
  const { t } = useI18n()
  ...
  const formatTwoDigits = (number: number): string => {
    return String(number).padStart(TWO_DIGITS, '0')
  }

  const formatHourToRange = (hour: string | number, stepMinutes = MINUTES_IN_HOUR): string => {
    const startHour = Number(hour)
    if (isNaN(startHour)) return ''

    const startMinute = 0
    const totalMinutes = startHour * MINUTES_IN_HOUR + startMinute + stepMinutes
    const endHour = Math.floor(totalMinutes / MINUTES_IN_HOUR) % HOURS_IN_DAY
    const endMinute = totalMinutes % MINUTES_IN_HOUR

    return `${formatTwoDigits(startHour)}:${formatTwoDigits(startMinute)} ~ ${formatTwoDigits(endHour)}:${formatTwoDigits(endMinute)}`
  }

  const formatDateOfWeek = (dateOfWeek: string | number): string => {
    return t(`general.date-of-week.${dateOfWeek.toString()}`)
  }
