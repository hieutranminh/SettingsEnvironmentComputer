...
export interface IUsePdfHeaderReturn {
  addHeader: (pdf: jsPDF, config: Required<IPdfConfig>, startY: number) => number
}
...
export const usePdfHeader = (): IUsePdfHeaderReturn => {
   ...
   */
  const addHeader = (pdf: jsPDF, config: Required<IPdfConfig>, startY: number): number => {
