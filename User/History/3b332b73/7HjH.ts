import { API_TYPES, DEFAULT_API_CONFIG, type IApiConfig } from '@/constants/apiEndpoints'
import { apiClient } from '@/plugins/axios'
import type { IApiResponse } from '@/types/ApiResponse'

// Helper function to build API endpoint with type and version
const buildEndpoint = (endpoint: string, config?: IApiConfig): string => {
...
export const apiPut = async <T>(
  endpoint: string,
  data?: unknown,
  config?: IApiConfig & { headers?: Record<string, string> },
): Promise<IApiResponse<T>> => {
  const fullEndpoint = buildEndpoint(endpoint, config)
...
export const apiCmd = {
  ...
  put: <T>(
    ...
    data?: unknown,
    config?: IApiConfig & { headers?: Record<string, string> },
  ): Promise<IApiResponse<T>> => apiPut<T>(endpoint, data, { type: API_TYPES.COMMAND, ...config }),
...
export const apiAggr = {
  get: <T>(endpoint: string): Promise<IApiResponse<T>> =>
    apiGet<T>(endpoint, { type: API_TYPES.AGGREGATE }),
  post: <T>(endpoint: string, data?: unknown): Promise<IApiResponse<T>> =>
