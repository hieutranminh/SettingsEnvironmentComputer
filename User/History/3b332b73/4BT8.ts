...
export const apiPostWithType = async <T>(path: string, data?: any, type: ApiType = 'cmd', version: ApiVersion = 'v1'): Promise<ApiResponse<T>> => {
  const endpoint = apiEndpoint.withType(type).withVersion(version).build(path)
  return await apiPost<T>(endpoint, data)
}

export const apiPutWithType = async <T>(path: string, data?: any, type: ApiType = 'cmd', version: ApiVersion = 'v1'): Promise<ApiResponse<T>> => {
  const endpoint = apiEndpoint.withType(type).withVersion(version).build(path)
  return await apiPut<T>(endpoint, data)
}

export const apiPatchWithType = async <T>(path: string, data?: any, type: ApiType = 'cmd', version: ApiVersion = 'v1'): Promise<ApiResponse<T>> => {
  const endpoint = apiEndpoint.withType(type).withVersion(version).build(path)
...
export const apiRead = {
  ...
  post: <T>(path: string, data?: any, version: ApiVersion = 'v1') => apiPostWithType<T>(path, data, 'read', version)
}

export const apiAggr = {
  get: <T>(path: string, version: ApiVersion = 'v1') => apiGetWithType<T>(path, 'aggr', version),
  post: <T>(path: string, data?: any, version: ApiVersion = 'v1') => apiPostWithType<T>(path, data, 'aggr', version)
}

export const apiCmd = {
  post: <T>(path: string, data?: any, version: ApiVersion = 'v1') => apiPostWithType<T>(path, data, 'cmd', version),
  put: <T>(path: string, data?: any, version: ApiVersion = 'v1') => apiPutWithType<T>(path, data, 'cmd', version),
  patch: <T>(path: string, data?: any, version: ApiVersion = 'v1') => apiPatchWithType<T>(path, data, 'cmd', version),
