...
const filters = ref<ServiceSalesFilterInterface>({
  ...
  fromDateTs: toUnixTimestamp(startOfMonth, TIMEZONE_TYPE.UTC, {
    keepLocalTime: true,
  }),
  toDateTs: toUnixTimestamp(endOfMonth, TIMEZONE_TYPE.UTC, {
    keepLocalTime: true,
  }),
...
const serviceSalesChartRef = ref<InstanceType<typeof ServiceSalesChart> | null>(
  null,
)
const serviceSalesTableRef = ref<InstanceType<typeof ServiceSalesTable> | null>(
  null,
)
...
const getDateRangeWithDateTypeIsDate = (): {
  ...
  return {
    fromDateTs: toUnixTimestamp(startOfDay, TIMEZONE_TYPE.UTC, {
      keepLocalTime: true,
    }),
    toDateTs: toUnixTimestamp(endOfDay, TIMEZONE_TYPE.UTC, {
      keepLocalTime: true,
    }),
  }
}
...
const getDateRangeWithDateTypeIsRange = (): {
  ...
  const handler =
    dateRangePresetHandlers[DATE_RANGE_PRESETS.FROM_FIRST_DAY_OF_MONTH_TO_TODAY]
  const [startDate, endDate] = handler()

  return {
    fromDateTs: toUnixTimestamp(startDate, TIMEZONE_TYPE.UTC, {
      keepLocalTime: true,
    }),
    toDateTs: toUnixTimestamp(endDate, TIMEZONE_TYPE.UTC, {
      keepLocalTime: true,
    }),
  }
}
...
const handlePrint = async (): Promise<void> => {
  const sections = []
  if (serviceSalesChartRef.value)
    sections.push(serviceSalesChartRef.value.getPrintConfiguration())
  if (serviceSalesTableRef.value)
    sections.push(serviceSalesTableRef.value.getPrintConfiguration())
