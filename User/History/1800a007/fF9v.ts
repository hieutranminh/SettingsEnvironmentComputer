...
import type {
  ...
  ServiceSalesByItemReportResult,
...
export const useServiceSalesByItemReport = () => {
  ...
  // Computed
  const hasData = computed(() => !!(reportData.value?.isOK && reportData.value?.result?.serviceSalesByItemItems?.length))
  const items = computed(() => (reportData.value?.result?.serviceSalesByItemItems ?? []) as ServiceSalesByItemReportItem[])
