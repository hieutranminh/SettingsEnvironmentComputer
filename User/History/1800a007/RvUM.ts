...
export const useServiceSalesByItemReport = () => {
  ...
  // Computed
  const hasData = computed(() => !!(reportData.value?.isOK && reportData.value?.data?.length))
  const items = computed(() => (reportData.value?.data ?? []) as ServiceSalesByItemReportItem[])
   ...
   */
  const fetchReport = async (filters: ServiceSalesByItemFilterInterface): Promise<ServiceSalesByItemReportResponse | null> => {
    try {
      startLoading(true)
      error.value = null
