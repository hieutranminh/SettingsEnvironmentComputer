...
const convertedToDate = computed<Date>(() => {
  ...
  return props.toDateTs ? fromUnixTimestamp(props.toDateTs).subtract(1, 'day').toDate() : getCurrentDate().toDate()
})
 ...
 */
const emitDateTypeChange = (value: DateType) => {
 ...
 * @param endDate
 */
const createDateRangeTs = (startDate: Date, endDate: Date) => ({
 ...
 */
const handleFromDateChange = (value: DateValue) => {
  if (!isValidDate(value)) return
  const startOfDay = getStartOf(value, 'day').toDate()
 ...
 */
const handleToDateChange = (value: DateValue) => {
  if (!isValidDate(value)) return
  const endOfDay = getEndOf(value, 'day').toDate()
 ...
 * @param dateRange
 */
const emitDateChange = (dateRange: { fromDateTs: number; toDateTs: number }) => {
