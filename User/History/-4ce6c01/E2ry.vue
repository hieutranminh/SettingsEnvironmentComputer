...
const convertedFromDate = computed<Date>(() => {
  return props.fromDateTs
    ? fromUnixTimestamp(props.fromDateTs).toDate()
...
const last12MonthsRange = computed((): { minDate: Date; maxDate: Date } => {
  ...
  const { start: minDateDayjs, end: maxDateDayjs } =
    getLastMonthsRange(RANGE_MONTHS)
...
const isPreviousMonthDisabled = computed((): boolean => {
  if (!convertedFromDate.value || !isValidDate(convertedFromDate.value))
    return true
  const previousMonth = subtractTime(
    convertedFromDate.value,
    1,
    'month',
  ).toDate()
 ...
 * @param dateRange
 */
const emitDateChange = (dateRange: {
  fromDateTs: number
  toDateTs: number
}): void => {
