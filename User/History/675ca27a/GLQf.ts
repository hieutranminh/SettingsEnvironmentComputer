...
const getDomainBasedConfig = (hostname: string): RuntimeConfig => {
  ...
  const buildEnvironment = import.meta.env.VITE_ENVIRONMENT as string
  const buildDebug = import.meta.env.VITE_DEBUG === 'true'
  const buildSourceMap = import.meta.env.VITE_SOURCE_MAP === 'true'
  const buildApiBaseUrl = import.meta.env.VITE_API_BASE_URL as string

  if (hostname.includes('dev') || buildEnvironment === 'development') {
    return {
      ...baseConfig,
      environment: 'development',
      debug: buildDebug !== undefined ? buildDebug : true,
      sourceMap: buildSourceMap !== undefined ? buildSourceMap : true,
      features: {
        analytics: false,
        logging: true,
        debugging: true,
      ...
      api: {
        baseUrl: buildApiBaseUrl || 'https://api-dev.hieu.com',
        timeout: 30000,
        retryAttempts: 3,
        enableMocking: true,
        version: 'v1'
  ...
  else if (hostname.includes('stag') || buildEnvironment === 'staging') {
    return {
      ...baseConfig,
      environment: 'staging',
      debug: buildDebug !== undefined ? buildDebug : false,
      sourceMap: buildSourceMap !== undefined ? buildSourceMap : true,
      features: {
        analytics: true,
        logging: true,
      ...
      api: {
        baseUrl: buildApiBaseUrl || 'https://api-staging.hieu.com',
        timeout: 15000,
        retryAttempts: 2,
        enableMocking: false,
        version: 'v1'
    ...
    return {
      ...
      debug: buildDebug !== undefined ? buildDebug : false,
      sourceMap: buildSourceMap !== undefined ? buildSourceMap : false,
      features: {
        analytics: true,
        logging: false,
        debugging: false,
      ...
      api: {
        baseUrl: buildApiBaseUrl || 'https://api.hieu.com',
        timeout: 10000,
        retryAttempts: 1,
        enableMocking: false,
        version: 'v1'
