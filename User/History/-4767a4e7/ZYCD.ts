...
export const validateExceedOneMonthRange = (fromDate: dayjs.Dayjs, toDate: dayjs.Dayjs): boolean => {
  const diffInMonths = toDate.diff(fromDate, 'month')
  return diffInMonths >= 1
}

export const validateToDateNotBeforeFromDate = (fromDate: dayjs.Dayjs, toDate: dayjs.Dayjs): boolean => {
  if (!toDate) return false

  const isToDateAfterFromDate = isAfter(toDate, fromDate)
  if (isToDateAfterFromDate) return true
