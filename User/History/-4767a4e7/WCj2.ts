...
export const formatDatePure = (
  ...
  const targetTimezone = timezone ?? (getTimezoneByCountryCode(shopCountry) as TimezoneType)
  const dayjsDate = convertTimezone(date, targetTimezone, { keepLocalTime }).locale(userLanguage)

  if (customFormat) return dayjsDate.format(customFormat)

  const patterns = FORMAT_PATTERNS[locale] ?? FORMAT_PATTERNS.en
  const formatMap: Record<string, string | (() => string)> = {
    date: patterns.date,
    time: patterns.time,
    datetime: patterns.datetime,
    yearMonth: patterns.yearMonth,
    relative: () => formatRelativeTimePure(date, { locale, userLanguage, shopCountry }),
  }

  const formatPattern = formatMap[format]
  if (formatPattern) {
    return typeof formatPattern === 'function' ? formatPattern() : dayjsDate.format(formatPattern)
  }

  return dayjsDate.format(includeTime ? patterns.datetime : patterns.date)
}
