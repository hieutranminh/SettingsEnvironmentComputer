...
export const createTimezoneConverter = (
  timezone: TimezoneType,
  defaultOptions: ITimezoneConvertOptions = {},
) => {
  return (
    date: dayjs.Dayjs | Date | string,
    options: ITimezoneConvertOptions = {},
...
export const dateRangePresetHandlers = {
  ...
  [DATE_RANGE_PRESETS.FROM_TODAY_TO_NEXT_6_DAYS]: () => {
    ...
    const endOf6DaysLater = getEndOf(
      addTime(currentDate, DAYS_TO_ADD_FOR_RANGE, 'day'),
      'day',
    ).toDate()
...
export const validateExceedOneYearRange = (fromDate: dayjs.Dayjs, toDate: dayjs.Dayjs): boolean => {
  const diffInYears = toDate.diff(fromDate, 'year')
