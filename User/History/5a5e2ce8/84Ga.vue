        ...
        plugins: {
          datalabels: {
            ...
            formatter: function(value, data) {
              ...
              const isLastDataset = data.datasetIndex == datasets_number - 1
              const isFirstDataset = data.datasetIndex == 0
              let tmp_value = formatMoney(value,0)

              // just get labels from datasets once because labels same of all datasets
              if(isChartBarStacked && isLastDataset){
                tmp_value = options.custom_data[data.dataIndex]
              }
              else if((!isFirstDataset || isChartBarStacked && !isLastDataset)) {
                tmp_value = ''
              }
