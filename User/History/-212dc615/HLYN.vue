    ...
    <template #content>
      ...
      >
        ...
        <ColumnGroup type="header">
          <Row>
            <Column :header="$t('service-sales.label-staff')" headerClass="bg-gray" style="width: 25%" />
...
const transformedData = computed<ServiceSalesReportItem[]>(() => {
  if (!props.data || props.data.length === 0) {
    return []
  }

  const totalAmount = sumBy(props.data, (x) => x?.amount ?? 0)

  return props.data.map((item) => ({
    ...item,
    key: item.key === 'NONE' ? t('general.label-no-input') : item.key,
    ratio: (item.amount / totalAmount) * 100,
  }))
})
