...
export const gitlabApi = {
   ...
   * Endpoint: POST api/cmd/v1/GitLab/CreateMerge
   */
  async createMerge(request: CreateMergeRequest): Promise<ApiResponse<CreateMergeResult>> {
    return gatewayService.command<CreateMergeResult>(GITLAB_SERVICE, 'CreateMerge', request)
  },
   ...
   * Endpoint: POST api/read/v1/GitLab/MergeRequest
   */
  async getMergeRequests(
    request: MergeRequestListRequest,
  ): Promise<ApiResponse<MergeRequestListResult>> {
    return gatewayService.read<MergeRequestListResult>(GITLAB_SERVICE, 'MergeRequest', request, {
      skipLoading: true,
    })
  },
}
