...
export default {
  ...
  computed: {
    ...
    sales_total_detail_filtered() {
      ...
      console.log('sales_total_detail', this.sales_total_detail)
  ...
  methods: {
    ...
    async loadSalesTotalDetailByStaff() {
      try {
        ...
        console.log('sales_total_detail response', this.sales_total_detail)
      } catch (error) {
        this._showDialogAlert(error.message)
      } finally {
        this.preLoader(false)
    ...
    reloadSalesHistoriesAsync() {
      this.loadSalesByService()
    },

    async loadSalesByService() {
      try {
        this.preLoader()
        const reportApi = new ReportApi()
        ...
        this.tableSalesByService = response.data
      } catch (error) {
        this._showDialogAlert(error.message)
      } finally {
        this.preLoader(false)
    ...
    async loadPaymentsAsync() {
      try {
        ...
        if (this.isMissingSalesSetup(salesSetup)) {
          throw new ApiError(salesSetup.error_messages)
        }

        const salesSetupPaymentMethodSetup = salesSetup?.payment_method_setup ?? []

        this.paymentOptions = [...this.additionalPaymentMethod, ...salesSetupPaymentMethodSetup]
      } catch (error) {
        this._showDialogAlert(error.message)
      }
    },
    ...
    viewDetail(sales) {
      ...
      sales_vm.fields.sales_id = sales.salesId
      sales_vm.fields.client_shop_id = sales.clientShopId
      sales_vm.fields.shop_id = sales.shopId
    ...
    async searchData() {
      this.newStaffValue = this.table_filter.staffId
      this.staffSalesTransferHistoryType = this.initialFilter.staffSalesTransferHistoryType
      this.searchType = this.initialFilter.searchType
