...
/**
 * Merged cells handling parameters
 */
interface IMergedCellsParams {
...
export const useExcelTable = (): IUseExcelTableReturn => {
   ...
   * @param params - Merged cells handling parameters
   * @returns Void
   *
   * Example:
   *   handleMergedCells({ tableContext: context, rowIndex: 1, columnIndex: 2, rowspan: 2, colspan: 3 })
   */
  const handleMergedCells = (params: IMergedCellsParams): void => {
    const { tableContext, rowIndex, columnIndex, rowspan, colspan } = params
   ...
   * @param params - Single cell processing parameters
   * @returns Updated column index after processing
   *
   * Example:
   *   const nextCol = processSingleCell({ tableCell: cell, tableContext: context, rowIndex: 1, columnIndex: 2, cellFormatter: formatHeaderCell })
   */
  const processSingleCell = (params: ISingleCellParams): number => {
    const { tableCell, tableContext, rowIndex, columnIndex, cellFormatter } = params
  ...
  const processTableRow = (
    ...
    cells.forEach((cell) => {
      ...
      currentColumn = processSingleCell({
        tableCell: cell as HTMLTableCellElement,
        tableContext,
        rowIndex,
