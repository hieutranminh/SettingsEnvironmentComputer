...
```typescript
const { filters, debouncedFilters, isDebouncing, updateDateType, updateDateRange } = useBranchSalesFilter({
  autoTrigger: true,
...
```typescript
const { filters, debouncedFilters, hasChanges, resetFilters, nextPage, previousPage } = useBranchSalesFilter({
  debounceMs: 1000,
  autoTrigger: false,
