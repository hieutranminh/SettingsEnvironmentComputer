<template>
  ...
  >
          ...
          <div class="booking-action__form-booking col-12 col-lg-7">
            ...
            >
              <a-form-control class="booking-action__booking-note-textarea">
                ...
                <a
                  :class="voiceNotesClass"
                  @click.prevent="onShowVoiceNotesModal"
                >
                  <b-icon-mic />
                </a>
              </a-form-control>
    ...
    <modal-aha-ai-notes
      :visible="isVoiceNotesModalVisible"
      @cancel="handleVoiceNotesModalCancel"
      @send="handleVoiceNotesSend"
    />
  </a-form-context>
</template>
...
import { BIconArrowRightShort, BIconArrowLeftShort, BIconMic } from 'bootstrap-vue'
...
export default {
  components: {
    ...
    ModalAhaAiNotes:                        () => import('Modules/calendar/components/modal-aha-ai-notes/modal-aha-ai-notes.vue'),
  },
  ...
  data() {
    return {
      ...
      isVoiceNotesModalVisible:           false,
    }
  },
  ...
  computed: {
    ...
    voiceNotesClass() {
      return ['action', 'action--voice', {
        'action--disabled': this.notes.length >= 5000,
      }]
    },
  },
  ...
  methods: {
    ...
    handleNotesInputWithLimit(eventOrValue) {
    ...
    */
    ...
    onShowVoiceNotesModal() {
      if(this.notes.length >= 5000) {
        return
      }

      this.isVoiceNotesModalVisible = true
    },

    handleVoiceNotesModalCancel() {
      this.isVoiceNotesModalVisible = false
    },

    handleVoiceNotesSend(voiceNotes) {
      const currentNotes = this.notes || ''
      const separator = currentNotes ? ' ' : ''
      this.notes = currentNotes + separator + voiceNotes
      this.handleNotesInputWithLimit(this.notes)
    },
  },
}
</script>
