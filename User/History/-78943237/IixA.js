 ...
 * Android handles microphone permission differently and may need special handling
...
export function createSpeechRecognition(options) {
  ...
  if (onResult) {
    recognition.onresult = (event) => {
      ...
      // Read ALL results from the beginning (index 0), not just from resultIndex
      // This ensures consistent behavior across different devices (PC, iPhone, Samsung Galaxy)
      ...
      // Fix for Samsung Galaxy: interimTranscript may contain already-finalized text
      // Remove the overlapping prefix from interimTranscript if it starts with fullTranscript
      if (fullTranscript && interimTranscript && interimTranscript.startsWith(fullTranscript)) {
        interimTranscript = interimTranscript.slice(fullTranscript.length).trim()
      }
