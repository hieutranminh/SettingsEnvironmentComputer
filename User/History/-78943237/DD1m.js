 ...
 * Uses Permissions API first (doesn't create audio stream) to avoid race conditions
 * Falls back to getUserMedia only when permission status is unknown
 * @returns {Promise<boolean>} True if microphone permission is granted
 */
export async function checkMicrophonePermission() {
  // Try Permissions API first - doesn't create audio stream, avoids race conditions
  if (navigator.permissions && navigator.permissions.query) {
    try {
      ...
      // 'prompt' state - need to request permission via getUserMedia
    } catch (permissionError) {
      // Permissions API not supported for microphone, fall through to getUserMedia
    }
  }

  // Fallback: Use getUserMedia only when Permissions API unavailable or state is 'prompt'
...
export function createSpeechRecognition(options) {
  ...
  function isSimilarToRecent(text) {
    ...
    recentFinalTexts = recentFinalTexts.filter(item => now - item.time < 3000)
    ...
    for (const recent of recentFinalTexts) {
      ...
      if (recentNorm.includes(normText) || normText.includes(recentNorm)) {
  ...
  function isExactDuplicate(transcript) {
    ...
    // Accumulated already contains this transcript
    if (normAccum.includes(norm)) return true
