...
export function createSpeechRecognition(options) {
  ...
  if (onResult) {
    // Android/Samsung fix: Track final transcripts by index to avoid duplicates
    // Key: result index, Value: transcript text
    const finalTranscriptsByIndex = new Map()
    ...
    recognition.onresult = (event) => {
      ...
      for (let i = 0; i < event.results.length; i += 1) {
        ...
        if (result.isFinal) {
          // Store/update the final transcript for this index
          // This handles cases where Android sends the same index multiple times
          if (transcript) {
            finalTranscriptsByIndex.set(i, transcript)
          }
        } else {
      ...
      // Build full final transcript from all stored transcripts (ordered by index)
      const sortedIndices = Array.from(finalTranscriptsByIndex.keys()).sort((a, b) => a - b)
      const fullFinalTranscript = sortedIndices
        .map(idx => finalTranscriptsByIndex.get(idx))
        .join(' ')

      onResult({
        interimTranscript:   interimTranscript.trim(),
        fullFinalTranscript: fullFinalTranscript,
      })
    }
  }
