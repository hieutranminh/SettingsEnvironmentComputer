...
export function createSpeechRecognition(options) {
  ...
  // Track processed result indices to avoid duplicates on Android/Samsung
  ...
  if (onResult) {
    recognition.onresult = (event) => {
      ...
      // Android/Samsung fix: Use lastProcessedIndex to track what we've already seen
      for (let i = 0; i < event.results.length; i += 1) {
        const result = event.results[i]
        const transcript = result[0].transcript.trim()

        if (result.isFinal) {
          // Only process if this is a new final result (index > lastProcessedIndex)
          if (i > lastProcessedIndex) {
            // Android duplicate fix: Check if this text is already in accumulated text
            if (!accumulatedFinalText.endsWith(transcript) && accumulatedFinalText !== transcript) {
              // Check if new transcript contains accumulated text (cumulative result)
              if (transcript.startsWith(accumulatedFinalText) && accumulatedFinalText.length > 0) {
                // Extract only the new part
                newFinalTranscript = transcript.slice(accumulatedFinalText.length).trim()
              } else {
