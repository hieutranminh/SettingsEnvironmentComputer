...
export function createSpeechRecognition(options) {
  ...
  if (onResult) {
    ...
    const processedFinalIndices = new Set()

    recognition.onresult = (event) => {
      let interimTranscript = ''
      const finalParts = []

      // Process results and track which indices we've seen as final
      for (let i = 0; i < event.results.length; i += 1) {
        const result = event.results[i]
        const transcript = result[0].transcript.trim()

        if (result.isFinal) {
          // Only add if we haven't processed this index as final before
          // Android may fire multiple events for the same final result
      ...
      // Build full final transcript from tracked parts
      const fullFinalTranscript = finalParts.join(' ')
