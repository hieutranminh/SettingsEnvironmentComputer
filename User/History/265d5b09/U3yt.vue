<template>
  <div class="contents send-message-action clearfix">
    ...
    >
      ...
      >
        ...
        >
          ...
          <div
            v-if="is_kr && !isRelatedBookingDeposit && !isConsentType"
      ...
      >
        ...
        >
          ...
          <template v-if="is_kr && !isRelatedBookingDeposit && !isConsentType">
            <span
    ...
    <div
      v-if="!isRelatedBookingDeposit && !isConsentType"
      :class="{'mt20' : is_kr && (type == options.messages_enums.send_page.multi_booking || type == options.messages_enums.send_page.deposit_guide || type == options.messages_enums.send_page.deposit_payment_confirm)}"
...
export default {
  ...
  computed: {
    ...
    is_kr(){
      if(this.shop_data.country == options.country_code.kr) return true
      else return false
    },
    rejection_message_info(){
      if(this.shop_data.country == options.country_code.kr) return this.$t('messages.rejection-message-info-kr')
      else return this.$t('messages.rejection-message-info-vn')
    },
    shopLanguage(){
      if(this.shop_data.country == options.country_code.kr) return options.language.korean
      else return options.language.english
    },
    isRelatedBookingDeposit(){
    ...
    isMultiTarget(){
      if(this.type == options.messages_enums.send_page.multi_booking
    ...
    isConsentType(){
      if(this.type == options.messages_enums.send_page.consent_form)
        return true
      else
        return false
    },
    placement(){
      if(this.isMobile()) return 'bottomleft'
      else return 'bottom'
    },
    ...
    replacedAlimtalkContents() {
      if(checkNullAndEmptyAndUndefined(this.contents))
        ...
        let result = getReplacedEmoticonAlimTalkContents(this.contents)
        result += '<br><br><button class="consent-link-btn">' + this.$t('messages.consent-view') + '</button>'
    ...
    sendTypeText() {
      if(this.text_message.fields.message_type === options.messages_enums.message_type.kao)
        return this.$t('messages.kakao-talk')
      else
        return this.$t('messages.text-message')
    },
  ...
  watch: {
    target_date(){
      if(formatDateBySetting(this.target_date) == formatDateBySetting(this.today)){
        if(this.today.getHours() >= 22 || this.today.getHours() <= 8) this.target_hour = '08'
  ...
  beforeMount() {
    ...
    this.randomId = Math.floor(Math.random() * 10000) // temp solution
  },
  ...
  methods: {
    ...
    async onLoadForm() {
      try {
        ...
        if(this.isMultiTarget && this.is_kr)
    ...
    onAddRejectMessage(){
      if(this.is_kr) this.contents += this.$t('messages.reject-message-kr')
    ...
    async onSendTestMessage(mobile_numbers) {
      ...
      // Check message contents contain '000샵'
      if (this.contents.indexOf(this.$t('message-autos.shop-name-1')) !== -1 || this.contents.indexOf(this.$t('message-autos.shop-name-2')) !== -1) {
        this._showDialogAlert(new Array(this.$t('message-autos.shop-name-check')))
        this.hideDialogById('send-test-message-modal')
        return
      }

      this.test_mobile_numbers = []
      mobile_numbers.forEach(e => {
        if (!emptyValue(e)) this.test_mobile_numbers.push(e)
      })
    ...
    async onCheckLengthBeforeSend(){
      ...
      if(contentAll == adText1 + adText2 || contentAll == adText1 || contentAll == adText2){
        this.showAlert(new Array(this.$t('validate_messages.include-ad-text')))
        return
      }

      if(this.text_message.fields.target_count == 0){
        this.showAlert(new Array(this.$t('validate_messages.send-text-message-target')))
        return
      }
      ...
      if(this.isConsentType) {
        if(this.text_message.fields.message_type !== options.messages_enums.message_type.kao) {
          ...
          if(this.contents.indexOf(this.$t('variable-data.consent-form-link-var', this.shopLanguage)) === -1) {
          ...
          const isDuplicateConsentLink = this.contents.match(consentLinkRegex)?.length > 1
          if (isDuplicateConsentLink) {
            this._showDialogAlert(this.$t('consent.enter-one-consent-link-alert'))
            return
          }

          // Check space around consent link
          const spaceBeforeConsentLinkRegex = new RegExp(`(\\s|^)(${this.$t('variable-data.consent-form-link-reg', this.shopLanguage)})`, 'g')
          const spaceAfterConsentLinkRegex = new RegExp(`${this.$t('variable-data.consent-form-link-reg', this.shopLanguage)}(?!\\s|$)`, 'g')

          const hasSpaceBeforeConsentLink = (() => {
            const matches = this.contents.matchAll(spaceBeforeConsentLinkRegex)

            for (const match of matches) {
              return match[1] === ' ' || match[1] === '\n' || match[1] === ''
            }
          })()
          const noHasSpaceAfterConsentLink = this.contents.match(spaceAfterConsentLinkRegex)

          const spaceAroundConsentLinkErrors = []
          if (!hasSpaceBeforeConsentLink) {
            spaceAroundConsentLinkErrors.push(this.$t('consent.no-has-space-before-consent-link'))
          }

          if (noHasSpaceAfterConsentLink) {
            spaceAroundConsentLinkErrors.push(this.$t('consent.no-has-space-after-consent-link'))
          }

          if (spaceAroundConsentLinkErrors.length) {
            this._showDialogAlert(spaceAroundConsentLinkErrors)
            return
          }
        }
        ...
        try {
          ...
          // Consent Link : Salon solution home url + /sign-consent-form?messageLinkKey= + LinkKey(Max 23Length) + &countryCode=XX
          ...
          if(this.text_message.fields.message_type !== options.messages_enums.message_type.kao) {
            ...
            if(this.contents?.length + totalLength > 2000) {
            ...
            if(this.current_bytes + totalLength > this.max_bytes) {
              this.showAlert([this.$t('validate_messages.max-message-contents-bytes').replace('{0}', this.max_bytes).replace('{1}', totalLength)])
              return
            }
          }
        } catch (error) {
      ...
      const fullType = this.$t(options.messages_enums.message_type_full.find(x => x.value == this.text_message.fields.message_type).text)
      ...
      if(this.isConsentType) {
        ...
        if(this.text_message.fields.message_type == options.messages_enums.message_type.lms
        && this.current_bytes < this.text_fees.fields.sms_max_bytes
        && this.text_message.fields.contents.indexOf(this.$t('variable-data.client-name-var')) == -1){
          this.alert_type = options.messages_enums.alert_type.message_type_change
          if(this.text_message.fields.target_count == 1) this.alert_data = [ this.$t('messages.send-message-change-type-one').replace('{0}', this.text_fees.fields.sms_max_bytes) ]
          else this.alert_data = [ this.$t('messages.send-message-change-type-multi').replace('{0}', this.text_fees.fields.sms_max_bytes).replace('{1}', this.text_message.fields.target_count) ]
        } else {
          this.alert_type = options.messages_enums.alert_type.none
          if(this.text_message.fields.target_count == 1) this.alert_data = [ this.$t('messages.send-message-one').replace('{0}', fullType) ]
          else this.alert_data = [ this.$t('messages.send-message-multi').replace('{0}', fullType).replace('{1}', this.text_message.fields.target_count) ]
        }
      }

      let sendHour = 0
      if(this.text_message.fields.is_send_now)
        sendHour = convertDateToTimezone(new Date()).getHours()
      else
        sendHour = Number(this.target_hour)

      if(this.isMultiTarget && this.is_kr && (sendHour >= 21 || sendHour < 8)) {
        const firstRowIndex = this.contents.indexOf('\n')
        if(firstRowIndex === -1) {
          if(this.contents.indexOf(this.$t('messages.multi-message-info1')) === -1)
            this.isIncludeSpam = true
          else
            this.isIncludeSpam = false
        } else {
          const firstRowContents = this.contents.substring(0, firstRowIndex)
          if(firstRowContents.indexOf(this.$t('messages.multi-message-info1')) === -1)
            this.isIncludeSpam = true
          else
            this.isIncludeSpam = false
        }
