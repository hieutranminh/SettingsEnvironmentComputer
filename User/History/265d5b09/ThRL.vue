  ...
  methods: {
    ...
    async loadSenderPhones() {
      try {
        ...
        this.sender_phone_select.rows = items

        if (items.length > 0) {
          const autoSender = items.find(x => x.is_auto_sender)
          this.text_message.fields.sender_phone = autoSender?.sender_phone ?? items[0].sender_phone
        }
      } catch (error) {
        this._showDialogAlert(error.message)
      }
    ...
    onChangeMessageType(func = null) {
      const { message_type } = options.messages_enums
      const messageType = this.text_message.fields.message_type

      const messageTypeConfig = {
        [message_type.sms]: {
          maxBytes:  this.text_fees.fields.sms_max_bytes,
          unitPrice: this.text_fees.fields.sms_fee,
        },
        [message_type.lms]: {
          maxBytes:  this.text_fees.fields.lms_max_bytes,
          unitPrice: this.text_fees.fields.lms_fee,
        },
        [message_type.mms]: {
          maxBytes:  this.text_fees.fields.lms_max_bytes,
          unitPrice: this.text_fees.fields.mms_fee,
        },
      }

      const config = messageTypeConfig[messageType]
      if (config) {
        this.max_bytes = config.maxBytes
        this.targets.unit_price = config.unitPrice
      }

      this.targets.total_cost = this.targets.target_count * this.targets.unit_price
      this.message_helper = new MessageHelper(messageType, this.max_bytes, this.text_fees.fields.lms_max_bytes)
    ...
    async submitActionAsync(isTest = false) {
      const tempIsSendNow = this.text_message.fields.is_send_now

      // Prepare message for test or scheduled sending
      if (isTest) {
        await this.prepareTestMessage()
      } else if (!this.text_message.fields.is_send_now) {
        this.setScheduledSendTime()
      }
      ...
      // Setup receivers and variable data
      this.text_message.fields.variable_data = null
      this.setupReceiversByType(isTest)

      // Send message
      await this.sendMessageAsync(isTest, tempIsSendNow)

      // Reset fields
      this.resetMessageFields()
    },

    async prepareTestMessage() {
      ...
      this.text_message.fields.is_send_now = true
      this.text_message.fields.receivers = this.test_mobile_numbers.map(phone => ({
    ...
    setScheduledSendTime() {
      ...
      this.text_message.fields.target_date_ts = convertDateFromTimezoneToTimestamp(this.target_date)
        + (SECONDS_PER_HOUR * Number(this.target_hour))
        + (SECONDS_PER_MINUTE * Number(this.target_minute))
    },

    setupReceiversByType(isTest) {
      const clientNameVar = this.$t('variable-data.client-name-var', this.shopLanguage)

      if (isTest) {
        if (!emptyValue(this.text_message.fields.contents)) {
          this.text_message.fields.contents = replaceAll(this.text_message.fields.contents, clientNameVar, '')
        }
        return
      }

      const { send_page, message_source_type } = options.messages_enums

      // Client type
      if (this.type === send_page.client) {
        this.setupClientReceiver(clientNameVar)
        return
      }

      // Booking deposit type
      if (this.isRelatedBookingDeposit) {
        this.setupBookingDepositReceiver()
        return
      }

      // Unregister client types
      if (this.type === send_page.cid_unregister_client || this.type === send_page.unregister_client) {
        this.text_message.fields.source_type = message_source_type.client
        this.text_message.fields.receivers.push({ receiver_phone: this.receivers })
        return
      }

      // Multi booking type
      if (this.type === send_page.multi_booking) {
        this.setupMultiBookingReceiver()
        return
      }

      // Client management or campaign type
      if (this.type === send_page.client_management || this.type === send_page.campaign) {
        this.setupCampaignReceiver(clientNameVar)
        return
      }

      // Consent type
      if (this.isConsentType) {
        this.setupConsentReceiver()
      }
    },

    setupClientReceiver(clientNameVar) {
      this.text_message.fields.source_type = options.messages_enums.message_source_type.client
      this.text_message.fields.variable_data = clientNameVar
      this.text_message.fields.contents = replaceAll(this.text_message.fields.contents, clientNameVar, '#$var1#$')
      this.text_message.fields.receivers.push({
    ...
    setupCampaignReceiver(clientNameVar) {
      this.text_message.fields.source_type = options.messages_enums.message_source_type.client
      this.text_message.fields.receivers = this.multi_data
      this.text_message.fields.contents = replaceAll(this.text_message.fields.contents, clientNameVar, '#$var1#$')
      this.text_message.fields.variable_data = clientNameVar
    ...
    async sendMessageAsync(isTest, tempIsSendNow) {
      this.preLoader(true)

      try {
        const textMessageApi = new TextMessageApi()
        const result = this.isConsentType
          ? await textMessageApi.createConsentTextMessageAsync(this.buildConsentMessageData())
          : await textMessageApi.createTextMessageAsync(this.text_message.fields)

        if (result.is_ok) {
          this.handleSendSuccess(isTest, tempIsSendNow)
        } else {
    ...
    buildConsentMessageData() {
      return {
        ...this.text_message.fields,
    ...
    handleSendSuccess(isTest, tempIsSendNow) {
      if (isTest) {
        this.showAlert([this.$t('messages.test-send-message-success')])
    ...
    revokeMMSImage() {
      if (this.mms_image?.startsWith('blob')) {
    ...
    onSetReceivers() {
      // Case: Unregister clients - parse phones from textarea
      const phones = this.receiver_phones.split('\n')
        .filter(phone => !emptyValue(phone))
        .map(phone => phone.replace(/-/g, ''))

      const existingPhones = new Set(
        this.text_message.fields.receivers.map(r => r.receiver_phone),
      )

      const newReceivers = phones
        .filter(phone => !existingPhones.has(phone))
        .map(phone => ({ receiver_phone: phone }))

      this.text_message.fields.receivers.push(...newReceivers)
    },
    ...
    async loadNetMoneyBalance() {
      ...
      if (result.is_ok) {
        this.targets.netmoney_balance = result.data.net_money_balance
      } else {
        this.showAlert(result.error_messages)
      }
    },

    async onChangeListType(listType, isCreate = false) {
      this.list_type = listType

      if (listType === options.messages_enums.list_type.my_message) {
        await this.onLoadMyMessageCategories(isCreate)
        return
      }

      // Load samples if not already loaded
      if (!this.isLoadSample) {
        await this.onLoadMainSample()
      }
      this.onLoadSample()
    },
    ...
    onShowSpamPopup(type) {
      ...
      const routeData = this.$router.resolve({ name: 'spam-info-popup', query: { type } })
      const windowFeatures = `location=yes, left=${POPUP_CONFIG.left}, top=${POPUP_CONFIG.top}, width=${POPUP_CONFIG.width}, height=${POPUP_CONFIG.height}`
      window.open(routeData.href, `spam-popup-${type}`, windowFeatures)
    },

    onAddRejectMessage() {
      const rejectMessage = this.isKr
        ? this.$t('messages.reject-message-kr')
        : this.$t('messages.reject-message-vn')
      this.contents += rejectMessage
      setTimeout(() => this.onCheckLength(), 4)
    },
    ...
    replaceVariableToBooking() {
      const variableReplacements = [
        { key: 'variable-data.client-name-var', replacement: '#$var1#$' },
        { key: 'variable-data.booking-date-var', replacement: '#$var2#$' },
        { key: 'variable-data.booking-mmdd-var', replacement: '#$var3#$' },
        { key: 'variable-data.booking-time-var', replacement: '#$var4#$' },
      ]

      return variableReplacements.reduce(
        (contents, { key, replacement }) => replaceAll(contents, this.$t(key, this.shopLanguage), replacement),
        this.text_message.fields.contents,
      )
    },
    ...
    setAlertData() {
      ...
      const fullType = this.$t(
        options.messages_enums.message_type_full.find(x => x.value === messageType).text,
      )
      ...
      const isLmsCanBeDowngraded = messageType === options.messages_enums.message_type.lms
      ...
      if (isLmsCanBeDowngraded) {
        ...
        const smsMaxBytes = this.text_fees.fields.sms_max_bytes
        this.alert_data = targetCount === 1
          ? [this.$t('messages.send-message-change-type-one').replace('{0}', smsMaxBytes)]
          : [this.$t('messages.send-message-change-type-multi').replace('{0}', smsMaxBytes).replace('{1}', targetCount)]
      } else {
        this.alert_type = options.messages_enums.alert_type.none
        this.alert_data = targetCount === 1
    ...
    onShowModalSendMessageAhaAi() {
      // Blur aha ai button to hide tooltip
      const ahaAiButton = this.$refs.ahaAiButton
      console.log('ahaAiButton', ahaAiButton)
      if (ahaAiButton && ahaAiButton.$el) {
        ahaAiButton.$el.blur()
      }
