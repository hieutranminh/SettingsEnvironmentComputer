      ...
      >
        ...
        <ColumnGroup v-if="enhancedTableRows.length" type="footer">
          <Row>
            ...
            <template v-for="(monthTotal, monthIndex) in footerTotals.monthlyTotals" :key="`footer-${monthTotal.month}`">
 ...
 * @example footerTotals.value // { totalNewClients: 100, totalRepeatClients: 45, totalRepeatPercentage: "45.00%", monthlyTotals: [...] }
 */
const footerTotals = computed<FooterTotals>((): FooterTotals => {
  ...
  try {
    // Calculate total new clients across all staff
    const totalNewClients = enhancedTableRows.value.reduce(
      (sum: number, row: EnhancedTableRow): number => sum + row.totalNewClients,
      0,
    )

    // Calculate total repeat clients across all staff
    const totalRepeatClients = enhancedTableRows.value.reduce(
      (sum: number, row: EnhancedTableRow): number => sum + row.summary.totalClients,
      0,
    )

    // Calculate total repeat percentage
    const totalRepeatPercentage = totalNewClients > 0 
      ? formatPercentage((totalRepeatClients / totalNewClients) * 100)
      : ''

    // Calculate monthly totals for each repeat month
    const monthlyTotals = repeatMonths.value.map((month: number, monthIndex: number) => {
      const monthlyClients = enhancedTableRows.value.reduce(
        (sum: number, row: EnhancedTableRow): number => 
          sum + (row.repeatColumns[monthIndex]?.clients ?? 0),
        0,
      )

      const monthlyPercentage = totalNewClients > 0
        ? formatPercentage((monthlyClients / totalNewClients) * 100)
        : ''
    ...
    // Return empty totals if calculation fails
    return {
      totalNewClients: 0,
      totalRepeatClients: 0,
      totalRepeatPercentage: '',
