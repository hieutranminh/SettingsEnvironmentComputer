  ...
  <Card>
    <template #content>
      ...
      >
        ...
        <Column>
          <template #body="{ data }">
            {{ formatNameWithNoInput(data.staffName) }}
          </template>
        </Column>
        ...
        <template v-for="(month, index) in repeatMonths" :key="`col-${month}`">
          <Column>
            <template #body="{ data }">
              {{ data.repeatColumns[index]?.clients ?? 0 }}
            </template>
          </Column>
          <Column>
            <template #body="{ data }">
              {{ data.repeatColumns[index]?.percentage ?? '' }}
            </template>
          </Column>
        </template>
...
// Constants
import { NEW_CLIENTS_REPEAT_CONSTANTS, PRINT_TYPE } from '@/constants'
import type { NewClientsRepeatReportItem, RepeatColumn, TableRow } from '@/types/client-report/NewClientsRepeat'
...
// Helper functions
const calculateRowSummary = (item: NewClientsRepeatReportItem): { totalClients: number; percentage: number } => {
