<template>
  <Card>
    <template #content>
      ...
      >
        ...
        <ColumnGroup type="header">
          <Row>
            <Column
              :rowspan="2"
              :header="$t('new-clients-repeat.label-staff')"
              headerClass="bg-gray"
            />
            ...
            <Column
              :colspan="2"
              :header="$t('new-clients-repeat.label-repeat-total')"
              headerClass="bg-gray"
            />
        ...
        <Column>
          <template #body="{ data }">{{ data.summary.percentage ? formatPercentage(data.summary.percentage) : ' ' }}</template>
        </Column>
        ...
        <ColumnGroup v-if="tableRows.length" type="footer">
          <Row>
            ...
            <template
              v-for="monthTotal in totals.monthlyTotals"
              :key="`footer-${monthTotal.month}`"
            >
      ...
      <div ref="emptyDataRef" class="empty-data" v-else>
...
interface IProps {
  data?: INewClientsRepeatReportItem[]
}
...
// Helper functions
const calculateRowSummary = (
  item: INewClientsRepeatReportItem,
): { totalClients: number; percentage: number } => {
  const totalClients = item.repeatInfos.reduce((sum, info) => sum + info.totalRepeatClients, 0)
  const percentage =
    item.totalNewClients !== 0
      ? (totalClients / item.totalNewClients) * NEW_CLIENTS_REPEAT_CONSTANTS.PERCENTAGE_MULTIPLIER
      : 0

  return { totalClients, percentage }
}

const createRepeatColumns = (item: INewClientsRepeatReportItem): IRepeatColumn[] => {
  ...
  return repeatMonths.value.map((month) => {
    ...
    const percentage =
      item.totalNewClients !== 0
        ? (clients / item.totalNewClients) * NEW_CLIENTS_REPEAT_CONSTANTS.PERCENTAGE_MULTIPLIER
        : 0
...
): Array<{ month: number; clients: number; percentage: string }> => {
...
const calculatePercentage = (numerator: number, denominator: number): string => {
  ...
  return numerator !== 0
    ? formatPercentage(
        (numerator / denominator) * NEW_CLIENTS_REPEAT_CONSTANTS.PERCENTAGE_MULTIPLIER,
      )
    : ''
}
...
const formatStaffName = (name: string): string => {
  switch (name) {
    case 'NONE':
      return formatNameWithNoInput(name)
    case 'MULTI':
      return formatNameWithMultiStaffs(name)
    default:
      return name
  }
}
...
const getPrintConfiguration = (): IPrintSection => {
  ...
  return {
    refType: hasDataTable.value ? PRINT_TYPE.TABLE : PRINT_TYPE.CANVAS,
