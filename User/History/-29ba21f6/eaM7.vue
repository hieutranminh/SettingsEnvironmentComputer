...
const calculateMonthlyTotals = (rows: EnhancedTableRow[], totalNewClients: number): RepeatColumn[] => {
...
const calculatePercentage = (numerator: number, denominator: number): string => {
  const percentage = numerator ? formatPercentage((numerator / denominator) * 100) : ''
  return denominator > 0 ? percentage : ''
}
