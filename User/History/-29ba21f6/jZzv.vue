      ...
      >
        ...
        <ColumnGroup v-if="enhancedTableRows.length" type="footer">
          <Row>
            ...
            <Column :footer="footerTotals.totalNewClients" footerClass="bg-gray" />
            <Column :footer="footerTotals.totalRepeatClients" footerClass="bg-gray" />
...
const footerTotals = computed<FooterTotals>((): FooterTotals => {
  ...
  try {
    ...
    const totalRepeatPercentage =
      totalNewClients > 0 ? formatPercentage((totalRepeatClients / totalNewClients) * 100) : ''

    // Calculate monthly totals for each repeat month
    const monthlyTotals = repeatMonths.value.map((month: number, index: number) => {
      const monthlyClients = enhancedTableRows.value.reduce(
        (sum: number, row: EnhancedTableRow): number => sum + (row.repeatColumns[index]?.clients ?? 0),
        0,
      )

      const monthlyPercentage = totalNewClients > 0 ? formatPercentage((monthlyClients / totalNewClients) * 100) : ''
