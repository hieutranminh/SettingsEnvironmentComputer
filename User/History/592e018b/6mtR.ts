...
export const useMessageDialog = () => {
  ...
  const showConfirm = (
    ...
    const confirmText = options?.confirmText || globalConfig.defaultConfirmText || 'Confirm'
    const cancelText = options?.cancelText || globalConfig.defaultCancelText || 'Cancel'
    const confirmSeverity = options?.confirmSeverity || globalConfig.defaultConfirmSeverity || 'primary'
    const cancelSeverity = options?.cancelSeverity || globalConfig.defaultCancelSeverity || 'secondary'
  ...
  const showLoading = (
    ...
    // Note: This would need to be implemented with a proper dialog instance reference
    return () => {
      // For now, we'll need to implement this differently
      // The dialog will close automatically when the component unmounts
    }
  }
