...
const processSingleDate = (value: DateValue, unit: TimeUnit): void => {
  ...
  const startOfUnit = getStartOf(value, unit, TIMEZONE_TYPE.UTC)
  const endOfUnit = getEndOf(value, unit, TIMEZONE_TYPE.UTC)
  console.log('startOfUnit - ', startOfUnit.toDate())
  console.log('endOfUnit - ', endOfUnit.toDate())
...
const dateRangePresetHandlers = {
  [DATE_RANGE_PRESETS.FROM_FIRST_DAY_OF_MONTH_TO_TODAY]: () => {
    ...
    const startOfMonth = getStartOf(currentDate, 'month', TIMEZONE_TYPE.UTC).toDate()
    const endOfDay = getEndOf(currentDate, 'day', TIMEZONE_TYPE.UTC).toDate()
  ...
  [DATE_RANGE_PRESETS.FROM_TODAY_TO_NEXT_6_DAYS]: () => {
    const currentDate = getCurrentDate()
    const startOfDay = getStartOf(currentDate, 'day', TIMEZONE_TYPE.UTC).toDate()
    const endOf6DaysLater = getEndOf(addTime(currentDate, 6, 'day'), 'day', TIMEZONE_TYPE.UTC).toDate()
...
const emitDateChange = (): void => {
  ...
  emit('dateChange', {
    ...
    fromDateTs: toUnixTimestamp(fromDate),
    toDateTs: toUnixTimestamp(toDate),
  })
}
...
const handleFromDateChange = (value: DateValue): void => {
  if (isValidDate(value)) {
    const startOfUnit = getStartOf(value, 'day', TIMEZONE_TYPE.UTC)
    filterData.value.fromDate = startOfUnit.toDate()
    emitDateChange()
  }
}
...
const handleToDateChange = (value: DateValue): void => {
  if (isValidDate(value)) {
    const endOfUnit = getEndOf(value, 'day', TIMEZONE_TYPE.UTC)
    filterData.value.toDate = endOfUnit.toDate()
    emitDateChange()
  }
}
