...
const reportDownloadFileName = computed(() => {
  console.log('Debug timezone info:', {
    shopCountry: authStore.shop?.country,
    shopTimezone: authStore.shop?.timezone,
    defaultTimezone: useDateFormat().defaultTimezone,
    currentDate: new Date(),
    formattedDate: formatDate(new Date(), { format: 'YYYY-MM-DD_HH_mm_ss' })
  })
  return `${config.value?.title} (${formatDate(new Date(), { format: 'YYYY-MM-DD_HH_mm_ss' })})`
})

const previewContentPdfClass = computed(() => [
  'preview-content-pdf',
  `preview-content-pdf-${config?.value?.orientation}`,
])
...
const handleDownload = (type: string) => {
  if (type === DOWNLOAD_TYPE.PDF) {
    printPreviewStore.downloadPdf(reportDownloadFileName.value)
  } else {
    printPreviewStore.downloadExcel(reportDownloadFileName.value)
  }
}
