  ...
  <template v-if="filters.shopId">
    ...
    <div class="section-container">
      ...
      <div class="section-left">
        <ClientsByTypeChart ref="clientsByTypeChartRef" :data="items" :chartDisplayType="chartDisplayType" />
      </div>

      <!-- Table -->
      <div class="section-right">
        <ClientsByTypeTable
          ref="clientsByTypeTableRef"
          :data="items"
          :reportType="filterData?.reportType ?? REPORT_TYPE_IN_CLIENTS_BY_TYPE.SEX"
        />
      </div>
    </div>
  </template>
...
const { filterData, items, fetchReport } = useClientsByTypeReport()
const { openPrintPreview } = usePrintPreview()

// State
const chartDisplayType = ref<ChartDisplayType>(CHART_DISPLAY_TYPE.PIE)
...
const handlePrint = async (): Promise<void> => {
  const sections = []
  // if (clientsByTypeChartRef.value) sections.push(clientsByTypeChartRef.value.getPrintConfiguration())
  if (clientsByTypeTableRef.value) sections.push(clientsByTypeTableRef.value.getPrintConfiguration())
