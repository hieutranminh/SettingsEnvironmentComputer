<template>
  ...
  >
        ...
        >
          <template #header>
            <p class="text-no-wrap">
              {{ $t(column.headerKey) }}
              <span v-if="'headerSuffix' in column && column.headerSuffix" class="text-blue">
                ({{ column.headerSuffix }})</span
              >
            </p>
          </template>
        </Column>
    ...
    <Column v-for="column in printColumns" :key="column.field" :field="column.field" :bodyClass="column.bodyClass">
...
import { BRANCH_SALES_TABLE_CONFIG, PRINT_TABLE_COLUMNS, PRINT_TABLE_HEADER_GROUPS } from '@/constants/branchSalesTable'
