  ...
  computed: {
    ...
    excelTableRows() {
      ...
      return this.sectionData.data.map(item => [
        ...
        formatMoney(item.sameDayCancel || 0, 0),
        `${(item.sameDayCancelRatio || 0).toFixed(1)}%`,
        formatMoney(item.noShow || 0, 0),
        `${(item.noShowRatio || 0).toFixed(1)}%`,
        formatMoney(item.sameDayCancelAndNoShow || 0, 0),
        `${(item.sameDayCancelAndNoShowRatio || 0).toFixed(1)}%`,
      ])
    },
    ...
    getPrintSections() {
      ...
      if (this.$refs.titleRef) {
        sections.push({
          ...
          section_excel_text: this.$t('report.booking'),
          alignment:          print_options.text_ref_type_alignment.left,
          fontWeight:         'bold',
          fontSize:           10,
          marginBottom:       this.aiSummaryPlainText ? 2 : 10,
