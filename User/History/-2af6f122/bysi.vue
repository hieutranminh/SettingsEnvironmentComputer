...
export default {
  ...
  computed: {
    ...
    bookingContainerStyle() {
      ...
      const visibleStartTime = Math.max(bookingStartTime, this.openingStartTimeInMinutes)
      const timeDifference = visibleStartTime - bookingStartTime
      const visibleEstimatedTime = Math.max(0, this.bookingEstimatedTime - timeDifference)

      const height = visibleEstimatedTime * this.timeSlotUnitHeight
      return {
        height: `${height}px`,
      }
    },
