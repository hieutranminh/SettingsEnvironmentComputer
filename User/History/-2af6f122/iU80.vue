...
export default {
  ...
  computed: {
    ...
    bookingStyle() {
      const top = (convertTimeToMinutes(this.booking?.bookedResources[0]?.startTime) - Number(this.openingStartTimeInMinutes) + (this.booking?.bookedResources[0]?.isNextDay ? convertTimeToMinutes(NEXT_DAY_BONUS_TIME) : 0 /* For 24:00:00 Added Time */ ) ) * this.timeSlotUnitHeight
  ...
  methods: {
    ...
    toggleBookingDescriptionVisible() {
    ...
    handleBookingEnter: debounce(function (event) {
      ...
      if (!this.isMobileDevice) {
        ...
        this.toggleBookingDescriptionVisible()
      }
    }, 10),
    ...
    handleBookingOutsideClick(event) {
      event.stopPropagation()
      if (!this.$refs.booking.contains(event.target)) {
