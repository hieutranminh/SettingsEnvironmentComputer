...
export const generateFileName = (reportName: string, date: Date = new Date(), extension: string = ''): string => {
  const formattedDate = moment(date).format(
    `${PRINT_PREVIEW_CONFIG.STANDARD_DATE_FORMAT} ${PRINT_PREVIEW_CONFIG.STANDARD_HOUR_FORMAT}`,
  )
...
export const validateTableHeaders = (headers: TableHeader[]): boolean => {
  ...
  return headers.every(
    (header) => header.key && header.label && typeof header.key === 'string' && typeof header.label === 'string',
  )
}
...
export const validateTableData = (data: TableRowItem[], headers: TableHeader[]): boolean => {
  ...
  return data.every((row) => typeof row === 'object' && row !== null && requiredKeys.every((key) => key in row))
}

export const applyCellStyles = (baseStyles: CellStyle, customStyles: CellStyle = {}): CellStyle => {
  return {
    ...baseStyles,
    ...customStyles,
  }
