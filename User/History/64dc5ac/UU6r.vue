  ...
  <Card>
    <template #content>
      ...
      >
        <Column field="title" :header="$t('mergeRequests.mergeTitle')" sortable class="col-title">
          <template #body="{ data }">
            ...
            <div class="branch-flow">
              <Tag
                v-tooltip.right="data.sourceBranch"
                :value="truncateBranch(data.sourceBranch)"
                severity="warn"
              />
              <i class="pi pi-arrow-right branch-flow__arrow" />
              <Tag
                v-tooltip.right="data.targetBranch"
                :value="truncateBranch(data.targetBranch)"
                severity="success"
              />
            </div>
          </template>
        ...
        <Column :header="$t('mergeRequests.actions')" class="col-action" :exportable="false">
          <template #body="{ data }">
            <div class="action-buttons">
              <Checkbox
                v-tooltip.top="$t('mergeRequests.squashCommits')"
                :model-value="getSquash(data.id)"
                binary
                @update:model-value="toggleSquash(data.id, $event)"
              />
              <Button
                ...
                size="small"
                severity="success"
                @click="emit('merge', { mergeRequest: data, squash: getSquash(data.id) })"
              />
            </div>
          </template>
        </Column>
...
export interface MergeEmitPayload {
  mergeRequest: MergeRequestItem
  squash: boolean
}
...
const rows = ref(10)
const squashStates = ref<Map<number, boolean>>(new Map())

const getSquash = (mrId: number): boolean => {
  return squashStates.value.get(mrId) ?? false
}

const toggleSquash = (mrId: number, value: boolean): void => {
  squashStates.value = new Map(squashStates.value.set(mrId, value))
}

const emit = defineEmits<{
  (event: 'update:first', value: number): void
  (event: 'merge', payload: MergeEmitPayload): void
}>()
