        ...
        <Column :header="$t('mergeRequests.reviews')" class="col-reviews">
          <template #body="{ data }">
            <div class="mr-reviews">
              <div v-if="data.reviewers.length > 0" class="mr-reviews__avatars">
                <Avatar
                  v-for="reviewer in data.reviewers"
                  :key="reviewer.id"
                  v-tooltip.top="reviewer.name"
                  :image="reviewer.avatarUrl"
                  shape="circle"
                  class="mr-reviews__avatar"
                />
              </div>
              <Tag
                v-if="data.approvedBy.length > 0"
                :value="`${data.approvedBy.length}/${data.reviewers.length} ${$t('mergeRequests.approved')}`"
                severity="success"
              />
              <Tag
                v-else
                :value="`0/${data.reviewers.length} ${$t('mergeRequests.approved')}`"
                severity="warn"
              />
            </div>
          </template>
