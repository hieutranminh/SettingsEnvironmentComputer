...
import { parseHtmlToRichText } from '../utils/htmlParser'
 ...
 */
const hasAlignmentOptions = (options: CellFormatOptions): boolean => {
 ...
 */
export interface UseExcelCellFormatterReturn {
  formatCell: (cell: ExcelJS.Cell, options: CellFormatOptions) => void
  formatHeaderCell: (cell: ExcelJS.Cell, text: string) => void
  formatDataCell: (cell: ExcelJS.Cell, text: string, columnIndex: number, columnAlignments?: ColumnAlignments) => void
  formatFooterCell: (cell: ExcelJS.Cell, text: string, columnIndex: number, columnAlignments?: ColumnAlignments) => void
 ...
 */
export const useExcelCellFormatter = (): UseExcelCellFormatterReturn => {
  /**
   * Applies font formatting to a cell
   * @param cell - Excel cell to format
   * @param options - Font formatting options
   */
  const applyFontFormatting = (cell: ExcelJS.Cell, options: CellFormatOptions): void => {
   ...
   */
  const applyAlignmentFormatting = (cell: ExcelJS.Cell, options: CellFormatOptions): void => {
   ...
   */
  const applyFillFormatting = (cell: ExcelJS.Cell, options: CellFormatOptions): void => {
   ...
   */
  const applyBorderFormatting = (cell: ExcelJS.Cell, options: CellFormatOptions): void => {
    if (options.border) {
      cell.border = {
   ...
   */
  const applyNumberFormatting = (cell: ExcelJS.Cell, options: CellFormatOptions): void => {
   ...
   */
  const formatCell = (cell: ExcelJS.Cell, options: CellFormatOptions): void => {
  ...
  const createFormatOptions = (
    ...
    const formatOptions: CellFormatOptions = {
      alignment,
      verticalAlignment: EXCEL_ALIGNMENT.VERTICAL.MIDDLE,
      border: true,
      wrapText: true,
   ...
   *
   * Example:
   *   formatCellWithValue(cell, '1,234.56', 3, { 3: 'right' }, false)
   */
  const formatCellWithValue = (
    cell: ExcelJS.Cell,
    text: string,
    columnIndex: number,
    columnAlignments?: ColumnAlignments,
  ...
  const formatDataCell = (
    ...
    formatCellWithValue(cell, text, columnIndex, columnAlignments, false)
  }
  ...
  const formatFooterCell = (
    ...
    formatCellWithValue(cell, text, columnIndex, columnAlignments, true)
  }
