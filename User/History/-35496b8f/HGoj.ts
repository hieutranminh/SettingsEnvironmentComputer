...
import { SPACING, CANVAS_RENDER_OPTIONS } from '../constants'
import { PDF_CANVAS_CONFIG } from '../constants/pdf'

import { usePdfHeader } from './usePdfHeader'

/**
 * Conversion ratio from pixels to points (pt = 3/4 * px)
 */
const PIXEL_TO_POINT_RATIO = 0.75

/**
 * Supported image formats for PDF
 */
...
export const usePdfCanvas = (): UsePdfCanvasReturn => {
  ...
  const convertToCanvas = async (element: HTMLElement): Promise<HTMLCanvasElement> => {
    return html2canvas(element, {
      scale: CANVAS_RENDER_OPTIONS.scale,
      useCORS: CANVAS_RENDER_OPTIONS.useCORS,
      allowTaint: CANVAS_RENDER_OPTIONS.allowTaint,
      logging: CANVAS_RENDER_OPTIONS.logging,
      backgroundColor: CANVAS_RENDER_OPTIONS.backgroundColor,
    })
  }
  ...
  const calculateDimensions = (canvas: HTMLCanvasElement, maxWidth: number, maxHeight: number): CanvasDimensions => {
    ...
    const originalWidth = PIXEL_TO_POINT_RATIO * canvas.width
    const originalHeight = PIXEL_TO_POINT_RATIO * canvas.height
  ...
  const needsNewPage = (currentY: number, canvasHeight: number, pageHeight: number): boolean => {
    ...
    return remainingHeight < PDF_CANVAS_CONFIG.MIN_HEIGHT_FOR_CURRENT_PAGE || canvasHeight > remainingHeight
  }
  ...
  const processCanvas = async (
    ...
    try {
      ...
      if (needsNewPage(currentY, dimensions.scaledHeight, docHeight)) {
        pdf.addPage()
        currentY = addHeader(pdf, config, SPACING.MARGIN.TOP)
      }
      ...
      // Add image to PDF
      pdf.addImage(canvas, PDF_IMAGE_FORMAT, xPosition, currentY, dimensions.scaledWidth, dimensions.scaledHeight)
