          ...
          <div class="consent-action__form-supplements">
            <template v-for="(consentArea, indexArea) in consentForm.consentSupplements">
              ...
              >
                ...
                <b-form-textarea
                  :ref="`contentTextarea-${indexArea}`"
                  v-model="consentArea.supplementText"
                  :rows="CONSENT_INPUT.MAX_ROW_TEXTAREA"
                  :maxlength="CONSENT_INPUT.MAX_LENGTH_CONTENT"
                  @compositionstart="handleContentCompositionStart(indexArea)"
                  @compositionend="handleContentCompositionEnd(indexArea)"
                  @input="handleContentInput(indexArea)"
                />
...
export default {
  ...
  methods: {
    ...
    /**
     * Handle Korean IME composition start
     * @param {number} indexArea - The index of the content area
     */
    handleContentCompositionStart (indexArea) {
     ...
     * @param {number} indexArea - The index of the content area
     */
    handleContentCompositionEnd (indexArea) {
      this.isComposing = false
      this.checkContentLength(indexArea)
    },
    ...
    /**
     * Check content length and show modal if exceeds 50 graphemes
     * @param {number} indexArea - The index of the content area
     */
    checkContentLength (indexArea) {
      const value = this.consentForm.consentSupplements[indexArea].supplementText
      const graphemeLength = this.getGraphemeLength(value)

      if (graphemeLength > 50) {
        this.currentContentAreaIndex = indexArea

        // Blur the textarea to prevent further input and close IME
        const textareaRef = this.$refs[`contentTextarea-${indexArea}`]
        if (textareaRef) {
          const textareaElement = textareaRef.$el || textareaRef
