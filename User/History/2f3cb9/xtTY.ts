/**
 * PDF text rendering utilities - FIXED VERSION
...
/**
 * Calculates total width and prepares parts for centered/right alignment
 * @param pdf - PDF document
 * @param parts - Array of text parts
 * @returns Total width and parts with calculated widths
 */
const calculatePartsInfo = (
  pdf: jsPDF,
  parts: TextPart[],
): { totalWidth: number; partsWithWidth: Array<TextPart & { width: number }> } => {
  let totalWidth = 0
  ...
  return { totalWidth, partsWithWidth }
}
...
const renderTextParts = (pdf: jsPDF, parts: TextPart[], x: number, y: number, options?: TextRenderOptions): number => {
  // If no parts, return original x
  if (parts.length === 0) return x

  // For simple cases with single part, use normal text rendering
  if (parts.length === 1) {
    setFontForPart(pdf, parts[0].isBold)
    pdf.text(parts[0].text, x, y, options)
    return x + pdf.getTextWidth(parts[0].text)
  }

  // For mixed formatting, we need to handle alignment manually
  let currentX = x

  // Calculate starting position based on alignment
  if (options?.align && options.align !== 'left') {
    const { totalWidth } = calculatePartsInfo(pdf, parts)

    if (options.align === 'center') {
      // For center alignment, adjust starting x position
      const pageWidth = pdf.internal.pageSize.getWidth()
      currentX = (pageWidth - totalWidth) / 2
    } else if (options.align === 'right') {
      // For right alignment, adjust starting x position
      const pageWidth = pdf.internal.pageSize.getWidth()
      const rightMargin = x // Use provided x as right margin
      currentX = pageWidth - rightMargin - totalWidth
    }
  }

  // Render each part at the calculated position
  parts.forEach((part) => {
    setFontForPart(pdf, part.isBold)
    // Important: Don't pass alignment options to individual text calls
    // We've already handled alignment by adjusting currentX
