...
/**
 * Text parts rendering parameters
 */
export interface ITextPartsRenderParams {
  pdf: jsPDF
  parts: ITextPart[]
  xPosition: number
  yPosition: number
 ...
 * @param params - Text parts rendering parameters
 * @returns Final X position after rendering all parts
 */
const renderTextParts = (params: ITextPartsRenderParams): number => {
  const { pdf, parts, xPosition, yPosition, options } = params
