  ...
  methods: {
    ...
    async onGenerate() {
      ...
      try {
        ...
        // Handle case where http interceptor retry logic returns undefined response
        // This happens due to a bug in http.js loopWaitForNextRetry
        ...
        if (!textMessage) {
          throw new Error('ERR_EMPTY_TEXT_MESSAGE')
        }
        ...
        // Handle retry-related errors (http interceptor returns undefined after retry)
        if (error?.message === 'ERR_INVALID_RESPONSE' || error?.message === 'ERR_EMPTY_TEXT_MESSAGE') {
          this._showDialogAlert(this.$t('aha-ai.message-ai-could-not-generate'))
          return
        }
