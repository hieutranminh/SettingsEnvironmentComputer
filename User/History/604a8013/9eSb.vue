...
export default {
  ...
  methods: {
    ...
    scrollToResultPanel() {
      ...
      const modal = this.$refs.modalSendMessageAhaAi
      const modalBody = modal?.$el?.querySelector('.modal-body')
      if (!modalBody) return
      ...
      const blockPosition = resultPanelHeight <= modalBodyHeight ? 'end' : 'start'
