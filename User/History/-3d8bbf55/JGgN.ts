...
export function usePrintConfig() {
  ...
  // Ultra-simple inline config creator
  const quickConfig = (title: string, fromDate?: string, toDate?: string, landscape = false): PrintPreviewConfig => {
    const dateRange = fromDate && toDate ? { from: fromDate, to: toDate } : undefined
    return createConfig(title, dateRange, { orientation: landscape ? 'landscape' : 'portrait' })
  }
