...
export const useServiceSalesReport = (): IUseServiceSalesReportReturn => {
  // State
  const filterData = ref<IServiceSalesFilterInterface | null>(null)
  ...
  const reportByType = computed(
    () => reportData.value?.result?.reportByType ?? REPORT_BY_TYPE.STAFF,
  )
