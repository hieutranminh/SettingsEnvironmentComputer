...
export default {
  ...
  methods: {
    ...
    initHeicWorker() {
      ...
      try {
        this.heicWorker = new Worker(new URL('Workers/heic-convert.worker.js', import.meta.url))
    ...
    handleWorkerMessage(event) {
      const { id, success, jpegBuffer, fileName, error } = event.data
      ...
      if (success) {
        const blob = new Blob([jpegBuffer], { type: IMG_TYPE })
