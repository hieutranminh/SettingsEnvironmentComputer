...
const HEIC_CONVERSION_TIMEOUT_MS = 30000
const DELAY_BETWEEN_HEIC_CONVERSIONS_MS = 500
  ...
  methods: {
    ...
    async getFileBlobURL(file) {
      ...
      if (URL && URL.createObjectURL) {
        if(/\.heic$|\.heif$/i.test(file.name)) {
          try {
            this.preLoader()
            const blobUrl = await this.convertHeicWithTimeout(file)
    ...
    async processQueue() {
      ...
      while (this.fileQueue.length > 0) {
        ...
        // Delay between HEIC conversions to allow garbage collection
        if (isHeicFile && this.fileQueue.length > 0) {
          await this.delay(DELAY_BETWEEN_HEIC_CONVERSIONS_MS)
        }
      }
