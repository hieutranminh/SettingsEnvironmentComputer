...
const DELAY_BETWEEN_HEIC_CONVERSIONS_MS = 300
  ...
  methods: {
    ...
    async getFileBlobURL(file) {
      ...
      if (URL && URL.createObjectURL) {
        if(/\.heic$|\.heif$/i.test(file.name)) {
          try {
            this.preLoader()
            const blobUrl = await this.convertHeicToJpeg(file)
    ...
    async processQueue() {
      ...
      while (this.fileQueue.length > 0) {
        ...
        // Delay between HEIC conversions to allow garbage collection and prevent memory overload
