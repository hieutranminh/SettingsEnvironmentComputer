...
export const usePrintPreview = () => {
  ...
  const canStartPrint = computed(() => 
    isWorkerSupported.value && 
    !printPreviewStore.isWorkerProcessing.value
  )
  ...
  const createWorker = (): Worker | null => {
    ...
    try {
      const worker = new Worker(
        new URL('../workers/print-preview.worker.ts', import.meta.url),
        { type: 'module' }
      )
  ...
  const handleRefreshToken = async (): Promise<void> => {
    try {
      ...
      if (printPreviewStore.printDataOption.value) {
        const { workerType, tableHeaders, reportHeaders, additionalOptions, requestPayload } = printPreviewStore.printDataOption.value
