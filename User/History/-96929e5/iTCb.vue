<!-- components/dialogs/MessageDialog.vue -->
<template>
  <div class="dialog-content">
    <Message :severity="type">
      <template v-if="isStructuredError">
        <p v-for="(error, index) in structuredErrors" :key="index">{{ error.errorMessage }}</p>
      </template>
      <template v-else-if="Array.isArray(message)">
        <p v-for="(item, index) in message" :key="index" v-html="item" />
      </template>
      <p v-else v-html="message" />
    </Message>

    <i18n-t keypath="general.total-of-records">
      <template #sumItems>
        <b style="color: red">10</b>
      </template>
      <template #items>
        <b style="color: red">10</b>
      </template>
    </i18n-t>

    <!-- Error Details Section -->
    <div v-if="isStructuredError">
      <div class="error-detail-toggle">
        <span @click="handleToggleErrorDetails">
          {{
            showErrorDetails ? $t('general.hide-error-details') : $t('general.show-error-details')
          }}
        </span>
      </div>

      <Message v-if="showErrorDetails" severity="secondary">
        <div v-for="(error, index) in structuredErrors" :key="index" class="error-detail-item">
          <strong>{{ error.errorCode }}</strong>

          <div v-if="error.errorValues && error.errorValues.length" class="error-values">
            <strong>{{ $t('general.error-values') }}</strong>
            <ul>
              <li v-for="(value, valueIndex) in error.errorValues" :key="valueIndex">
                {{ value }}
              </li>
            </ul>
          </div>
        </div>
      </Message>
    </div>
  </div>

  <div class="dialog-actions">
    <Button label="Close" severity="info" @click="handleClose" />
  </div>
</template>

<script setup lang="ts">
import { inject, onMounted, ref, computed } from 'vue'

interface ErrorDetail {
  errorCode: string
  errorMessage: string
  errorValues: string[]
}

interface DialogRef {
  value: {
    data: {
      type: string
      message: string | string[] | ErrorDetail[] | (string | ErrorDetail)[]
    }
    close: () => void
  }
}

const dialogRef = inject<DialogRef>('dialogRef')
const type = ref<string>('')
const message = ref<string | string[] | ErrorDetail[] | (string | ErrorDetail)[]>('')
const showErrorDetails = ref<boolean>(false)

const isStructuredError = computed((): boolean => {
  return (
    Array.isArray(message.value) &&
    message.value.length > 0 &&
    message.value.some(
      (item) =>
        typeof item === 'object' && item !== null && 'errorCode' in item && 'errorMessage' in item,
    )
  )
})

const structuredErrors = computed((): ErrorDetail[] => {
  if (isStructuredError.value && Array.isArray(message.value)) {
    return message.value.filter(
      (item) =>
        typeof item === 'object' && item !== null && 'errorCode' in item && 'errorMessage' in item,
    ) as ErrorDetail[]
  }
  return []
})

const handleClose = (): void => {
  if (dialogRef?.value?.close) {
    dialogRef.value.close()
  }
}

const handleToggleErrorDetails = (): void => {
  showErrorDetails.value = !showErrorDetails.value
}

onMounted(() => {
  if (dialogRef?.value?.data) {
    type.value = dialogRef.value.data.type
    message.value = dialogRef.value.data.message
  }
})
</script>

<style lang="scss" scoped>
.dialog-content {
  padding: 1rem 0;
}

.dialog-actions {
  display: flex;
  justify-content: center;
  gap: 0.5rem;
  padding-top: 1rem;
}

.error-detail-toggle {
  text-align: right;
  margin: 10px 0;
  font-size: 12px;
  color: var(--primary-color);

  span {
    cursor: pointer;
  }
}

.error-detail-item {
  font-size: 12px;

  &:not(:last-child) {
    margin-bottom: 10px;
  }

  .error-values {
    ul {
      padding-left: 15px;

      li {
        list-style: disc;
      }
    }
  }
}
</style>
