    ...
    <div v-if="isStructuredError">
      ...
      >
        ...
        >
          ...
          <div
            v-if="error.errorValues && error.errorValues.length"
            class="error-values"
          >
            <strong>Error Values:</strong>
            <ul>
              <li
                v-for="(value, valueIndex) in error.errorValues"
                :key="valueIndex"
              >
                {{ value }}
              </li>
...
const isStructuredError = computed((): boolean => {
  return (
    ...
    message.value.some(
      (item) => typeof item === 'object' && item !== null && 'errorCode' in item && 'errorMessage' in item,
    )
  )
})
