...
const handleResponseServiceUnauthorization = async (
  ...
  const hasErrorUnAuthorized = response?.data?.errorMessages?.some((errorMessage: ApiError) => {
...
export const handleResponse = async (response: AxiosResponse<ApiResponse<unknown>>): Promise<AxiosResponse<ApiResponse<unknown>>> => {
  // Handle service unauthorized errors from response data
  return await handleResponseServiceUnauthorization(response)
}

export const handleResponseError = (error: AxiosError<ApiResponse<unknown>>): Promise<never> => {
  // Handle HTTP 401 unauthorized errors
  if (error?.response?.status === HTTP_STATUS.UNAUTHORIZED) {
    return handleResponseUnauthorization(error as AxiosError<ApiError>) as Promise<never>
  }
  ...
  if (error?.response?.status && error.response.status >= 400) {
    ...
    // Access errorMessages from the API response data
    const errorMessages = (error.response.data as ApiResponse<unknown>)?.errorMessages
    if (errorMessages && errorMessages.length > 0) {
      const firstError = errorMessages[0]
      console.error('Client error:', firstError)
      return Promise.reject(new Error(`${firstError.errorMessage} (${firstError.errorCode})`))
    }
