...
const handleTokenRefreshForResponse = async (
  config: RefreshTokenConfig,
): Promise<AxiosResponse> => {
  const success = await handleTokenRefresh()

  if (!success) {
    throw new Error('Failed to refresh tokenxx')
  }
...
): Promise<AxiosResponse<IApiResponse<unknown>>> => {
  if (response?.data?.isOK) return response

  const hasErrorUnAuthorized = response?.data?.errorMessages?.some((errorMessage: IApiError) => {
...
const handleResponseUnauthorization = async (
...
export const handleResponseError = (error: AxiosError<IApiResponse<unknown>>): Promise<never> => {
  // Handle HTTP 401 unauthorized errors
  if (error?.response?.status === HTTP_STATUS.UNAUTHORIZED) {
    return handleResponseUnauthorization(
      error as unknown as AxiosError<IApiError>,
    ) as Promise<never>
  }

  // Handle server errors (5xx)
  if (error?.response?.status && error.response.status >= SERVER_ERROR_STATUS) {
    return Promise.reject(new Error('Server error. Please try again later.'))
  }

  // Handle client errors (4xx)
  if (error?.response?.status && error.response.status >= CLIENT_ERROR_STATUS) {
    const errorMessages = (error.response.data as IApiResponse<unknown>)?.errorMessages
    if (errorMessages && errorMessages.length > 0) {
      return Promise.reject(errorMessages)
    }
    return Promise.reject(new Error('Client error occurred'))
  }
