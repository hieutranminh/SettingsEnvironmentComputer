 ...
 */
export const createFontUtils = (pdf: jsPDF): any => {
  return {
 ...
 */
export const waitForFonts = async (timeoutMs: number = 5000): Promise<boolean> => {
  const startTime = Date.now()
  
  while (!fontsLoaded && (Date.now() - startTime) < timeoutMs) {
