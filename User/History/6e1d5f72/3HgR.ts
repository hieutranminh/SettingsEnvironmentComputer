...
import {
  getBranchSalesTotalReport,
  type BranchSalesTotalReportRequest,
  type BranchSalesTotalReportResponse,
} from '@/services/sales/sales-reports/branch-sales-total-report.read'
...
export const useBranchSalesTotalReport = () => {
   ...
   */
  const fetchReport = async (filters: BranchSalesFilterInterface, append = false): Promise<BranchSalesTotalReportResponse | null> => {
    try {
      startLoading(true)
      error.value = null
      ...
      if (append && reportData.value) {
        // Append new items to existing data
        reportData.value = {
          ...response,
          items: [...reportData.value.items, ...response.items],
        }
      } else {
