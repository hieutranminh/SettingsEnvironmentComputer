<template>
  <div class="calendar-aha-ai">
    ...
    <modal-aha-ai-result-available-times
      ...
      @cancel="modalStates.resultAvailableTimes = false"
...
export default {
  ...
  methods: {
    ...
    async callGetAvailableTimesAndOpenResult(resultData, selectedItems) {
      try {
        this.preLoader()

        // Calculate estimatedTime from exact match and selected relative match items
        const calculatedEstimatedTime = this.calculateEstimatedTimeFromItems(resultData.servicesAndBookingItems, selectedItems)
        const payload = {
        ...
        this.savedFilters = this.mapResultDataToFilters(resultData, selectedItems)
    ...
    calculateEstimatedTimeFromItems(servicesAndBookingItems, selectedItems) {
      let totalEstimatedTime = 0
      let hasItems = false

      // Calculate from exact match items
      if (servicesAndBookingItems?.servicesAndBookingItemsExactMatch) {
        const { serviceItems, bookingItems } = servicesAndBookingItems.servicesAndBookingItemsExactMatch
        const exactServiceItems = serviceItems || []
        const exactBookingItems = bookingItems || []

        if (exactServiceItems.length > 0 || exactBookingItems.length > 0) {
          hasItems = true
          totalEstimatedTime += exactServiceItems.reduce((sum, item) => sum + (item.estimatedTime || 0), 0)
          totalEstimatedTime += exactBookingItems.reduce((sum, item) => sum + (item.estimatedTime || 0), 0)
        }
      }

      // Calculate from selected relative match items
      if (selectedItems) {
        const { selectedServiceItems, selectedBookingItems } = selectedItems
        const relativeServiceItems = selectedServiceItems || []
        const relativeBookingItems = selectedBookingItems || []

        if (relativeServiceItems.length > 0 || relativeBookingItems.length > 0) {
          hasItems = true
          totalEstimatedTime += relativeServiceItems.reduce((sum, item) => sum + (item.estimatedTime || 0), 0)
          totalEstimatedTime += relativeBookingItems.reduce((sum, item) => sum + (item.estimatedTime || 0), 0)
        }
      }

      // Return null if no items to use original estimatedTime
      return hasItems ? totalEstimatedTime : null
    },
    ...
    mapResultDataToFilters(resultData, selectedItems) {
      ...
      // Calculate estimatedTime from exact match and selected relative match items
      const calculatedEstimatedTime = this.calculateEstimatedTimeFromItems(servicesAndBookingItems, selectedItems)
    ...
    onConfirmSearchAvailableTimesFromAhaAi(resultData, selectedItems) {
      // Call API with queryParameters and open result modal
      this.callGetAvailableTimesAndOpenResult(resultData, selectedItems)
    },
  },
}
</script>
