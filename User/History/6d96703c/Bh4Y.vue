...
// APIs
import { getAhaAIBookingAgent } from 'Modules/api/aha-ai/aha-ai-api'
// Constants
...
export default {
  ...
  methods: {
    ...
    handleSearchAvailableTimesFromAhaAi(resultData) {
      ...
      // Pass data to modal-aha-ai for displaying services/booking items
      this.$refs.modalAhaAi.errors = []
    ...
    mapResultDataToFilters(resultData) {
      const { dateTimeFilters, estimatedTime, isPeriodSelection, bookingResourceSetupIds: resultResourceIds } = resultData

      // Map booking resource setup IDs
      const bookingResourceSetupIds = this.getFilteredResources(resultResourceIds)
    ...
    onConfirmSearchAvailableTimesFromAhaAi(resultData) {
      console.log('resultData', resultData)
      // this.resultData = resultData.result
