  ...
  methods: {
    ...
    async callGetAvailableTimesAndOpenResult(resultData) {
      try {
        ...
        // Calculate estimatedTime from servicesAndBookingItemsExactMatch
        const calculatedEstimatedTime = this.calculateEstimatedTimeFromExactMatch(resultData.servicesAndBookingItems)
        const payload = {
        ...
        this.savedFilters = this.mapResultDataToFilters(resultData)
    ...
    calculateEstimatedTimeFromExactMatch(servicesAndBookingItems) {
      if (!servicesAndBookingItems) {
        return null
      }

      const { servicesAndBookingItemsExactMatch } = servicesAndBookingItems
      if (!servicesAndBookingItemsExactMatch) {
        return null
      }

      const serviceItems = servicesAndBookingItemsExactMatch.serviceItems || []
      const bookingItems = servicesAndBookingItemsExactMatch.bookingItems || []

      // If no exact match items, return null to use original estimatedTime
      if (serviceItems.length === 0 && bookingItems.length === 0) {
        return null
      }

      // Sum estimatedTime from all exact match items
      const serviceEstimatedTime = serviceItems.reduce((sum, item) => sum + (item.estimatedTime || 0), 0)
      const bookingEstimatedTime = bookingItems.reduce((sum, item) => sum + (item.estimatedTime || 0), 0)

      return serviceEstimatedTime + bookingEstimatedTime
    },
    ...
    mapResultDataToFilters(resultData) {
      const { dateTimeFilters, estimatedTime, isPeriodSelection, bookingResourceSetupIds: resultResourceIds, resources, servicesAndBookingItems } = resultData

      // Map booking resource setup IDs - use resources field if bookingResourceSetupIds is null
      const resourceIds = resultResourceIds || (resources ? resources.map(r => r.id) : [])
      ...
      // Calculate estimatedTime from exact match items
      const calculatedEstimatedTime = this.calculateEstimatedTimeFromExactMatch(servicesAndBookingItems)

      return {
        shopId:            this.shop_data.shop_id,
        isPeriodSelection: isPeriodSelection || false,
        estimatedTime:     calculatedEstimatedTime ?? estimatedTime ?? 0,
        bookingStartTime:  {
          timeOfDay: timeOfDay,
          startTime: startTime,
          endTime:   endTime,
