<template>
  <div class="calendar-aha-ai">
    ...
    <modal-aha-ai
      ...
      :is-originated-from-client-information="isOriginatedFromClientInformation"
      @send="onSend"
...
import { CalendarEventBus, formatTimeBasedOnMinutes, roundUpTo30Minutes, convertTimeToMinutes, roundUpEstimatedTime, convertMinutesToFormattedTime } from 'Modules/calendar/utils/index'
// Models
...
const DEFAULT_ESTIMATED_TIME_FALLBACK = 30
const ERROR_CODES = {
...
// CalendarEventBus event names
const CALENDAR_EVENTS = {
  SHOW_SEARCH_AVAILABLE_TIMES_FROM_CLIENT_INFO: 'show-search-available-times-popup-from-client-information',
  SHOW_SEARCH_AVAILABLE_TIMES_FROM_BOOKING:     'show-search-available-times-popup-from-booking-items',
  SHOW_AHA_AI_BOOKING_ITEMS:                    'show-aha-ai-booking-items-popup',
  SHOW_SETUP:                                   'show-setup-popup',
  SHOW_HELP:                                    'show-help-popup',
  CLIENT_LOOKUP_FROM_AHA_AI:                    'client-lookup-from-aha-ai',
  BOOKING_LOOKUP_FROM_AHA_AI:                   'booking-lookup-from-aha-ai',
 ...
 * @property {number} estimatedTime - Estimated booking time in minutes (rounded up)
 * @property {number|null} originalTotalEstimatedTime - Raw total estimated time from items before rounding (null if no items)
...
export default {
  ...
  data() {
    return {
      ...
      isOriginatedFromClientInformation: false,
  ...
  computed: {
    ...
    ...mapState('_calendar/bookingAction', {
      isBookingActionVisible: state => state.visible,
    }),
  ...
  created() {
    CalendarEventBus.$on(CALENDAR_EVENTS.SHOW_SEARCH_AVAILABLE_TIMES_FROM_CLIENT_INFO, this.onShowSearchAvailableTimesPopupFromClientInformation)
    CalendarEventBus.$on(CALENDAR_EVENTS.SHOW_SEARCH_AVAILABLE_TIMES_FROM_BOOKING, this.onShowSearchAvailableTimesPopupFromBookingItems)
    CalendarEventBus.$on(CALENDAR_EVENTS.SHOW_AHA_AI_BOOKING_ITEMS, this.onShowAhaAiBookingItemsPopup)
    CalendarEventBus.$on(CALENDAR_EVENTS.SHOW_SETUP, this.onShowSetupPopup)
    CalendarEventBus.$on(CALENDAR_EVENTS.SHOW_HELP, this.onShowHelpPopup)
  },

  beforeDestroy() {
    CalendarEventBus.$off(CALENDAR_EVENTS.SHOW_SEARCH_AVAILABLE_TIMES_FROM_CLIENT_INFO, this.onShowSearchAvailableTimesPopupFromClientInformation)
    CalendarEventBus.$off(CALENDAR_EVENTS.SHOW_SEARCH_AVAILABLE_TIMES_FROM_BOOKING, this.onShowSearchAvailableTimesPopupFromBookingItems)
    CalendarEventBus.$off(CALENDAR_EVENTS.SHOW_AHA_AI_BOOKING_ITEMS, this.onShowAhaAiBookingItemsPopup)
    CalendarEventBus.$off(CALENDAR_EVENTS.SHOW_SETUP, this.onShowSetupPopup)
    CalendarEventBus.$off(CALENDAR_EVENTS.SHOW_HELP, this.onShowHelpPopup)
  },
  ...
  methods: {
    ...
    resetNavigationSource() {
      ...
      this.isOriginatedFromClientInformation = false
    },
    ...
    async callAhaAIBookingAgent(prompt) {
      ...
      const bookingSearchType = this.isOriginatedFromClientInformation
        ? BOOKING_SEARCH_TYPE.SEARCH_AVAILABLE_TIMES_AND_ADD_BOOKINGS
        : this.isFromSearchAvailableTimes
          ? BOOKING_SEARCH_TYPE.SEARCH_AVAILABLE_TIMES_ONLY
    ...
    handleClientLookupFromAhaAi(resultData) {
      CalendarEventBus.$emit(CALENDAR_EVENTS.CLIENT_LOOKUP_FROM_AHA_AI, resultData)
    },

    handleBookingLookupFromAhaAi(resultData) {
      CalendarEventBus.$emit(CALENDAR_EVENTS.BOOKING_LOOKUP_FROM_AHA_AI, resultData)
    },
    ...
    handleSearchAvailableTimesFromAhaAi(resultData) {
      ...
      // Validate resources
      const resourceValidationError = this.validateResources(resultData.resources)
      ...
      const servicesAndBookingItemsValidationError = this.validateServicesAndBookingItems(resultData.servicesAndBookingItems, estimatedTime)
    ...
    handleAddBookingFromAhaAi(resultData) {
      ...
      const hasNoItems = !this.hasAnyMatchedItems(resultData.servicesAndBookingItems)
      if (resultData.estimatedTime === 0 && hasNoItems) {
        this.setModalAhaAiErrors([this.$t('aha-ai.validate-specific-estimated-time-required')])
        return
      }
    ...
    async openAddBookingFromAhaAi(resultData) {
      try {
        ...
        if (!this.isBookingActionVisible) {
          CalendarEventBus.$emit(CALENDAR_EVENTS.CLOSE_CLIENT_INFO_ON_SELECT)

          await this.openBookingActionBySlot(selectedSlotData)
        } else {
          this.updateExistingBookedResourceFromSlotData(selectedSlotData)
        }

        if (bookedItems.length > 0) {
          const bookedResource = this.bookedResources?.[0]
          if (bookedResource) {
            if ((bookedItems.length + (bookedResource.bookedItems?.length || 0)) > MAX_ITEMS_RESOURCE) {
              throw new Error(this.$t('bookings.warning_can_not_exceed_booked_items_max', { items_max: MAX_ITEMS_RESOURCE }))
            }
    ...
    updateExistingBookedResourceFromSlotData(slotData) {
      const bookedResource = this.bookedResources?.[0]
      if (!bookedResource) return

      const clonedResource = bookedResource.clone()

      if (slotData.resource) {
        clonedResource.bookingResourceSetupId = slotData.resource.id
        clonedResource.resourceId = slotData.resource.resource_id
        clonedResource.resourceName = slotData.resource.resource_name
      }

      if (slotData.startTime != null) {
        clonedResource.startTime = convertMinutesToFormattedTime(slotData.startTime)
        clonedResource.isNextDay = !!(Math.floor(slotData.startTime / MINUTES_OF_24H))
      }

      if (slotData.bookingDateTS) {
        this.setBookingDateTS(slotData.bookingDateTS)
      }

      this.updateBookedResource(clonedResource)
    },
    ...
    buildSlotDataForAddBooking(resultData, selectedServiceItems, selectedBookingItems) {
      ...
      return {
        ...
        bookingDateTS: resultData.bookingDateTS,
        ...
        estimatedTime: calculatedEstimatedTime || resultData.estimatedTime || DEFAULT_ESTIMATED_TIME_FALLBACK,
      }
    },
    ...
    async callGetAvailableTimesAndOpenResult(resultData) {
      try {
        ...
        const payload = {
          ...
          estimatedTime: calculatedEstimatedTime || resultData.queryParameters.estimatedTime || DEFAULT_ESTIMATED_TIME_FALLBACK,
        }
    ...
    calculateEstimatedTimeFromItems(servicesAndBookingItems, selectedServiceItems, selectedBookingItems) {
      ...
      const totalEstimatedTime = exactServiceEstimatedTime + exactBookingEstimatedTime + selectedServiceEstimatedTime + selectedBookingEstimatedTime
    ...
    validateServicesAndBookingItems(servicesAndBookingItems, estimatedTime) {
      if (!servicesAndBookingItems) {
        return null
      }
    ...
    mapResultDataToFilters(resultData, selectedServiceItems, selectedBookingItems) {
      ...
      return {
        ...
        estimatedTime:              roundUpEstimatedTime(calculatedEstimatedTime || queryParameters.estimatedTime || DEFAULT_ESTIMATED_TIME_FALLBACK),
        originalTotalEstimatedTime: calculatedEstimatedTime ?? null,
        bookingStartTime:           {
          timeOfDay: timeOfDay,
          startTime: startTime,
          endTime:   endTime,
    ...
    onShowSearchAvailableTimesPopupFromClientInformation() {
      ...
      this.resetNavigationSource()
      this.isOriginatedFromClientInformation = true
      this.modalStates.searchAvailableTimes = true
    },
