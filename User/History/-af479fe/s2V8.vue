 ...
 */
const sumForCurrentType = computed(() =>
  props.data.reduce((acc, item) => acc + getAmountByType(item, displayItemType.value), 0),
)
...
const averageAmount: ComputedRef<number> = computed(() => {
  ...
  const totalAmount = getAmountByType(sumForCurrentType.value as SalesByMonthReportItem, displayItemType.value)
