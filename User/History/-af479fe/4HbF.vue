    ...
    <template #content>
      <template v-if="data.length">
        <div class="select-display-item">
          <label for="display-item-type">{{ $t('sales-by-month.label-display-item') }}</label>
          <Select
...
// Types
import type { ISalesByMonthReportItem } from '@/types/sales-report/SalesByMonth'
import type { IPrintSection } from '@/types/print'
// Utils
import { formatAmount } from '@/utils/common'

interface IProps {
  data?: ISalesByMonthReportItem[]
...
const displayItemTypeOptions = computed(() => {
  ...
  if (props.prepaidSalesCountingType === PREPAID_SALES_COUNTING_TYPE.SOLD) {
    baseOptions.push(
      ...
      {
        label: t('sales-by-month.label-prepaid-service'),
        value: DISPLAY_ITEM_TYPE.PREPAID_SERVICE,
      },
    )
  }
  return baseOptions
...
const averageAmount: ComputedRef<number> = computed(() => {
  ...
  const totalAmount = getAmountByType(
    totals.value as ISalesByMonthReportItem,
...
const averageData: ComputedRef<number[]> = computed(() =>
  new Array(props.data.length).fill(averageAmount.value),
)
...
const chartData = computed(() => {
  ...
  const orangeColor = documentStyle.getPropertyValue('--p-orange-400')
  ...
  return {
    ...
    datasets: [
      ...
      {
        label: `${t('general.average')} (${formatAmount(averageAmount.value, { decimalCount: 0 }) || 0})`,
 ...
 */
const getAmountByType = (item: ISalesByMonthReportItem, type: number): number => {
  const typeMap: Record<number, keyof ISalesByMonthReportItem> = {
...
watch(
  () => props.prepaidSalesCountingType,
  () => {
    displayItemType.value = DISPLAY_ITEM_TYPE.ALL
  },
)
</script>
