<template>
  <div class="section-revenue-by-service">
    ...
    <!-- Chart and Table -->
    <div :class="revenueByServiceContentClass">
      <!-- Chart -->
...
export default {
  ...
  computed: {
    ...
    excelTableHeaders() {
      const headers = [this.isViewByCategory ? this.$t('report.category') : this.$t('report.service')]
    ...
    tableCustomStyles() {
      // First column (Category/Service) is left, month columns are right-aligned
      const styles = [{ halign: 'left' }]
    ...
    revenueByServiceContentClass() {
      return ['revenue-by-service-content', {
        'revenue-by-service-content--mobile': this.isMobileDevice,
      }]
    },

    // Show AI Summary only if aiSummary has content
    showAiSummary() {
