...
export function usePrintPreview() {
  ...
  const openPreview = async (options: {
    ...
    const fullConfig: PrintPreviewConfig = {
      header: {
        ...createDefaultHeaderConfig(options.config.header.title),
      ...
      exportType: options.config.exportType || 'both',
      customData: options.config.customData,
    }

    const printOptions: PrintPreviewOptions = {
      type: options.type,
      config: fullConfig,
      data: options.data,
    }
    ...
    store.openModal()
    await store.generatePreview(printOptions)
  }
