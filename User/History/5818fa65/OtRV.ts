...
export function usePrintPreview() {
  ...
  const isVisible = computed(() => {
    const value = printPreviewStore.isVisible
    console.log('👁️ isVisible changed:', value)
    return value
  })
  const sections = computed(() => printPreviewStore.sections)
  const pdfBlobUrl = computed(() => printPreviewStore.pdfBlobUrl)
  const isLoading = computed(() => {
    const value = printPreviewStore.isLoading
    console.log('🔄 isLoading changed:', value)
    return value
  })
  ...
  const openPrintPreview = async (printSections: PrintSection[], config?: PdfConfig): Promise<void> => {
    console.log('🚀 openPrintPreview called in composable')
