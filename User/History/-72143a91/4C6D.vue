...
const {
  ...
  showDialog,
  hideAllDialogs,
  showErrorWithDetails: showErrorWithDetailsComposable,
  showConfirmDialog: showConfirmDialogComposable
} = useErrorDialog()
...
const showNestedDialogs = () => {
  // Show first dialog
  const firstDialogId = showError('First dialog opened')

  // Show second dialog after a delay
  setTimeout(() => {
    const secondDialogId = showWarning('Second dialog (nested)')
