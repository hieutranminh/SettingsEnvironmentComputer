...
import { ensureFontsReady, waitForFonts } from '@/utils/fontUtils'
...
export const usePdfGenerator = (): IUsePdfGeneratorReturn => {
  ...
  const generatePdf = async (sections: IPrintSection[], config?: IPdfConfig): Promise<Blob> => {
    try {
      ...
      const fontsReady = await waitForFonts(3000)
      if (!fontsReady) {
        console.warn('Fonts not ready after timeout, proceeding with fallback fonts')
      }
