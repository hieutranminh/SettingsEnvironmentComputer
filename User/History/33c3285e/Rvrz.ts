...
export const usePdfGenerator = (): UsePdfGeneratorReturn => {
  ...
  const processSections = async (
    ...
    for (let i = 0; i < sections.length; i++) {
      ...
      context.currentY = await processSection(pdf, section, config, context)
  ...
  const processSection = async (
    ...
    switch (section.refType) {
      case PRINT_TYPE.TABLE:
        return processTable(pdf, section, config, context)

      case PRINT_TYPE.CANVAS:
        return processCanvas(pdf, section, config, context)

      case PRINT_TYPE.TEXT:
        return processText(pdf, section, config, context)
