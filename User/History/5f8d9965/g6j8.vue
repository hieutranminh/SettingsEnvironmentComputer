  ...
  computed: {
    ...
    fromBookingDate: {
      ...
      set(fromBookingDate) {
        ...
        if (this.typeDate === options.type_date.date || this.toBookingDateTS < this.fromBookingDateTS) {
  ...
  watch: {
    value: {
      ...
      handler(value) {
        ...
        const now = convertDateToTimezone()
        this.toBookingDateTS = value?.toBookingDateTS ?? convertDateToMomentUTC(now).endOf('day').unix()
