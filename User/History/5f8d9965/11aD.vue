...
export default {
  ...
  computed: {
    ...
    toBookingDate: {
      get() {
        if (this.toBookingDateTS === null || this.toBookingDateTS === undefined) {
          return null
        }
    ...
    formValidateSchema() {
      ...
      const baseSchema = validation.schemaTS

      // If toBookingDate is not shown (typeDate is date, not date_range), allow toBookingDateTS to be null
      if (!this.isToBookingDateShown) {
        const schemaKeys = baseSchema._ids._byKey
        const toBookingDateTSKey = schemaKeys.get('toBookingDateTS')
        
        if (toBookingDateTSKey) {
          // Create a new schema with toBookingDateTS as optional/allow null
          return Joi.object({
            fromBookingDateTS: baseSchema._ids._byKey.get('fromBookingDateTS').schema,
            toBookingDateTS: baseSchema._ids._byKey.get('toBookingDateTS').schema.allow(null).optional(),
          }).unknown()
        }
      }
