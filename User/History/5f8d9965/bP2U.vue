...
export default {
  ...
  computed: {
    ...
    formValidateSchema() {
      ...
      // Always allow toBookingDateTS to be null, but validate other rules when it has a value
      return Joi.object({
        fromBookingDateTS: baseSchema._ids._byKey.get('fromBookingDateTS').schema,
        toBookingDateTS: baseSchema._ids._byKey.get('toBookingDateTS').schema
          .allow(null)
          .when(Joi.exist(), {
            then: baseSchema._ids._byKey.get('toBookingDateTS').schema,
