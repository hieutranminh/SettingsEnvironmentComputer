...
export default {
  ...
  computed: {
    ...
    dateFilterPrintText() {
      ...
      const toDate = formatTSToUTCDate(this.filter?.toBookingDateTS, options.standard_date_format.ymd)
  ...
  watch: {
    bookingLookupResult: {
      handler(newValue) {
        if (newValue) {
          const searchQuery = this.extractSearchQueryFromBookingLookupResult(newValue)
          if (searchQuery) {
            this.filter.nameOrMobile = searchQuery
            this.loadBookings()
          }
        }
      },
  ...
  created() {
    if (this.bookingLookupResult) {
      const searchQuery = this.extractSearchQueryFromBookingLookupResult(this.bookingLookupResult)
      if (searchQuery) {
        this.filter.nameOrMobile = searchQuery
      }
    }
  ...
  methods: {
    ...
    async loadBookings() {
      ...
      try {
        ...
        this.printFilter = {...this.filter, shopId: this.shop_data.shop_id, totalItems: this.pagingInfo.totalItems}
      } catch (error) {
    ...
    handleBookingLookupFromAhaAi(bookingLookupResult) {
      console.log('handleBookingLookupFromAhaAi', bookingLookupResult)
    },

    extractSearchQueryFromBookingLookupResult(bookingLookupResult) {
      if (!bookingLookupResult) return ''

      // Try to get from query field, or from first item's clientName/clientMobileNumber
      return bookingLookupResult.query
        || bookingLookupResult.searchText
        || bookingLookupResult.searchQuery
        || (bookingLookupResult.result?.items?.length > 0
          ? (bookingLookupResult.result.items[0].clientName || bookingLookupResult.result.items[0].clientMobileNumber || '')
          : '')
    },
  },
}
