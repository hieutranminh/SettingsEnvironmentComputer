<template>
  <div class="contents user-command-history">
    ...
    <div class="condition">
      <ul class="cf">
        ...
        <li>
          ...
          <span>
            <b-form-select
              v-model="filters.businessTypeCodes"
    ...
    <div class="buttonbox cf por mb10 d-flex">
      <span><div v-html="$t('user-command-history.label-total-record', { total: table_data.pagination.total_items })" /></span>
      <span class="ml-auto">
        <b-form-checkbox
          v-model="filters.isViewFailedOnly"
          :value="true"
          :unchecked-value="false"
          class="check-custom"
        >
          {{ $t('user-command-history.label-view-failed-only') }}
        </b-form-checkbox>
      </span>
    </div>
    ...
    <modal-user-command-history-result
      :current-result="currentResult"
      :modal-id="modalUserCommandHistoryResultId"
    />
  </div>
</template>
...
export default {
  ...
  data() {
    return {
      ...
      modalUserCommandHistoryResultId: 'modal-user-command-history-result',
    }
  },
  ...
  computed: {
    functionTypeOptions() {
      return [
        ...
        { value: FUNCTION_TYPE.COMMAND_PARSER, text: this.$t('user-command-history.label-option-command-parser') },
        { value: FUNCTION_TYPE.CLIENT_SEARCH, text: this.$t('user-command-history.label-option-client-search') },
        { value: FUNCTION_TYPE.BOOKING_ASSISTANT, text: this.$t('user-command-history.label-option-booking-assistant') },
        { value: FUNCTION_TYPE.SERVICE_BOOKING_ITEMS, text: this.$t('user-command-history.label-option-service-booking-items') },
        { value: FUNCTION_TYPE.CRM_FILTER_EXTRACTOR, text: this.$t('user-command-history.label-option-crm-filter-extractor') },
        { value: FUNCTION_TYPE.MESSAGE_GENERATOR, text: this.$t('user-command-history.label-option-message-generator') },
        { value: FUNCTION_TYPE.IMAGE_GENERATOR, text: this.$t('user-command-history.label-option-image-generator') },
        { value: FUNCTION_TYPE.REPORT_SUMMARY, text: this.$t('user-command-history.label-option-report-summary') },
        { value: FUNCTION_TYPE.COMPREHENSIVE_ANALYSIS_INSIGHTS, text: this.$t('user-command-history.label-option-comprehensive-analysis-insights') },
        { value: FUNCTION_TYPE.STAFF_PERFORMANCE_ANALYSIS_INSIGHTS, text: this.$t('user-command-history.label-option-staff-performance-analysis-insights') },
        { value: FUNCTION_TYPE.OPERATIONAL_TREND_ANALYSIS_INSIGHTS, text: this.$t('user-command-history.label-option-operational-trend-analysis-insights') },
        { value: FUNCTION_TYPE.CRM_FILTER_CLASSIFY, text: this.$t('user-command-history.label-option-crm-filter-classify') },
      ]
    },

    businessTypeOptions() {
      const defaultOptions = [
        { text: this.$t('general.all-select'), value: '' },
      ]
