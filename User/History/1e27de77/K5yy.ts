...
const rule: TSESLint.RuleModule<MessageIds, Options> = {
  ...
  meta: {
    type: 'suggestion',
    docs: {
      description: 'Disallow functions longer than 30 lines, except ones starting with use, Pinia stores, and schema functions',
    },
    schema: [],
    messages: {
  ...
  create(context) {
     ...
     */
    function isDestructuringOrObjectLine(
      line: string,
      lines: string[],
      lineIndex: number,
    ): boolean {
    ...
    /**
     * Check if function is a Pinia store definition
     * @param node - Function node
     * @returns True if this is a Pinia store
     */
    function isPiniaStore(node: TSESTree.FunctionExpression): boolean {
      const parent = node.parent
      
      // Check if this is a call to defineStore
    ...
    function checkFunction(
      ...
      if (functionName.startsWith('use') || functionName.endsWith('Schema')) return

      // Skip Pinia store definitions
      if (node.type === 'FunctionExpression' && isPiniaStore(node)) return
