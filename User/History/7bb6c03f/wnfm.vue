...
export default {
  ...
  methods: {
    ...
    initialFilters() {
      console.log('endTime', this.savedFilters.bookingStartTime.endTime)
      console.log('lastTimeOption', this.timeOptions[this.timeOptions.length - 1]?.value)
      if (this.savedFilters) {
        this.filters = {
          ...this.savedFilters,
          bookingDates:     this.savedFilters.bookingDates.sort((a, b) => a - b),
          estimatedTime:    roundUpEstimatedTime(this.savedFilters.estimatedTime),
          bookingStartTime: {
            ...this.savedFilters.bookingStartTime,
            startTime: roundDownTo60Minutes(this.savedFilters.bookingStartTime.startTime),
            endTime:   this.savedFilters.bookingStartTime.endTime > this.timeOptions[this.timeOptions.length - 1]?.value
              ? this.timeOptions[this.timeOptions.length - 1]?.value
              : roundDownTo60Minutes(this.savedFilters.bookingStartTime.endTime),
          },
        }
        return
      }
