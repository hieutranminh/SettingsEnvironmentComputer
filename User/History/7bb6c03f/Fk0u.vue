  ...
  methods: {
    ...
    generateTimeOptionsFromRange() {
      ...
      for(let time = startTimeInMinutes; time <= finishTimeInMinutes; time = time + DEFAULT_TIME_SLOT) {
        const isNextDay = time >= MINUTES_OF_24H
        const timeText = formatTimeBasedOnMinutes({ minutes: time, prefix: this.$t('general.next-day-text'), format: isNextDay ? 'HH:mm' : 'hh:mm A' })
        options.push({
          value: time,
          text:  timeText,
        })
