...
export const CalendarPlugin = store => {
  ...
  const getters = {
    ...
    dates(state, getters) {
      ...
      while(dates.length < state.numberOfDay) {
        ...
        availableBookingResources.map(resource => {
          const isBookingResourceOffDay = resource.specific_off_days.some(specificOffDay =>
            convertDateToMomentUTC(specificOffDay).startOf('day').isSame(convertDateToMomentUTC(date).startOf('day')),
          )
          if (!isBookingResourceOffDay) checkResourceOffDay = false
        })
        const isDayOff = !!state.specificOffDays.find(specificOffDay => {
          return convertDateToMomentUTC(new Date(specificOffDay)).startOf('day').isSame(convertDateToMomentUTC(date).startOf('day'))
        })
    ...
    schedule(state, getters) {
      ...
      for (let date of dates) {
        ...
        const specificOffDayIndex = specificOffDays.findIndex(specificOffDay => {
          return convertDateToMomentUTC(specificOffDay).startOf('day').isSame(convertDateToMomentUTC(date).startOf('day'))
        })
        ...
        availableBookingResources.map(resource => {
          resource.specific_off_days.some(specificOffDay => {
            if (convertDateToMomentUTC(specificOffDay).startOf('day').isSame(convertDateToMomentUTC(date).startOf('day'))) checkResourceOffDay = true
          })
        })
        ...
        */
        ...
        for (let index in availableBookingResources) {
          ...
          const bookingResourceOffDayIndex = bookingResource.specific_off_days.findIndex(specificOffDay => {
            return convertDateToMomentUTC(specificOffDay).startOf('day').isSame(convertDateToMomentUTC(date).startOf('day'))
          })
    ...
    timeSlots30Minutes(state, getters, rootState) {
      const isMobileDevice = rootState.device.deviceInfo.isMobileDevice
      const startTimeInMinutes = convertTimeToMinutes(state.startTime)
      const finishTimeInMinutes = convertTimeToMinutes(state.finishTime) + Number(state.crossDate) * options.minutes_of_24h
      ...
      const timeSlots = []
      const slots = (finishTimeInMinutes - startTimeInMinutes) / stepInMinutes

      for(let index = 0; index < slots; index++) {
        const minutes = startTimeInMinutes + (stepInMinutes * index)
        const text = (() => {
          if(isMobileDevice) {
            return formatTimeSlotAtMobile({ prefix: i18n.t('general.next-day-text'), minutes })
          }
  ...
  const actions = {
    ...
    gotoNextDate({ state, getters, commit }) {
      ...
      while(dates.length < numberOfDay) {
        ...
        if (state.isExcludeOffBookingResource) {
          const isDayOff = !!state.specificOffDays.find(specificOffDay => {
            return convertDateToMomentUTC(specificOffDay).isSame(nextDate, 'day')
          })
    ...
    async checkOffDay({state}, data) {
      ...
      const specificOffDayIndex = state.specificOffDays.findIndex(specificOffDay => {
        return convertDateToMomentUTC(specificOffDay).startOf('day').isSame(convertDateToMomentUTC(data.bookingDate).startOf('day'))
      })
