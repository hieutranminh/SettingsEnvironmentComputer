...
export default {
  ...
  methods: {
    ...
    validateAhaAiResult(result) {
      const { servicesAndBookingItemsNotExist, servicesAndBookingItemsExactMatch, servicesAndBookingItemsRelativeMatch } = result

      // Check if output is empty or invalid after parsing
      if (!servicesAndBookingItemsNotExist.length
      && !servicesAndBookingItemsExactMatch?.bookingItems?.length
