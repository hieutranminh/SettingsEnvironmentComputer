    ...
    <div class="modal-aha-ai-body">
      ...
      >
        ...
        <p
          ...
          :key="item.type === BOOKING_ITEM_TYPE.SERVICE_ITEM ? `service-${item.serviceId}` : `booking-${item.id}`"
        >
          {{ $t('aha-ai.message-found', { itemName: item.type === BOOKING_ITEM_TYPE.SERVICE_ITEM ? item.serviceName : item.bookingItemName }) }}
        </p>
