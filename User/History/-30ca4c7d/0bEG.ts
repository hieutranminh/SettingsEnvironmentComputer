...
import { PDF_MARGINS, PDF_SECTION_SPACING, PDF_TABLE_STYLES, PDF_PAGE_BREAK, PDF_FONT_SIZES, PDF_TEXT_ALIGNMENT, PDF_PAGE_NUMBERING } from '../constants/pdf'
...
export const usePdfTable = (): UsePdfTableReturn => {
  ...
  const addTableTitle = (
    ...
    const align: TextAlignment = (alignment as TextAlignment) || PDF_TEXT_ALIGNMENT.DEFAULT_HORIZONTAL
  ...
  const createPageDrawHandler =
    ...
    ({ doc }: { doc: jsPDF }) => {
      ...
      if (pageNumber > PDF_PAGE_NUMBERING.FIRST_PAGE_NUMBER && !context.pagesWithHeaders.has(pageNumber)) {
