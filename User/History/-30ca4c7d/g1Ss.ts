...
/**
 * Extended jsPDF interface with autoTable properties
 */
interface IJsPDFWithAutoTable extends jsPDF {
  lastAutoTable?: {
    finalY?: number
  }
}
...
/**
 * CreateTableBaseConfig interface
 */
interface ICreateTableBaseConfig {
...
export const usePdfTable = (): IUsePdfTableReturn => {
  ...
  const addTableTitle = (
    ...
    if (!section.title) return startY

    pdf.setFontSize(PDF_FONT_SIZES.TABLE_TITLE)
    pdf.setFont(FONTS.BOLD, PDF_FONT_WEIGHTS.BOLD)

    const xPosition = calculateTitlePosition(section.titleAlignment, pageWidth)
    const align: TextAlignment =
      (section.titleAlignment as TextAlignment) || PDF_TEXT_ALIGNMENT.DEFAULT_HORIZONTAL

    pdf.text(section.title, xPosition, startY, { align })
  ...
  const calculateTitlePosition = (alignment: string | undefined, pageWidth: number): number => {
    ...
    switch (align) {
      ...
      case PDF_TEXT_ALIGNMENT.HORIZONTAL.RIGHT:
   ...
   */
  const createTableStyles = (section: IPrintSection): Record<string, unknown> => ({
    headStyles: {
   ...
   */
  const createTableMargins = (context: IPdfProcessingContext): Record<string, unknown> => ({
    margin: {
  ...
  const createPageDrawHandler =
    (config: Required<IPdfConfig>, context: IPdfProcessingContext) =>
    ({ doc }: { doc: jsPDF }): void => {
   ...
   */
  const createOptionalTableConfig = (section: IPrintSection): Record<string, unknown> => ({
   ...
   */
  const createTableConfig = (options: ICreateTableConfigOptions): UserOptions => {
  ...
  const getFinalYPosition = (
    ...
    // Check if autoTable was executed and has finalY property
    const pdfWithAutoTable = pdf as IJsPDFWithAutoTable
    if (pdfWithAutoTable.lastAutoTable?.finalY) {
      return pdfWithAutoTable.lastAutoTable.finalY
    }
  ...
  const processTable = async (
    ...
    try {
      ...
      // Add table title
      currentY = addTableTitle(pdf, section, currentY, pageWidth)
      ...
      const message = parseError(error, 'Failed to process table')
