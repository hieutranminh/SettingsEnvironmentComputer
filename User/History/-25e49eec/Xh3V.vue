<template>
  ...
  <!-- Filter -->
  <BranchSalesFilter :modelValue="filters" @filtersChanged="handleFiltersChanged" />
...
const dateMappedText = computed(() => {
  if (filters.value.dateType === DATE_TYPE.DATE) {
    return formatDate(fromUnixTimestamp(filters.value.fromDateTs), { timezone: 'UTC' })
  } else if (filters.value.dateType === DATE_TYPE.MONTH) {
    return formatDate(fromUnixTimestamp(filters.value.fromDateTs), { format: 'YYYY-MM', timezone: 'UTC' })
  } else if (filters.value.dateType === DATE_TYPE.RANGE) {
    return `${formatDate(fromUnixTimestamp(filters.value.fromDateTs), { timezone: 'UTC' })} - ${formatDate(fromUnixTimestamp(filters.value.toDateTs), { timezone: 'UTC' })}`
  }
  return ''
})
