...
const dateMappedText = computed(() => {
  console.log('0', filters.value.toDateTs)
  console.log('1', formatDate(fromUnixTimestamp(filters.value.toDateTs), { format: 'YYYY-MM-DD HH:mm:ss' }))
  console.log(
    '2',
    formatDate(fromUnixTimestamp(filters.value.toDateTs), { format: 'YYYY-MM-DD HH:mm:ss', timezone: 'UTC' }),
  )
  if (filters.value.dateType === DATE_TYPE.DATE) {
    ...
    return `${formatDate(fromUnixTimestamp(filters.value.fromDateTs))} - ${formatDate(fromUnixTimestamp(filters.value.toDateTs), { timezone: 'UTC' })}`
  }
  return ''
})
