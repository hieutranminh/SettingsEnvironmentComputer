...
const dateMappedText = computed(() => {
  ...
  console.log('1', formatDate(fromUnixTimestamp(filters.value.toDateTs)))
  if (filters.value.dateType === DATE_TYPE.DATE) {
    ...
    return `${formatDate(fromUnixTimestamp(filters.value.fromDateTs))} - ${formatDate(
      fromUnixTimestamp(filters.value.toDateTs),
    )}`
  }
  return ''
})
