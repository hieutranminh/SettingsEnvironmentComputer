...
import { useSavedDateRangeFilter } from '@/composables/useSavedDateRangeFilter'
// Constants
...
const { filtersData, items, pagingInfo, fetchData } = useBranchSalesTotalReport()
const { openPrintPreview } = usePrintPreview()

// Filters state
const filters = ref<IBranchSalesFilterInterface>({
...
const { initializeWithDefaultRange } = useDateRangeFilter(filters, {
  defaultDateType: DATE_TYPE.DATE,
  autoUpdateOnDateTypeChange: true,
})

// Initialize saved date range filter composable
const { dateRangeText, saveCurrentFilters, initializeSavedFilters } = useSavedDateRangeFilter({
  defaultDateType: DATE_TYPE.DATE,
})

// Handle filters change
const handleFiltersChanged = async (): Promise<void> => {
  filters.value.pageNumber = PAGINATION.DEFAULT_PAGE_NUMBER

  await fetchData(filters.value)
  
  // Save current filters after successful fetch
...
onMounted(async () => {
  ...
  // Save initial filters after first fetch
  saveCurrentFilters(filters.value)
})
</script>
