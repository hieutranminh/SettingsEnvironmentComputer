...
import { MOBILE_PORTRAIT_MAX_WIDTH, CELL_HEIGHT_SETTING_NAME, NAVER_TALKTALK_URL } from 'Constant'
...
export function convertDateToTimeStampMs(date, is_convert_timezone, has_hours) {
  if (is_convert_timezone) date = convertDateToTimezone(date)
...
export function gotoLoginPageWhenPossibble(vue, countryCode) {
  ...
  if (login_page) {
    if (login_page.startsWith('http')) {
      ...
      // Prevent NavigationDuplicated error when already on login page
      const currentPath = vue.$route?.path || ''
      const normalizedCurrent = currentPath.split('?')[0].split('#')[0]
      const normalizedLogin = login_page.startsWith('/') ? login_page : '/' + login_page
      if (normalizedCurrent !== normalizedLogin) {
        vue.$router.push({ path: login_page }).catch(err => {
          if (err.name !== 'NavigationDuplicated') {
            console.log('Router navigation error:', err) // eslint-disable-line no-console
          } else {
            console.log('Navigation to login page ignored (already there)') // eslint-disable-line no-console
          }
        })
      }
    }
...
export async function convertImageToJPEG (imageFullPath, imageType) {
  try {
...
export function parseTimezoneToNumber(timezone) {
