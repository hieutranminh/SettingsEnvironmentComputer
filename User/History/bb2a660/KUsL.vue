...
const LIMIT_RANGE_MONTH = 7
const startOfMonth = getStartOf(getCurrentDate(), 'month').subtract(LIMIT_RANGE_MONTH, 'month').toDate()
...
const handlePrint = async (): Promise<void> => {
  if (salesByMonthChartRef.value && salesByMonthTableRef.value) {
    const printConfigChart = salesByMonthChartRef.value.getPrintConfiguration()
    const printConfigTable = salesByMonthTableRef.value.getPrintConfiguration()

    await printPreviewStore.openPrintPreview([printConfigChart, printConfigTable], {
      title: t('sales-by-month.title'),
