  ...
  <template v-if="filters.shopId">
    ...
    <SalesByMonthChart
      ...
      :prepaidSalesCountingType="
        savedFilters?.prepaidSalesCountingType ?? PREPAID_SALES_COUNTING_TYPE.SOLD
      "
    />
...
const { savedFilters, items, fetchReport } = useSalesByMonthReport()
const { openPrintPreview } = usePrintPreview()

// State
const MONTHS_TO_SUBTRACT = 8
const startOfMonth = getStartOf(getCurrentDate(), 'month')
  .subtract(MONTHS_TO_SUBTRACT, 'month')
  .toDate()
const endOfMonth = getEndOf(getCurrentDate(), 'month').toDate()
const filters = ref<ISalesByMonthFilterInterface>({
  shopId: 0,
...
const dateMappedText = computed(() => {
  return `${formatDate(fromUnixTimestamp(savedFilters.value?.fromDateTs ?? 0), { timezone: TIMEZONE_TYPE.UTC })} - ${formatDate(fromUnixTimestamp(savedFilters.value?.toDateTs ?? 0), { timezone: TIMEZONE_TYPE.UTC })}`
})
