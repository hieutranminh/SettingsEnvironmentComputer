...
export const usePrintPreviewStore = defineStore('printPreview', () => {
  ...
  const isReady = computed(() => isWorkerDone.value && !isWorkerError.value && pdfBlobUrl.value && excelBlob.value)

  const hasError = computed(() => isWorkerError.value || errorMessage.value)

  const canDownload = computed(() => isReady.value && !isWorkerProcessing.value)
