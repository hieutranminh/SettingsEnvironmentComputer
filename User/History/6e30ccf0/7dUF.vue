...
import { useBranchShopFilter } from '@/composables/useBranchShopFilter'
...
// Constants
import { DATE_TYPE, type DateType, FILTER_CONSTANTS } from '@/constants'
// Stores
...
interface Emits {
  (e: 'update:modelValue', value: BranchSalesFilterInterface): void
...
// Computed
const customBranchTypeIdModel = computed({
  get: () => props.modelValue.customBranchTypeId,
  set: (value): void => {
    emitUpdateModelValue({
      customBranchTypeId: value,
    })
  },
})

const branchGroupIdModel = computed({
  get: () => props.modelValue.branchGroupId,
  set: (value): void => {
    emitUpdateModelValue({
      branchGroupId: value,
    })
...
const branchShopIdsModel = computed({
  get: () => props.modelValue.branchShopIds,
  set: (value): void => {
    emitUpdateModelValue({
      branchName: '',
      branchShopIds: value,
...
const branchNameModel = computed({
  get: () => props.modelValue.branchName,
  set: (value): void => {
    emitUpdateModelValue({
      branchName: value,
    })
...
const handleDateChange = (value: { fromDateTs: number; toDateTs: number }, fetchOnNavigate: boolean): void => {
  emitUpdateModelValue({
 ...
 */
const handleBranchTypeChange = async (): Promise<void> => {
  await updateBranchShopSelection()
}
 ...
 * Triggers branch shop selection update to maintain filter consistency
 */
const handleBranchGroupChange = async (): Promise<void> => {
  await updateBranchShopSelection()
}
...
const emitUpdateModelValue = (value: Partial<BranchSalesFilterInterface>): void => {
