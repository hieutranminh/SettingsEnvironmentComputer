...
// Branch shop filter composable
const { branchShopOptions, isAllSelected } = useBranchShopFilter(
...
const branchNameModel = useModelBinding(props, emit, 'branchName')
...
const updateBranchShopSelection = async (): Promise<void> => {
  ...
  emitUpdateModelValue({
    branchName: '',
    branchShopIds: branchShopOptions.value.map((item) => item.id),
  })
}
