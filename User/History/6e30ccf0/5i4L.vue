      ...
      <div class="filter">
        <div class="filter-form">
          ...
          <div class="filter-form-group">
            <div class="filter-form-item" data-testid="sales-filter-group-branch-type">
            ...
            <div class="filter-form-item" data-testid="sales-filter-group-branch-group">
            ...
            <div class="filter-form-item" data-testid="sales-filter-group-branch">
              ...
              >
                <template #header>
                  <div class="multiselect-header-select-all" @click.stop="handleToggleAll">
                    <Checkbox
                      ...
                      :tabindex="-1"
                    />
                    <span>{{ $t('general.select-all') }}</span>
                  </div>
                </template>
            ...
            <div class="filter-form-item" data-testid="sales-filter-group-branch-name">
              <label for="branch-name-input">{{ $t('branch-sales.label-branch-name') }}</label>
              <InputText v-model="branchNameModel" @update:modelValue="handleBranchNameChange" />
            </div>
          </div>
        </div>
...
const handleToggleAll = (): void => {
  const branchShopIds = isAllSelected.value
    ? []
    : branchShopOptions.value.map((option) => option.id)
  emitUpdateModelValue({ branchShopIds })
}
...
const handleSearchClick = (): void => {
  ...
  if (props.modelValue.dateType === DATE_TYPE.RANGE) {
    ...
    if (!validationResult.isValid) {
      errors.push(t(validationResult.errorKey!))
    }
  }

  if (!branchShopOptions.value.length) {
    errors.push(t('branch-sales.validate-no-branches-belong-to-branch-type-and-group'))
  }

  if (!props.modelValue.branchShopIds.length) {
    errors.push(t('branch-sales.validate-branch-is-required'))
  }
...
const handleDateChange = (
  ...
  if (!branchShopOptions.value.length)
    return showError(t('branch-sales.validate-no-branches-belong-to-branch-type-and-group'))

  if (!props.modelValue.branchShopIds.length)
    return showError(t('branch-sales.validate-branch-is-required'))
 ...
 * Triggers branch shop selection update to maintain filter consistency
 */
const handleBranchNameChange = async (): Promise<void> => {
  await nextTick()

  // When branch name changes, reset filters to ALL and show all available branch shops
  const updatedFilteredBranchShopIds = branchSalesTotalFilter.value.map((item) => item.branchShopId)

  emitUpdateModelValue({
    customBranchTypeId: FILTER_VALUES.ALL,
    branchGroupId: FILTER_VALUES.ALL,
    branchShopIds: updatedFilteredBranchShopIds,
  })
}
