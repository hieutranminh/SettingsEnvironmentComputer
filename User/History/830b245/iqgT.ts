...
export class PrintService {
  ...
  private async drawTable(doc: jsPDF, data: PrintData, config: PrintConfiguration, startY: number): Promise<number> {
    ...
    // Draw table rows
    const startIndex = 0 // For PDF generation, we show all data
    const endIndex = data.printData.length
  ...
  private async drawChart(doc: jsPDF, data: PrintData, config: PrintConfiguration, startY: number): Promise<number> {
    if (!data.sectionRef) {
      return startY
    }
      ...
      switch (section.type) {
        ...
        case 'table':
          if (section.data && section.columns) {
            yPosition = await this.drawTable(
              doc,
              { ...data, printData: section.data, columns: section.columns },
              config,
              yPosition,
            )
          }
  ...
  private addFooter(doc: jsPDF, config: PrintConfiguration): void {
    ...
    doc.setFontSize(10)
    doc.setFont('helvetica', 'normal')
    doc.text(`Page ${doc.getCurrentPageInfo().pageNumber}`, pageWidth / 2, pageHeight - margin.bottom / 2, {
      align: 'center',
    })
  }
  ...
  async generateCSV(data: PrintData): Promise<Blob> {
    ...
    const rows = data.printData.map((row) => data.columns!.map((col) => row[col.field] || '').join(',')).join('\n')
