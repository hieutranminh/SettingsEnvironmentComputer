      ...
      >
        ...
        >
          <div class="sales-prepaid-services__wrapper--mobile">
            <b-form-checkbox
              v-if="isDeductPrepaidServicesButtonShown && !isSelectRowCheckboxDisabled(row)"
            ...
            <div
              class="sales-prepaid-services__main-container--mobile"
              :class="{ 'inactive': isSelectRowCheckboxDisabled(row) }"
            >
        ...
        >
          <b-form-checkbox
            v-if="!isSelectRowCheckboxDisabled(row)"
            :checked="isRowChecked(row)"
            class="sales-prepaid-services__checkbox"
            @change="onSelectPrepaidServiceRowData(row)"
          />
...
const PREPAID_SERVICE_SELECT_FIELD = { field: 'select_row', label: 'general.select', width: '5%', sortable: false, expand: true }
...
export default {
  ...
  computed: {
    ...
    isDeductPrepaidServicesButtonDisabled() {
      // Button disabled when no service is selected
      return this.selectedPrepaidServicesList.length === 0
    },
    ...
    prepaidServiceFields() {
      ...
      if(this.isDeductPrepaidServicesButtonShown) {
        prepaidServiceFields.unshift(PREPAID_SERVICE_SELECT_FIELD)
      }
  ...
  watch: {
    ...
    validPrepaidServices: {
      handler(newValidServices) {
        // Auto-select when only one valid service exists
        if (newValidServices.length === 1) {
          const onlyValidService = newValidServices[0]
          const isAlreadySelected = this.selectedPrepaidServicesList.some(item => item.id === onlyValidService.id)
          if (!isAlreadySelected) {
            this.selectedPrepaidServicesList = [onlyValidService]
          }
        }
      },
      immediate: true,
  ...
  methods: {
    ...
    isRowChecked(row) {
      return this.selectedPrepaidServicesList.some(item => item.id === row.id)
    },
  },
}
</script>
