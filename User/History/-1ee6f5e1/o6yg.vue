    ...
    >
      ...
      >
        ...
        <b-tab :title="$t('services.prepaid-services')">
          <client-prepaid-services :has-column-book="false" />
        </b-tab>
      </b-tabs>
    </b-modal>
...
import { MINUTES_OF_24H, PRIORITY_DISPLAY_CLIENT_PREPAID_GOODS, SECOND_OF_1M, MAX_BOOKING_SHOW_CLIENT_INFOR } from 'Constant'
...
export default {
  ...
  watch: {
    clientId: {
      immediate: true,
      handler(clientId) {
        clientId && this.loadClientInventory()
      },
    },
  },
      ...
      try {
        ...
        switch(priorityPrepaidGoodsDisplayDefault) {
          case PRIORITY_DISPLAY_CLIENT_PREPAID_GOODS.PREPAID_CARD_FIRST:
            if (hasValidPrepaidCard) {
              this.activeTab = TAB_OPTIONS.prepaidCards
            } else if (hasValidPrepaidService){
              this.activeTab = TAB_OPTIONS.prepaidServices
            }
            break

          case PRIORITY_DISPLAY_CLIENT_PREPAID_GOODS.PREPAID_SERVICE_FIRST:
            if (hasValidPrepaidService) {
              this.activeTab = TAB_OPTIONS.prepaidServices
            } else if (hasValidPrepaidCard) {
              this.activeTab = TAB_OPTIONS.prepaidCards
            }
            break

          default: {
            this.activeTab = TAB_OPTIONS.bookings
          }
        }
        ...
        if (this.activeTab === TAB_OPTIONS.prepaidServices) {
          if (clientPrepaidServices.length === 0) {
            this.activeTab = PRIORITY_DISPLAY_CLIENT_PREPAID_GOODS.PREPAID_SERVICE_FIRST && clientPrepaidServices.length > 0
              ? TAB_OPTIONS.prepaidServices
              : TAB_OPTIONS.bookings
          }
        } else if (this.activeTab === TAB_OPTIONS.prepaidCards) {
          if (clientPrepaidCards.length === 0) {
            this.activeTab = PRIORITY_DISPLAY_CLIENT_PREPAID_GOODS.PREPAID_CARD_FIRST && clientPrepaidCards.length > 0
              ? TAB_OPTIONS.prepaidCards
              : TAB_OPTIONS.bookings
          }
        }
      } catch (error) {
