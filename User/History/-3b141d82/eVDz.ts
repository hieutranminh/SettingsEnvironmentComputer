...
import type { IPrintSection, IPdfConfig } from '@/types/print'
...
interface ISectionProcessor {
  canProcess: (section: IPrintSection) => boolean
...
interface IUseExcelGeneratorReturn {
  generateExcel: (sections: IPrintSection[], config?: IPdfConfig, filename?: string) => Promise<void>
...
export const useExcelGenerator = (): IUseExcelGeneratorReturn => {
   ...
   */
  const createRequiredConfig = (config: IPdfConfig): Required<IPdfConfig> => {
    return {
      title: config.title ?? '',
   ...
   * @param params - Section processing parameters
   * @returns Updated row position
   */
  const processSection = async (params: ISectionProcessingParams): Promise<number> => {
    const { worksheet, section, currentRow, maxColumns } = params

    // Add section title if exists
    if (section.title) {
      const updatedRow = addSectionTitle(worksheet, section, currentRow, maxColumns)
      // Update currentRow for processor
      const updatedParams = { ...params, currentRow: updatedRow }

      // Find and execute appropriate processor
      const processor = sectionProcessors.find((processor) => processor.canProcess(section))
      if (processor) {
        return await processor.process(updatedParams)
      }
