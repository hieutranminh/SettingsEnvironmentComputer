...
export default {
  ...
  methods: {
    // Clone charts section and remove display-item dropdowns for print
    createChartsPrintElement() {
      const original = this.$refs.chartsSection
      if (!original) return null

      // Clone the entire element
      const clone = original.cloneNode(true)

      // Remove all .display-item elements from the clone
      const displayItems = clone.querySelectorAll('.display-item')
      displayItems.forEach(el => el.remove())

      // Append clone to body temporarily (needed for html2canvas to capture)
