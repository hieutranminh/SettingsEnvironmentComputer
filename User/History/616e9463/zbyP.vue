<template>
  ...
  <template v-if="filters.shopId">
    <!-- Filter -->
    <SalesByDiscountCategoryFilter v-model="filters" @filtersChanged="handleFiltersChanged" />

    <!-- Chart and Table Layout -->
    <div class="sales-by-discount-category-content">
      <!-- Chart - Left Side -->
      <div class="chart-container">
        <SalesByDiscountCategoryChart ref="salesByDiscountCategoryChartRef" :data="items" />
      </div>

      <!-- Table - Right Side -->
      <div class="table-container">
        <SalesByDiscountCategoryTable ref="salesByDiscountCategoryTableRef" :data="items" />
      </div>
    </div>
  </template>
</template>
...
import { useSalesByDiscountCategoryReport } from '@/composables/report-by-branch/useSalesByDiscountCategory'
...
import SalesByDiscountCategoryChart from './partials/SalesByDiscountCategoryChart.vue'
import SalesByDiscountCategoryFilter from './partials/SalesByDiscountCategoryFilter.vue'
import SalesByDiscountCategoryTable from './partials/SalesByDiscountCategoryTable.vue'
...
const { filterData, items, fetchReport } = useSalesByDiscountCategoryReport()
...
const filters = ref<SalesByDiscountCategoryFilterInterface>({
...
// Component refs
const chartRef = ref<InstanceType<typeof SalesByDiscountCategoryChart> | null>(null)
const tableRef = ref<InstanceType<typeof SalesByDiscountCategoryTable> | null>(null)
...
const handlePrint = async (): Promise<void> => {
  ...
  if (chartRef.value) sections.push(chartRef.value.getPrintConfiguration())
  if (tableRef.value) sections.push(tableRef.value.getPrintConfiguration())
...
.sales-by-discount-category-content {
  display: grid;
