...
export const authAggrService = {
  ...
  login: async (data: ILoginRequest, culture: string): Promise<IApiResponse<ILoginResponse>> => {
    ...
    return await apiAggr.post<ILoginResponse>(`auth/Login/Subscriber?culture=${culture}`, payload)
  },
  ...
  loginByKey: async (loginKey: string, culture: string): Promise<IApiResponse<ILoginResponse>> => {
    const params = new URLSearchParams({ key: loginKey, culture: 'ko-KR', 'ui-culture': 'ko-KR' })
    return await apiAggr.post(`auth/Login/LoginKey?${params.toString()}`, null)
  },
