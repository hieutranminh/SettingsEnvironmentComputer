      ...
      <table ref="tableRef">
        ...
        <tbody v-if="tableRows.length > 0">
          ...
          >
            ...
            <td class="text-center">
              {{ formatNumber(row.sameDayCancelCount) }}
            </td>
            <td class="text-center">
              {{ formatPercent(row.sameDayCancelRatio) }}
            </td>
            <td class="text-center">
            ...
            <td class="text-center">
              {{ formatNumber(row.sameDayCancelAndNoShowCount) }}
            </td>
            <td class="text-center">
              {{ formatPercent(row.sameDayCancelAndNoShowRatio) }}
            </td>
          </tr>
        </tbody>
  ...
  computed: {
    tableRows() {
      ...
      const rows = this.sectionData.data.map(item => ({
        ...
        sameDayCancelCount:          item.sameDayCancel || 0,
        sameDayCancelRatio:          item.sameDayCancelRatio || 0,
        noShowCount:                 item.noShow || 0,
        noShowRatio:                 item.noShowRatio || 0,
        sameDayCancelAndNoShowCount: item.sameDayCancelAndNoShow || 0,
        sameDayCancelAndNoShowRatio: item.sameDayCancelAndNoShowRatio || 0,
        isTotal:                     false,
      }))
