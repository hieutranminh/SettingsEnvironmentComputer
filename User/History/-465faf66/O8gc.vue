...
export default {
  ...
  computed: {
    ...
    /**
     * Get blocked time start time in minutes
     * @returns {Number} Start time in minutes (with next day offset if applicable)
     */
    blockedStartTime() {
    ...
    /**
     * Get blocked time end time in minutes
     * @returns {Number} End time in minutes (with next day offset if applicable)
     */
    endTime() {
    ...
    /**
     * Calculate blocked time visual style (position and height)
     *
     * @description
     * Clips the blocked time display to the calendar opening hours.
     * If blocked time starts before opening time, only the visible portion is rendered.
     *
     * @example
     * - Opening time: 10:30 AM (630 minutes)
     * - BlockedTime: 10:00 AM - 9:00 PM (600-1260 minutes)
     * - Current slot start (inject): 10:30 AM (630 minutes)
     *
     * Calculation:
     * - visibleStartTime = max(600, 630) = 630
     * - height = (1260 - 630) * unitHeight = 630 minutes worth of height
     * - top = (630 - 630) * unitHeight = 0px (at the top of the first slot)
     *
     * @returns {Object} Style object with top and height in pixels
     */
    blockedTimeStyle() {
      ...
      // Calculate height based on visible portion only
      const height = (this.endTime - visibleStartTime) * this.timeSlotUnitHeight

      // Calculate position relative to opening time
      const top = (this.startTime - this.openingStartTimeInMinutes) * this.timeSlotUnitHeight
