...
/**
 * Build surface color config
 */
function buildSurfaceColorConfig(surfaceColor: SurfaceColorName): Record<string, string> | null {
...
const getInitialThemeConfig = (): { preset: unknown; options: { darkModeSelector: string } } => {
  ...
  const surfaceConfig = buildSurfaceColorConfig(preferences.surfaceColor)
  ...
  if (surfaceConfig) {
    semanticConfig.surface = surfaceConfig
  }
...
export function updateThemePreset(
  ...
  const surfaceConfig = surfaceColor ? buildSurfaceColorConfig(surfaceColor) : null
