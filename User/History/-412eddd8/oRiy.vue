...
// Constants
import { PRINT_TYPE, REPORT_TYPE_IN_CLIENTS_BY_TYPE, type ReportTypeInClientsByType } from '@/constants'
// Types
...
const tableData = computed<ClientsByTypeReportItem[]>(() => {
...
// Helper functions
const transformDataItem = (item: ClientsByTypeReportItem): ClientsByTypeReportItem => {
  const safeTotalClient = item.totalClient ?? 0
  const ratio = totalClientCount.value > 0 ? (safeTotalClient / totalClientCount.value) * PERCENTAGE_MULTIPLIER : 0
...
const formatPercentage = (value: number): string => {
  return `${formatAmount(value, { decimalCount: DECIMAL_PLACES }) || 0}%`
}
