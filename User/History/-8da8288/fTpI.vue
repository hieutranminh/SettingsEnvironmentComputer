...
const averageAmount: ComputedRef<number> = computed(() => {
  ...
  const totalAmount = getAmountByType(totals.value as ISalesByDateReportItem, displayItemType.value)
 ...
 */
const getAmountByType = (item: ISalesByDateReportItem, type: number): number => {
  const typeMap: Record<number, keyof ISalesByDateReportItem> = {
