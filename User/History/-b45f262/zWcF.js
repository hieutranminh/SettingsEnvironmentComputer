...
import { getCacheData, initialIsDisabledCacheBooking } from 'CommonHelpers'
...
export const BOOKING_CACHE_DURATION = 5000 // 5 seconds
const bookingCache = new Map()
const isUseCacheForSetup = true
...
const actions = {
  async getBookingCalendarLive({ state, commit, dispatch }, bookingFilter) {
    ...
    const cacheKey = JSON.stringify(bookingFilterClone)
    const now = Date.now()
    ...
    let response = null
    let isDisabledCacheBooking = initialIsDisabledCacheBooking('getBookingCalendarLive')

    const cachedData = isDisabledCacheBooking ? null : getCacheData(bookingCache, cacheKey, BOOKING_CACHE_DURATION, 'getBookingCalendarLive')

    if(!isDisabledCacheBooking) {
      // Initial if enter booking page first (initial variable is in app-footer(sale page))
      // Load initial disabled cache booking:
      isDisabledCacheBooking = initialIsDisabledCacheBooking('bookings > getBookingCalendarLive()')
    }

    if(isUseCacheForSetup && cachedData && cachedData.safeCountNumberCalling >= 2 && !isDisabledCacheBooking) {
      response = cachedData
    } else {
      // Get Data From Api
      response = await getBookingCalendarLive(state.bookingFilter)

      // Cache response
      bookingCache.set(cacheKey, {
        timestamp:              now,
        data:                   cloneDeep(response),
        safeCountNumberCalling: bookingCache.has(cacheKey) ? bookingCache.get(cacheKey).safeCountNumberCalling + 1 : 1,
      })

      // Clear old cache entries
      const clearOldCache = () => {
        bookingCache.forEach((value, key) => {
          if (now - value.timestamp > BOOKING_CACHE_DURATION) {
  ...
  async noShowNaverBooking({ state, dispatch }, data) {
    ...
    console.log('noShowNaverBooking response:', result)

    const existedBooking = state.bookingSet[result.bookingId]
    const clonedBooking = cloneDeep(existedBooking)

    // Update refund information if exists
    if (clonedBooking.extSystemBookingDescriptionBase) {
      // Check for refund in externalSystemBookingRefund (direct response)
      if (result.externalSystemBookingRefund) {
        console.log('Found externalSystemBookingRefund:', result.externalSystemBookingRefund)
        clonedBooking.extSystemBookingDescriptionBase.refundAmount = result.externalSystemBookingRefund.refundAmount || 0
        clonedBooking.extSystemBookingDescriptionBase.refundDateTimeTS = result.externalSystemBookingRefund.refundDateTimeTS || 0
      }
      // Check for refund in bookingDeposit structure
      else if (result.bookingDeposit?.bookingDepositPayment?.bookingDepositPaymentRefund) {
        console.log('Found bookingDepositPayment.bookingDepositPaymentRefund:', result.bookingDeposit.bookingDepositPayment.bookingDepositPaymentRefund)
      ...
      console.log('Updated extSystemBookingDescriptionBase:', clonedBooking.extSystemBookingDescriptionBase)
    }
