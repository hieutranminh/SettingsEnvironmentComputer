...
const actions = {
  ...
  async removeBookingById({ dispatch, state, rootState }, { bookingId }) {
    const booking = state.bookingSet?.[bookingId]
    if (!booking) return

    // Cleanup drag state if the cancelled booking is the ghostBooking being dragged
    const dragState = rootState._calendar?.drag
    if (dragState?.ghostBooking?.bookingId === bookingId || dragState?.ghostBooking?.originalBookingId === bookingId) {
      await dispatch('_calendar/drag/endDraggingBooking', null, { root: true })
    }
