...
const actions = {
  ...
  async addBooking({ commit, dispatch, state }, { booking }) {
    ...
    // Use for...of instead of forEach to properly handle async operations and prevent race conditions
    for (const bookedResource of booking.bookedResources) {
      ...
      if (existedBooking?.bookingSource === BOOKING_SOURCE.NAVER || (existedBooking && hasSingleResource && booking.bookedResources.length <= 1)) {
        await dispatch('removeBookingById', { bookingId: booking.bookingId })
      }

      if (!excludeBookingStatuses.includes(booking.status)) {
        commit(`_calendar/bookings/${bookingItemModuleName}/addItem`, {
          ...booking,
          bookedResources: [bookedResource],
        }, { root: true })
      }
