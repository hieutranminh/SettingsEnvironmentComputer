...
export default {
  ...
  props: {
    ...
    // When provided, controls the selected display item from parent
    displayItem: {
      type:    String,
      default: null,
    },
    // Hide the display item dropdown (for print mode)
    hideDisplayItem: {
      type:    Boolean,
      default: false,
    },
  ...
  data() {
    return {
      internalDisplayItem: this.defaultDisplayItem,
    }
  },

  computed: {
    // Use prop displayItem if provided, otherwise use internal state
    selectedDisplayItem: {
      get() {
        return this.displayItem ?? this.internalDisplayItem
      },
      set(value) {
