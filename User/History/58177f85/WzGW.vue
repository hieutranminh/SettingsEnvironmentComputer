    ...
    <template #content>
      ...
      >
        ...
        <ColumnGroup type="header">
          <Row>
            <Column :header="$t('product-sales-by-item.label-product')" headerClass="bg-gray" style="width: 25%" />
...
import type { ProductSalesByItemReportItem } from '@/types/sales-report/ProductSalesByItem'
// Utils
import { formatAmount } from '@/utils/common'

interface Props {
  data?: ProductSalesByItemReportItem[]
}
...
const transformedData = computed<ProductSalesByItemReportItem[]>(() => {
  if (!props.data || props.data.length === 0) {
    return []
  }

  const totalAmount = props.data.reduce((acc, x) => acc + (x?.amount ?? 0), 0)
