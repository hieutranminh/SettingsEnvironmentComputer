  ...
  <Card>
    <template #content>
      ...
      <DataTable
        ...
        scrollHeight="550px"
        tableClass="product-sales-by-item-table"
        showGridlines
      >
        <!-- Header -->
        <ColumnGroup type="header">
          <Row>
            <Column :header="handleDisplayReportByType" headerClass="bg-gray first-column" />
            <Column :header="$t('product-sales-by-item.label-qty')" headerClass="bg-gray" />
            <Column :header="$t('product-sales-by-item.label-amount')" headerClass="bg-gray" />
            <Column :header="$t('product-sales-by-item.label-ratio')" headerClass="bg-gray" />
          </Row>
        </ColumnGroup>
...
// Constants
import { PRINT_TYPE, REPORT_BY_TYPE_PRODUCT, type ReportByTypeProduct } from '@/constants'
// Types
import type { IPrintSection } from '@/types/print'
import type { IProductSalesByItemReportItem } from '@/types/sales-report/ProductSalesByItem'
// Utils
import { formatAmount, formatPercentage } from '@/utils/common'

interface IProps {
  data?: IProductSalesByItemReportItem[]
...
const transformedData = computed<IProductSalesByItemReportItem[]>(() => {
  if (!props.data || props.data.length === 0) {
    return []
  }

  const totalAmount = props.data.reduce((acc, item) => acc + (item?.amount ?? 0), 0)
...
const handleDisplayReportByType = (): string => {
  switch (props.reportByType) {
    case REPORT_BY_TYPE_PRODUCT.PRODUCT:
      return t('product-sales-by-item.label-product')
    case REPORT_BY_TYPE_PRODUCT.PRODUCT_CATEGORY:
      return t('product-sales-by-item.label-product-category')
    case REPORT_BY_TYPE_PRODUCT.STAFF:
      return t('product-sales-by-item.label-staff')
    default:
      return t('product-sales-by-item.label-product')
  }
}
...
:deep(.product-sales-by-item-table) {
  .bg-gray {
