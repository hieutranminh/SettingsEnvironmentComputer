  ...
  <div>
    ...
    >
      ...
      <div class="search-client-table">
        ...
        <div
          v-if="!isMobileDevice"
          class="search-client-table__table-result"
        >
          <view-table
            :data="tableData"
          ...
          >
            ...
            <template #notes="{ row }">
              <aha-note-with-tooltip
                v-if="row.notes"
                :value="row.notes"
                :tooltip-id="`modal-search-client-notes-${row.clientId}`"
                placement="left"
        ...
        >
          <client-search-mobile
            ...
            @client-clicked="handleRowMobileClicked"
            @show-client-information="onShowClientInformation"
          />
        </div>
      </div>
    </b-modal>
...
import ClientSearchMobile from 'Modules/clients/components/client-search/components/client-search-mobile.vue'
...
export default {
  ...
  methods: {
    ...
    onCancel () {
      ...
      this.currentPage = 1
      this.pagination.pageSize = options.pagination.default
    },
    ...
    async handleRowMobileClicked(row) {
      const pageIndex = this.items.findIndex(item => item.clientId === row.clientId)
      await this.handleRowClick({ row, pageIndex })
    },

    onClickMore() {
      this.currentPage += 1
      this.pagination.pageNumber = 1
      this.pagination.pageSize = this.currentPage * options.pagination.default

      this.loadClientSearch()
    },

    async onShowClientInformation(client) {
      try {
        this.preLoader()
        await this.openClientInformation({
          client:                  client,
          isShowButtonGroupAction: false,
        })
      } catch (error) {
        this._showDialogAlert(error.message)
      } finally {
