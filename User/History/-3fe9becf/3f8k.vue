  ...
  methods: {
    ...
    async fetchReportDataSales(filter) {
      try {
        ...
        // Check if error is AI server error but still has result data
        const hasAiServerError = error.codes?.includes(ERROR_CODES.ERR_COM_API_SERVER_ERROR)
        const hasResultData = error.result != null && Object.keys(error.result).length > 0

        if (hasAiServerError && hasResultData) {
          // Save data even when AI failed - sections will show ai-analysis-failed message
          this.reportDataSales = error.result
    ...
    async fetchReportDataBooking(filter) {
      try {
        ...
        if (hasAiServerError && hasResultData) {
          // Save data even when AI failed - sections will show ai-analysis-failed message
          this.reportDataBooking = error.result
          return true
        }
