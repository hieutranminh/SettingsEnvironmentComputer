...
export default {
  ...
  computed: {
    ...
    parsedAiOutputBooking() {
      if (!this.reportDataBooking?.aiOutput) {
        return null
      }
      try {
        return JSON.parse(this.reportDataBooking.aiOutput)
      } catch (error) {
        return null
      }
    },
    ...
    aiSummaryBooking() {
      // If has data but no booking_efficiency section, show error message
      if (this.bookingData && !this.hasAiSectionBooking('booking_efficiency')) {
        return [this.$t('report.ai-analysis-failed')]
      }
      return this.getAiSummaryBySectionBooking('booking_efficiency')
    },
  },
  ...
  methods: {
    ...
    getAiSummaryBySectionBooking(sectionKey) {
      if (!this.parsedAiOutputBooking?.sections) {
        return []
      }
      const section = this.parsedAiOutputBooking.sections.find(item => item.section === sectionKey)
      return section?.summary ?? []
    },

    hasAiSectionBooking(sectionKey) {
      if (!this.parsedAiOutputBooking?.sections) {
        return false
      }
      return this.parsedAiOutputBooking.sections.some(item => item.section === sectionKey)
    },
    ...
    async fetchReportData(filter) {
      try {
        ...
        const [salesSuccess, bookingSuccess] = await Promise.all([
          this.fetchReportDataSales(filter),
          this.fetchReportDataBooking(filter),
        ])

        console.log('reportDataBooking', this.reportDataBooking)

        return salesSuccess && bookingSuccess
      } catch (error) {
