...
export default {
  ...
  data() {
    return {
      ...
      isCleaningUp:              false, // Flag to prevent starting while cleanup in progress
      basePrompt:                '',
      finalTranscripts:          [],
      callbacks:                 null,
  ...
  methods: {
    ...
    async $mixinSpeechRecognition_startVoiceRecognition(options) {
      ...
      // Check if already initializing, active, or cleaning up
      if (this.isInitializingRecognition || this.speechRecognition || this.isCleaningUp) {
        return
      }
     ...
     * Waits for browser to fully release microphone before allowing new session
     */
    $mixinSpeechRecognition_cleanup() {
      ...
      if (this.speechRecognition) {
        ...
        const recognitionToCleanup = this.speechRecognition

        // Set up one-time handler to detect when browser fully releases microphone
        const originalOnEnd = recognitionToCleanup.onend
        recognitionToCleanup.onend = () => {
          originalOnEnd?.()
          // Small delay to ensure browser fully releases microphone hardware
          setTimeout(() => {
            this.isCleaningUp = false
          }, 100)
        }

        abortRecognition(recognitionToCleanup)
        this.speechRecognition = null
      }
    ...
    $mixinSpeechRecognition_reset() {
      this.$mixinSpeechRecognition_cleanup()
      this.isCleaningUp = false // Force reset for fresh state
      this.basePrompt = ''
      this.finalTranscripts = []
      this.hasSpeechDetected = false
    },
