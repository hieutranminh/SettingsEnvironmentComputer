...
export default {
  ...
  methods: {
     ...
     * If no speech detected for 5 seconds after receiving first result, automatically stop recognition
     */
    _startSilenceTimer() {
      this._clearSilenceTimer()
      ...
      this.silenceTimer = setTimeout(() => {
        if (this._isComponentActive() && this.speechRecognition) {
          // Build current prompt to check if we have any text
          const currentPrompt = this._buildUpdatedText('')

          // Only auto-stop if we have received results and have prompt text
          if (this.hasReceivedResult && currentPrompt && currentPrompt.trim().length > 0) {
            // Save callbacks before cleanup (cleanup sets callbacks to null)
            const savedCallbacks = this.callbacks

            // Auto-stop after 5 seconds of silence
            this.$mixinSpeechRecognition_cleanup()

            // Call onEnd callback after cleanup to switch to typing mode
    ...
    /**
     * Reset silence timer when new result is received
     */
    _resetSilenceTimer() {
    ...
    _handleOnResult(recognition, result) {
      ...
      // Mark that we have received at least one result
      this.hasReceivedResult = true

      // Start or reset silence timer on any result (interim or final)
      // This indicates user is still speaking or has spoken recently
