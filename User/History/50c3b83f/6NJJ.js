...
// Auto-stop voice recognition after silence
const INITIAL_SILENCE_TIMEOUT_MS = 20000 // Timeout when user hasn't spoken anything yet
const AFTER_SPEECH_SILENCE_TIMEOUT_MS = 20000 // Timeout after user has spoken and paused
...
export default {
  ...
  data() {
    return {
      ...
      isIntentionallyStopped:    false, // Flag to track if we intentionally stopped recognition
    }
  },
  ...
  methods: {
     ...
     * Uses different timeout based on whether user has spoken
     * - Initial: 20 seconds (waiting for first speech)
     * - After speech: 20 seconds (user finished speaking)
     */
    _startSilenceTimer() {
      this._clearSilenceTimer()
      const timeoutMs = this.hasSpeechDetected
        ? AFTER_SPEECH_SILENCE_TIMEOUT_MS
      ...
      this.silenceTimeoutId = setTimeout(() => {
        if (this.speechRecognition) {
          ...
          // Mark as intentionally stopped by our timeout
     ...
     * @returns {string}
     */
    _buildUpdatedText(interimTranscript) {
      const finalText = this.finalTranscripts.join(' ')
      const parts = [this.basePrompt, finalText, interimTranscript]
    ...
    _handleOnResult(recognition, result) {
      ...
      // Mark that speech has been detected (for shorter timeout after speaking)
      this.hasSpeechDetected = true

      // Accumulate final transcripts (confirmed text)
      if (result.finalTranscript) {
        this.finalTranscripts.push(result.finalTranscript)
      }

      const updatedText = this._buildUpdatedText(result.interimTranscript)
     ...
     * Browser may stop recognition due to its internal timeout (5-7s of silence)
     * We need to restart if it wasn't intentionally stopped by us
     * @param {Object} recognition - Recognition instance
     */
    _handleOnEnd(recognition) {
      ...
      // If we intentionally stopped, clean up normally
      ...
      // Browser stopped recognition unexpectedly (internal timeout)
      // Restart recognition if we still have callbacks (user hasn't stopped)
      if (this.callbacks && this._isComponentActive()) {
        this._restartRecognition()
      }
    },

    /**
     * Restart speech recognition after browser's internal timeout
     * Preserves current state (basePrompt, finalTranscripts)
     */
    async _restartRecognition() {
      ...
      try {
        // Create new recognition instance with same handlers
        ...
        if (!recognition) {
          // Failed to create recognition, end session
          this._clearSilenceTimer()
    ...
    $mixinSpeechRecognition_cleanup() {
      this._clearSilenceTimer()
      this.isIntentionallyStopped = true // Mark as intentionally stopped to prevent restart
      if (this.speechRecognition) {
