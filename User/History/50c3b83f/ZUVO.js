...
export default {
  ...
  methods: {
     ...
     * @param {string} fullTranscript - All confirmed/final transcripts from speech recognition
     * @param {string} interimTranscript - Current interim transcript (still being processed)
     * @returns {string}
     */
    _buildUpdatedText(fullTranscript, interimTranscript) {
      const parts = [this.basePrompt, fullTranscript, interimTranscript]
     ...
     * On Safari mobile and Android, permission is handled automatically by Speech Recognition API
     * Manual permission check on these platforms can cause false negative errors
     * @param {Function} onError - Error callback
     * @returns {Promise<boolean>} True if permission granted
     */
    async _checkAndRequestPermission(onError) {
      // Safari mobile and Android handle permission automatically via Speech Recognition API
      // Skip manual permission check to avoid false negative errors and microphone conflicts
      if (isSafariMobile() || isAndroid()) {
        return true
      }
     ...
     * @param {Object} recognition - Recognition instance
     * @param {Object} result - Recognition result containing fullTranscript and interimTranscript
     */
    _handleOnResult(recognition, result) {
      ...
      // Build updated text using fullTranscript (all confirmed text) and interimTranscript (currently processing)
      // This approach reads all results from the beginning each time, preventing duplicate text
      // that can occur on some devices (Samsung Galaxy) where interim includes already-final text
      const updatedText = this._buildUpdatedText(result.fullTranscript, result.interimTranscript)
