...
export default {
  ...
  methods: {
    ...
    /**
     * Check if new transcript is a duplicate of existing transcripts
     * Handles Android/Samsung duplicate issue where same text is sent multiple times
     * @param {string} newTranscript - New transcript to check
     * @returns {boolean} True if duplicate
     */
    _isDuplicateTranscript(newTranscript) {
      if (!newTranscript || this.finalTranscripts.length === 0) {
        return false
      }

      const trimmedNew = newTranscript.trim()
      const currentFinalText = this.finalTranscripts.join(' ')

      // Check if current final text already ends with this transcript
      if (currentFinalText.endsWith(trimmedNew)) {
        return true
      }

      // Check if last transcript is the same
      const lastTranscript = this.finalTranscripts[this.finalTranscripts.length - 1]
      if (lastTranscript && lastTranscript.trim() === trimmedNew) {
        return true
      }

      // Check if new transcript contains all previous text (Android sometimes sends cumulative results)
      if (trimmedNew.startsWith(currentFinalText)) {
        return true
      }
    ...
    _handleOnResult(recognition, result) {
      ...
      // Android/Samsung fix: Only add if not a duplicate
      if (result.finalTranscript) {
        const trimmedTranscript = result.finalTranscript.trim()
        const currentFinalText = this.finalTranscripts.join(' ')

        if (this._isDuplicateTranscript(trimmedTranscript)) {
          // Skip duplicate transcript
        } else if (trimmedTranscript.startsWith(currentFinalText) && currentFinalText.length > 0) {
          // Android cumulative result: replace all with new cumulative text
          const newPart = trimmedTranscript.slice(currentFinalText.length).trim()
          if (newPart) {
            this.finalTranscripts.push(newPart)
          }
        } else {
          // Normal case: add new transcript
