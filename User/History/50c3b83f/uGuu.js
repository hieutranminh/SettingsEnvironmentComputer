...
export default {
  ...
  methods: {
    ...
    _handleOnResult(recognition, result) {
      ...
      // Android/Samsung fix: Handle duplicate and cumulative results
      if (result.finalTranscript) {
        ...
        // e.g., currentFinalText = "Hello", new = "Hello"
        if (currentFinalText === trimmedTranscript) {
          // Skip exact duplicate
        }
        // Case 2: Current text ends with new transcript - skip (duplicate suffix)
        // e.g., currentFinalText = "Hello world", new = "world"
        else if (currentFinalText.length > 0 && currentFinalText.endsWith(trimmedTranscript)) {
          // Skip duplicate suffix
        }
        // Case 3: Android cumulative result - extract only new part
        // e.g., currentFinalText = "Hello", new = "Hello my name is Hieu"
        else if (currentFinalText.length > 0 && trimmedTranscript.startsWith(currentFinalText)) {
          const newPart = trimmedTranscript.slice(currentFinalText.length).trim()
          if (newPart) {
            this.finalTranscripts.push(newPart)
          }
        }
        // Case 4: Normal new transcript - add it
        else {
