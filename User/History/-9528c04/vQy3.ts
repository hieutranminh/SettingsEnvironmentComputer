...
export const clientReportReadService = {
  ...
  /**
   * Fetches clients by type report for a specific chain
   * @param request - The request payload containing clients by type report request
   * @returns Promise with the clients by type report response
   */
  getClientsByTypeReport: (request: ClientsByTypeReportRequest): Promise<ClientsByTypeReportResponse> => {
    return apiRead.post<ClientsByTypeReportResult>('sales/ClientReport/GetClientsByTypeReport', request)
  },
}
