      ...
      <div class="table">
        ...
        >
          ...
          <template
            ...
            slot-scope="{row}"
          >
            TZ - {{ row.created_date_time_ts | formatDateToYMD }} <br>
            UTC - {{ row.created_date_time_ts | formatUTCDate }}
          </template>
          <template
            slot="action"
            slot-scope="{row}"
...
const TABLE_FIELD = [
  {field: 'date', label: 'receivables-edit-history.date', width: '20%', sortable: false, expand: true},
