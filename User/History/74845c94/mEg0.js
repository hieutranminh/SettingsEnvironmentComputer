...
/**
 * Compare if a specific off day matches a calendar date
 * @param {Date|string|number} offDay - Date object, string, or timestamp representing specific off day
 * @param {moment} calendarDate - Moment UTC representing calendar date
 * @returns {boolean} - True if dates match in shop timezone
 */
export const isSameOffDay = (offDay, calendarDate) => {
  const shopTimezone = store.state.authentication.shop.timezone

  // Get UTC timestamp from offDay
  let timestamp
  if (offDay instanceof Date) {
    // Date object from convertTimestampToDate has year/month/date/hour/minute/second from UTC
    // but was created with local timezone, so internal timestamp is wrong.
    // Reconstruct UTC timestamp from the stored values.
    timestamp = Date.UTC(
      offDay.getFullYear(),
      offDay.getMonth(),
      offDay.getDate(),
  ...
  } else if (typeof offDay === 'number') {
    // Timestamp in seconds, convert to milliseconds
    timestamp = offDay * 1000
  } else {
    // String - parse with moment and get timestamp
    timestamp = moment(offDay).valueOf()
  }

  // Convert timestamp to shop timezone to get correct date
  const offDayMoment = moment(timestamp).utcOffset(shopTimezone)

  // Compare year, month, date (ignoring time)
  return offDayMoment.date() === calendarDate.date() &&
         offDayMoment.month() === calendarDate.month() &&
         offDayMoment.year() === calendarDate.year()
}
