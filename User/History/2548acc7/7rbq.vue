  ...
  <Card>
    <template #content>
      ...
      <DataTable
        ...
        :scrollable="true"
        scrollHeight="600px"
        tableClass="branch-prepaid-goods-table"
        dataKey="branch-prepaid-goods-table"
        showGridlines
      >
        <!-- Header -->
        <ColumnGroup type="header">
          <Row>
            <Column
              :rowspan="2"
              :header="$t('branch-prepaid-goods.label-branch')"
              headerClass="bg-gray"
            />
            <Column
              :colspan="2"
            ...
            <Column
              :rowspan="2"
              :header="$t('branch-prepaid-goods.label-total-balance')"
              headerClass="bg-gray"
            />
          </Row>
          ...
          <Row>
            ...
            <Column :header="$t('branch-prepaid-goods.label-records')" headerClass="bg-gray" />
            <Column
              :header="$t('branch-prepaid-goods.label-remaining-qty')"
              headerClass="bg-gray"
            />
            <Column :header="$t('branch-prepaid-goods.label-balance')" headerClass="bg-gray" />
        ...
        <Column>
          <template #body="slotProps">
            {{ handleDisplayRemainingQty(slotProps.data) }}
          </template>
        </Column>
        <Column>
          <template #body="slotProps">
        ...
        <ColumnGroup v-if="transformedData.length > 1" type="footer">
          <Row>
            ...
            <Column
              :footer="formatAmount(lastItem?.prepaidCards?.records || 0)"
              footerClass="bg-gray"
            />
            <Column
              :footer="formatAmount(lastItem?.prepaidCards?.balance || 0)"
              footerClass="bg-gray"
            />
            <Column
              :footer="formatAmount(lastItem?.discountCards?.records || 0)"
              footerClass="bg-gray"
            />
            <Column
              :footer="formatAmount(lastItem?.discountCards?.balance || 0)"
              footerClass="bg-gray"
            />
            <Column
              :footer="formatAmount(lastItem?.prepaidServices?.records || 0)"
              footerClass="bg-gray"
            />
            <Column
              :footer="formatAmount(lastItem?.prepaidServices?.remainingQty || 0)"
              footerClass="bg-gray"
            />
            <Column
              :footer="formatAmount(lastItem?.prepaidServices?.balance || 0)"
              footerClass="bg-gray"
            />
            <Column
              :footer="formatAmount(lastItem?.unlimitedPrepaidServices?.records || 0)"
              footerClass="bg-gray"
            />
            <Column footerClass="bg-gray">
              <template #footer>
                <b>
                  {{
                    lastItem.unlimitedPrepaidServices?.records
                      ? lastItem.unlimitedPrepaidServices?.remainingQty === -1
                        ? $t('general.no-limit')
                        : formatAmount(lastItem.unlimitedPrepaidServices?.remainingQty || 0)
                      : ''
                  }}
                </b>
              </template>
            ...
            <Column
              :footer="formatAmount(lastItem?.unlimitedPrepaidServices?.balance || 0)"
              footerClass="bg-gray"
            />
            <Column :footer="formatAmount(lastItem?.totalBalance || 0)" footerClass="bg-gray" />
          </Row>
        </ColumnGroup>
      </DataTable>
    </template>
...
// Types
import type { IBranchPrepaidGoodsReportItem } from '@/services/sales/sales-reports/branch-prepaid-goods-report.read'
import type { PrintSection } from '@/types/print'
// Utils
import { formatAmount } from '@/utils/common'

interface Props {
  data?: IBranchPrepaidGoodsReportItem[]
}
...
const transformedData = computed<IBranchPrepaidGoodsReportItem[]>(() => {
...
// Display remaining qty
const handleDisplayRemainingQty = (data: IBranchPrepaidGoodsReportItem): string => {
  return data.unlimitedPrepaidServices?.records
    ? data.unlimitedPrepaidServices?.remainingQty === -1
      ? t('general.no-limit')
      : formatAmount(data.unlimitedPrepaidServices?.remainingQty)
    : ''
}
...
:deep(.branch-prepaid-goods-table) {
